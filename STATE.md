...
- Building [0103 — coordinator interview report](design-specs/0103-coordinator-interview-report.md): scoped, read-only reporting of completed native interviews for an explicit admission period, with exact recommendations, scores, historical absence and self-privacy. This does not claim legacy's first-time-only population.

Next:

- Returning-assistant application/classification remains a separate gap. Public-submission provenance is not proof that an applicant has never served; legacy's previous-participation flag is not a public self-report question. Do not infer classification from account links or backfill it without evidence.
