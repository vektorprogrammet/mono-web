...
| Golden school service, with coverage (26 steps)   | `1a583b88`                | `just golden school-service`                                                                                       | `/tmp/vektor-placements-0096-MfyKzF/receipt.json`                         | Real providers                                                |
| Golden recruitment (19 steps)                     | `1a583b88`                | `just golden recruitment`                                                                                          | `/tmp/vektor-placements-0096-WZnJl3/receipt.json`                         | Real providers                                                |
| Admission outcomes and substitutes on call        | `1a583b88`                | `bun run --cwd apps/dashboard e2e:real-substitutes` (12 API and 7 browser gates)                                   | `/tmp/vektor-substitutes-1sjqzD/evidence.json`                            | Outcome messages, real providers                              |
| Coverage record upgrade (migrations 72 to 75)     | `1a583b88`                | Throwaway proof: `1961c14b` seeds offers, then PGlite 18.3, PostgreSQL 18.6 and 17.11 upgrade                      | `/tmp/vektor-substitutes-upgrade-proof/{pglite,pg18,pg17}-evidence.json`  | Production data                                               |
...
- Hosted `Tests` run `36191536836` passed every job at `6c812703`, including the PostgreSQL 17 lane.
  The `Browser journeys` matrix runs every `just golden`, `just e2e`, and `just proof` name that no other hosted job runs; `just layout write` generates its legs from the justfile ([hosted journeys](docs/web-system-functional-testing.md#hosted-journeys)).
  ...
  The legs added since, golden reimbursement, e2e interview-response, and proof rule-reconciliation, have passed locally only. `just proof authorization-rules` now passes its migration preflight but fails later, so it stays excluded ([hosted journeys](docs/web-system-functional-testing.md#hosted-journeys)).
...
- Placement drafts take a new applicant's application availability once the interview is conducted, as the legacy scheduler took every interviewed applicant (`packages/database/src/placements/draft.ts`).
  Admission outcomes are now recorded. Decide whether supply follows the Admitted outcome; the system document announces outcomes after allocation.
- Drafts show school wishes but do not use them to choose a school (follow-up: use wishes in scoring). Scoring needs one teaching-language registry first:
  applications and returning registrations use `AssistantLanguageSchema` (`packages/domain/src/application/availability.ts`), and schools `SchoolLanguageSchema` (`packages/domain/src/schools/schema.ts`).
...
- Admission outcomes are recorded by hand in `/dashboard/vikarer` by department admission management (`admissions.outcomes.decide`); they are not derived from interview recommendations.
  Rekruttering decides outcomes via a future delegation (O8-12); outcome messages are not sent.
...
| Year of study owner (AD7)                | Admissions owns the application's year of study. Done 2026-09-26: the substitute pool, its other writer, is removed.                                                                                                                                                                                                                                    | S               |
...
| Authority model (O8-5 to O8-10)          | Add RosterMember and Applicant role types; scheduling membership is a named requirement; payment destinations become Economy data; contact stays anonymous with its quota; school contacts stay data.                                                                                                                                                   | M               |
...
| Package layout                           | Layer-first packages with CML context folders in every layer; a context's service owns complete commands, locks included. Done 2026-09-26 for Placements: `packages/placements` folded into `packages/domain/src/placements` and `packages/database/src/placements`.                                                                                    | M               |
| Substitutes scope (interview)            | Substitute is an admission outcome: admitted but unplaced applicants are on call. Record absences and who covered each lesson date; coordination stays in Slack. Remove the offer and dispatch flow. Done 2026-09-26: Admissions records the outcome; Placements records coverage and derives attendance; the pool and offers are removed.              | M               |
...
Recruitment keeps recommendation, invitation, account claim, affiliation, and placement separate. The admission outcome is a separate recorded decision.
