...
it("preserves file-effect identity across restart and retries an interrupted reservation", async () => {
  ...
  const staged = await first.stageBytes(
  ...
  expect((await stat(config.stagingRoot)).mode & 0o777).toBe(0o700);
  expect((await stat(join(config.stagingRoot, staged.file.fileRef))).mode & 0o777).toBe(0o600);

  const request = receiptOutboxRequest(commandId, "receipt-one", "PromoteReceiptFile", staged.file);
  await expect(Effect.runPromise(first.service.apply(request))).rejects.toBeInstanceOf(
    ReceiptFileInjectedFailure,
  );
  ...
  await Effect.runPromise(restarted.service.apply(reordered));
  expect(await restarted.readCommitted(staged.file, 1024)).toEqual(Buffer.from(bytes));
  expect((await stat(config.committedRoot)).mode & 0o777).toBe(0o700);
  expect((await stat(join(config.committedRoot, staged.file.objectKey))).mode & 0o777).toBe(0o600);
  const conflict = { ...request, receiptId: "different-receipt" };
  await expect(
    Effect.runPromise(makeReceiptFileStore(config).service.apply(conflict)),
  ).rejects.toBeInstanceOf(ReceiptFileEffectConflict);
  const markers = await readdir(join(config.committedRoot, ".effects"));
  expect(markers).toHaveLength(1);
  expect((await stat(join(config.committedRoot, ".effects", markers[0]!))).mode & 0o777).toBe(
    0o600,
  );
});
...
it("reserves one identity when conflicting callers overlap", async () => {
  ...
  const results = await Promise.allSettled([
    ...
    Effect.runPromise(
      makeReceiptFileStore(config).service.apply({ ...request, receiptId: "receipt-b" }),
    ),
  ]);
