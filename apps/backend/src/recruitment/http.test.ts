...
describe("native recruitment HTTP boundary", () => {
  it.live("accepts one bounded JSON object and rejects invalid transport bodies", () =>
    Effect.gen(function* () {
      const http = ingress();
      ...
      expect(
        yield* problemOf(
          yield* http.fetch(
            invitation(":reject", { body: '{"message":"first","message":"second"}' }),
          ),
        ),
      ...
      expect(
        yield* problemOf(yield* http.fetch(invitation(":reject", { body: streamed }))),
      ).toEqual({
        status: 413,
  ...
  it.live("applies strong validators only after the private representation exists", () =>
    Effect.gen(function* () {
      const http = ingress();
      const etag = invitationETag(snapshot.source);
      const fresh = yield* http.fetch(invitation(""));
      ...
      expect({
        ...
        body: yield* Effect.promise(() => fresh.json()),
      }).toEqual({
        status: 200,
        etag,
      ...
      const notModified = yield* http.fetch(invitation("", { headers: { "if-none-match": etag } }));
      ...
      expect({
        ...
        body: yield* Effect.promise(() => notModified.text()),
      }).toEqual({
        status: 304,
        etag,
        cacheControl: "private, no-store",
      ...
      expect(
        yield* problemOf(
          yield* http.fetch(invitation("", { headers: { "if-match": '"vkr2.stale"' } })),
        ),
    ...
    Effect.gen(function* () {
      ...
      for (const [header, credential] of [
        ...
        for (const [action, body] of operations) {
          ...
          expect({ action, header, ...(yield* answer(response)) }).toEqual({
            action,
            header,
            status: 401,
      ...
      expect(
        yield* answer(yield* http.fetch(invitation("", { headers: { cookie: "theme=dark" } }))),
      ).toEqual({ status: 200 });
    }),
  );
  ...
  it.live("maps recruitment failures to the frozen RFC 9457 problem vocabulary", () =>
    Effect.gen(function* () {
      const http = ingress();
      ...
      const cases = [
        ...
        [
          new RecruitmentDecodeError({ message: "fixture" }),
          schedulingBoard,
          500,
          "internal.error",
        ],
      ...
      for (const [failure, request, status, code] of cases) {
        ...
        expect({
          ...
          ...(yield* problemOf(yield* http.fetch(request()))),
        }).toEqual({
          failure: failure._tag,
          status,
          code,
      ...
      const unauthenticated = yield* ingress(() =>
      ...
      expect(yield* problemOf(unauthenticated)).toEqual({
        status: 401,
        code: "credential.invalid",
      });

      expect(
        yield* problemOf(yield* http.fetch(person("/api/recruitment/interviews?x=1"))),
      ).toEqual({
        status: 400,
  ...
  () =>
    Effect.gen(function* () {
      ...
      for (const code of ["40001", "40P01"]) {
        ...
        expect(yield* problemOf(yield* http.fetch(interviewReport()))).toEqual({
          status: 409,
