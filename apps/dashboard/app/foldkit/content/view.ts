...
const stateView = (model: Model, h: HtmlBuilder<Message>, h2: Html): Html => {
  if (model.workspace._tag === "Loading") {
    return h.p([h.Role("status"), h.Class("content-workspace__loading")], [
...
const rows = (model: Model, h: HtmlBuilder<Message>): Html => {
  ...
  if (entries.length === 0) {
    return h.p([h.Role("status"), h.Class("content-workspace__empty")], [
...
export const view = (model: Model, h: HtmlBuilder<Message>): Html =>
  h.section([
    ...
    h.header([h.Class("content-workspace__header")], [
      ...
      bannerView(model, h),
      model.workspace._tag === "Loading"
        ? stateView(model, h, h.empty)
        : rows(model, h),
    ]),
    h.section(
      [
      ...
      [
        ...
        h.button(
          [
            h.Type("button"),
            h.OnClick(DismissedBanner()),
            h.Class("content-workspace__dismiss"),
          ],
          ["Lukk melding"],
        ),
      ],
