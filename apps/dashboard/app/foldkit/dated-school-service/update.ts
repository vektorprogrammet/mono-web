...
export const update = (model: Model, message: Message): Update.Return<Model, Message> =>
  Match.value(message).pipe(
    ...
    Match.tagsExhaustive({
      ...
      SelectedDecision: ({ value }) => ({ model: { ...model, decision: value, reason: "", evidenceSource: "" }, commands: [] }),
      ...
      SelectedCommitment: ({ commitmentId }) => ({ model:
        { ...model, selectedCommitmentId: commitmentId, decision: "CompleteService", evidenceSource: "", reason: "" }, commands: [] }),
    }),
  );
