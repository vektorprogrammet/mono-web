...
const decodeObservation = (responseState: "Pending" | "Accepted" | "Rejected" | "RequestedNewTime") =>
  S.decodeUnknownSync(InvitationResponseObservationSchema)({
    scheduledAt: "2031-09-20T13:30:00.000Z",
    room: "K-101",
    campus: "Gløshaugen",
    responseState,
    responseMessage: responseState === "RequestedNewTime"
      ? "Kan vi avtale et annet tidspunkt?"
      : null,
...
it("executes the mutation before the mandatory fresh read", async () => {
  ...
  const client: InvitationResponseClient = {
    recruitmentInvitationResponses: {
      confirm: () => Effect.sync(() => { operations.push("confirm") }),
      read: () => Effect.sync(() => {
