...
export const makeUpdate = ({ LoadAssignmentBoard, AssignApplicant }: RecruitmentCommands) => (
  model: Model,
  message: Message,
    ...
    M.tagsExhaustive({
      SelectedFilter: ({ status }) => {
        ...
        return [
          ...
          [...mapDialogCommands(dialogCommands), LoadAssignmentBoard({ status })],
        ];
      },
      SucceededLoadBoard: ({ board }) => [
        { ...model, board: AssignmentBoardData.Success({ data: board }), feedback: null },
        [],
      ],
      FailedLoadBoard: ({ message }) => [
        {
          ...clearAssignment(model),
      ...
      SelectedSchema: ({ interviewSchemaId }) => {
        ...
        return [{ ...model, selectedInterviewSchemaId: interviewSchemaId, assignmentError: null }, []];
      },
      SubmittedAssignment: () => {
        ...
        if (
          ...
          return [
            { ...model, assignmentError: "Velg både intervjuer og intervjuskjema." },
            [],
          ];
        }
        const board = boardFrom(model);
