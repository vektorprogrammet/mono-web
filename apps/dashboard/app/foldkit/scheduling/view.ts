...
const fieldError = (
  id: string,
  field: ReadyModel["scheduledAt"],
  h: HtmlBuilder<Message>,
): Html =>
  field._tag === "Invalid"
    ? h.p([h.Id(`${id}-error`), h.Class("fs-field-error"), h.Role("alert")], [
        field.errors.join(" "),
      ])
    : h.empty;
...
const scheduleDetails = (
  ...
  return h.dl([h.Class("fs-details")], [
    h.div([], [
      h.dt([], ["Tidspunkt"]),
      h.dd([], [formatInstant(interview.schedule.scheduledAt)]),
    ]),
    h.div([], [h.dt([], ["Rom"]), h.dd([], [interview.schedule.room])]),
    h.div([], [
      h.dt([], ["Campus"]),
      h.dd([], [interview.schedule.campus ?? "Ikke oppgitt"]),
    ]),
    h.div([], [
      h.dt([], ["Kartlenke"]),
      h.dd([], [interview.schedule.mapLink ?? "Ikke oppgitt"]),
    ]),
    h.div([], [
      h.dt([], ["Svar fra søker"]),
...
const interviewCard = (
  ...
  return h.article(
    ...
    [
      ...
      h.dl([h.Class("fs-contact-grid")], [
        h.div([], [h.dt([], ["Søkerens e-post"]), h.dd([], [interview.applicant.email])]),
        h.div([], [h.dt([], ["Søkerens telefon"]), h.dd([], [interview.applicant.phone])]),
        h.div([], [
          h.dt([], ["Intervjuerens e-post"]),
          h.dd([], [interview.interviewer.email]),
        ]),
        h.div([], [
          h.dt([], ["Intervjuerens telefon"]),
          h.dd([], [interview.interviewer.phone]),
        ]),
      ]),
      scheduleDetails(interview, h),
      isScheduled
        ? h.empty
        : h.div([h.Class("fs-card-actions")], [
            actionButton(
              "Planlegg intervju",
...
const successfulBoard = (
  ...
  h.section([h.Class("fs-board"), h.AriaLabelledBy("fs-board-title")], [
    h.div([h.Class("fs-board__heading")], [
      h.div([], [
...
const boardView = (model: ReadyModel, h: HtmlBuilder<Message>): Html =>
  AsyncData.match(model.board, {
    onIdle: () =>
      h.div([h.Class("fs-loading"), h.Role("status"), h.AriaLive("polite")], [
        "Forbereder intervjuoversikten …",
      ]),
    onLoading: () =>
      h.div([h.Class("fs-loading"), h.Role("status"), h.AriaLive("polite")], [
        h.span([h.Class("fs-spinner"), h.AriaHidden(true)], []),
        "Henter intervjuoversikten …",
      ]),
    onRefreshing: (board) => successfulBoard(model, board, h),
    onFailure: (error) =>
      h.section([h.Class("fs-error"), h.Role("alert")], [
        ...
        h.dialog(
          ...
          isVisible && interview !== undefined && interview.schedule === null
            ? [
                ...
                h.div([...panel, h.Class("fs-dialog__panel")], [
                  h.div([h.Class("fs-dialog__heading")], [
                  ...
                  h.dl([h.Class("fs-dialog__authority")], [
                    h.div([], [
                      h.dt([], ["Søker"]),
                  ...
                  model.scheduleError === null
                    ? h.empty
                    : h.p([h.Class("fs-error fs-error--inline"), h.Role("alert")], [
                        model.scheduleError,
                      ]),
                  h.form(
                    [
                    ...
                    [
                      ...
                      mapLink.length === 0
                        ? h.empty
                        : h.p([h.Class("fs-map-preview"), h.Role("status")], [
                      ...
                      h.div([h.Class("fs-dialog__actions")], [
                        ...
                        actionButton(
                          model.isScheduling ? "Lagrer og henter ny oversikt …" : "Lagre og legg i kø",
                          SubmittedSchedule(),
                          model.isScheduling,
...
const readyView = (model: ReadyModel, h: HtmlBuilder<Message>): Html =>
  h.section([h.Class("foldkit-scheduling"), h.AriaLabelledBy("fs-page-title")], [
    h.header([h.Class("fs-page-header")], [
...
const invalidInputView = (h: HtmlBuilder<Message>): Html =>
  h.section([h.Class("foldkit-scheduling fs-error fs-error--fatal"), h.Role("alert")], [
