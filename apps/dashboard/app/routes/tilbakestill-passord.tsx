...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (!Predicate.isString(token) || !token) return data({ state: "InvalidOrExpired" }, { headers });

  if (!Predicate.isString(password) || password.length < 12 || password !== form.get("confirmation"))
...
export default function PasswordReset() {
  ...
  const resetToken = useRef(
    globalThis.window === undefined ? null : new URL(window.location.href).searchParams.get("token"),
  );
...
Match.when("InvalidOrExpired", () => (<p>Lenken er ugyldig eller utløpt.</p>)),
Match.when("OutcomeUnknown", () => (<p>Resultatet er usikkert. Be om en ny lenke før du prøver igjen.</p>)),
Match.orElse((state) => (<Form
            method="post"
