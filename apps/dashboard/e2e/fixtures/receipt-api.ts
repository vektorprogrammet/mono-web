...
async function parseMultipart(request: Request): Promise<{
  form: FormData | null;
  receiptFixtureBody: Extract<ReceiptFixtureBody, { kind: "multipart" }>;
}> {
  try {
...
async function handlePersonalCreate(request: Request, url: URL): Promise<Response> {
  ...
  const numericAmountOre = Predicate.isString(amountOre) ? Number(amountOre) : Number.NaN;
  ...
  if (
    ...
    numericAmountOre <= 0 ||
    !Predicate.isString(receiptDate) ||
    !parsed.receiptFixtureBody.filePresent
  ) {
    record(request, url, parsed.receiptFixtureBody, 422);
  ...
  record(request, url, parsed.receiptFixtureBody, 201);
...
async function handlePersonalUpdate(
  ...
  record(request, url, parsed.receiptFixtureBody, 200);
