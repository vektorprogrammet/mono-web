...
const parseArgs = (args) => {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === "--output") {
      ...
      if (!Predicate.isString(outputPath) || outputPath.length === 0) {
...
export const mergeRuntimeEvidenceRegisters = async (inputBytesList) => {
  ...
  for (const inputBytes of inputBytesList) {
    ...
    for (const receipt of register.receipts) {
      const receiptBytes = canonicalRuntimeEvidenceBytes(buildRuntimeEvidenceRegister([receipt]));
  ...
  return canonicalRuntimeEvidenceBytes(buildRuntimeEvidenceRegister(receipts));
};
