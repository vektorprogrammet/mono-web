...
async function runPostgresSql(sql, environment, label, captureOutput = false) {
  return runCommand(postgresProgram("psql"), postgresSqlArgs(sql, captureOutput), {
    cwd: repositoryRoot,
    env: environment,
    label,
    captureOutput,
...
async function main() {
  ...
  try {
    ...
    assertDurableEvidence(postgresEvidence, privateFile, lifecycle, ownerPersona.personId);
    evidence = {
      topology: {
