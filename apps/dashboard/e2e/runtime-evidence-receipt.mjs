...
export async function emitRuntimeEvidenceReceipts({
  ...
  for (const input of runnerSourceInputBytes) {
    if (input === null || !Predicate.isObjectOrArray(input) || !SOURCE_REF.test(input.sourceRefId)) {
  ...
  const normalizedJourneys = journeys.map((journey) => {
    if (
      journey === null ||
      !Predicate.isObjectOrArray(journey) ||
      !Predicate.isString(journey.journeyRefId) ||
  ...
  const bytes = canonicalRuntimeEvidenceBytes(buildRuntimeEvidenceRegister(receipts));
