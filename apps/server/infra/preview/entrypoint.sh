...
MARIADB_SOCKET="${MARIADB_SOCKET:-/tmp/mariadb-preview.sock}"
if command -v mariadbd >/dev/null 2>&1 && ! pgrep -x mariadbd >/dev/null 2>&1; then
  install -d -o mysql -g mysql "${MARIADB_DATA_DIR:-/var/lib/mysql}" /run/mysqld
  ...
  mariadbd --user=mysql --datadir="${MARIADB_DATA_DIR:-/var/lib/mysql}" --socket="${MARIADB_SOCKET}" --bind-address="${MARIADB_HOST}" --port="${MARIADB_PORT}" >/tmp/mariadb-preview.log 2>&1 &
fi
wait_for_mariadb() {
  i=0
  while ! mariadb-admin ping --socket="${MARIADB_SOCKET}" -u root --silent >/dev/null 2>&1; do
    i=$((i + 1)); [ "$i" -le "${MARIADB_WAIT_ATTEMPTS:-60}" ] || { echo 'MariaDB readiness timeout' >&2; cat /tmp/mariadb-preview.log >&2; exit 1; }; sleep 1
  done
}
wait_for_mariadb
mariadb --socket="${MARIADB_SOCKET}" -u root -e "ALTER USER root@localhost IDENTIFIED BY '${DATABASE_ROOT_PASSWORD}'; CREATE DATABASE IF NOT EXISTS \`${DATABASE_NAME}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci; CREATE USER IF NOT EXISTS '${DATABASE_USER}'@'%' IDENTIFIED BY '${DATABASE_PASSWORD}'; GRANT ALL PRIVILEGES ON \`${DATABASE_NAME}\`.* TO '${DATABASE_USER}'@'%'; FLUSH PRIVILEGES;"
