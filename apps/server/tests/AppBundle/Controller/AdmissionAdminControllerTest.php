...
class AdmissionAdminControllerTest extends BaseWebTestCase
{
    ...
    private function helperTestStatus(string $status, string $button_text, string $flash_text)
    {
        ...
        // interview5 fixture already has PENDING status, so re-scheduling doesn't increase the count
        $this->assertEquals($count_no_answer, $crawler->filter('td:contains("Ingen svar")')->count());
        ...
        // interview5 fixture starts as PENDING (not "Ikke satt opp"), so count_no_setup is unchanged
        $this->assertEquals($count_no_setup, $crawler->filter('td:contains("Ikke satt opp")')->count());
    ...
    private function getResponseCodeFromEmail(KernelBrowser $client)
    {
        ...
        $start = strpos((string) $body, 'intervju/') + 9;
        $messageStartingWithCode = substr((string) $body, $start);
        $end = strpos($messageStartingWithCode, '"');

        return substr((string) $body, $start, $end);
    }
