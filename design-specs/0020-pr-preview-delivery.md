...
2. The first target remains `p20/vektor-p20`: logical target `p20`, Alchemy app `vektor`, Alchemy stage `p20`, canonical hostname `p20.vektor.phibkro.org`, and resource prefix `vektor-p20`. PR #21 is authorized to extend `apps/homepage/src/lib/host.ts` from `pNNN` to `p[1-9][0-9]*`; the exact mapping is frozen in the identity table below. Any other target, host, or stage mapping requires a reviewed revision of this spec.
...
8. The selected first-preview architecture is container-local MariaDB: the PHP/MariaDB image and one `vektor-p20` container instance provide MySQL-compatible relational behavior without an external database provider. No unresolved database-provider decision remains. Exact Cloudflare account, token, remote-state, registry, DNS/TLS, and retention availability remains a preflight blocker, not an architecture ambiguity.
...
11. `vektorprogrammet.no` is forbidden as a preview target, DNS or route target, deployment target, credentialed endpoint, or outbound runtime destination. Existing source references are observed in O-0020-14 only; PR #21 must apply a preview-time host override or an egress block before route/browser evidence, and rendered output, configuration, logs, and evidence must contain no such host.
12. The trusted delivery workflow and all provider-mutating tooling come from the PR #21 base branch. Credential-free CI may build and test the exact head SHA, but it never receives provider credentials, raw source backups, or artifact-store write authority. A credentialed preview is permitted only for a same-repository PR. Fork PRs are CI/plan-only unless an operator explicitly approves that exact head SHA under the same sandbox and egress policy.
...
| O-0020-12 | Existing homepage host mapping uses `vektor.phibkro.org` for development and reserved `p000` local proof. PR #21 explicitly extends the parser grammar to `p[1-9][0-9]*` and freezes `p20.vektor.phibkro.org` as the first preview hostname; PR #21 must add host tests for `p20` and multi-digit stages. | `apps/homepage/src/lib/host.ts:1-47`; `apps/homepage/test/host.test.ts` |
| O-0020-13 | The committed source has existing production-host references that must not become preview targets or outbound requests: Symfony metadata templates, the admin metadata template, the dashboard fallback avatar, and the Symfony user-picture URL builder. PR #21 must override those values for preview or block egress before observation. | `apps/server/templates/base.html.twig`; `apps/server/templates/adminBase.html.twig`; `apps/dashboard/app/routes/dashboard.tsx`; `apps/server/src/App/Identity/Infrastructure/UserService.php` |
...
| Resource prefix / container instance | `vektor-p20` / exactly one |
| Remote-state key | `vektor/p20` |
| Concurrency key | `preview-vektor-p20` |
| Cleanup selector | `app=vektor,stage=p20,pr=21,target=p20` and name prefix `vektor-p20-` |
| Database identity | MariaDB namespace `vektor-p20` inside the isolated container; no external database resource |
...
The plan allow-list is exactly: Alchemy app/stage state, one Worker route, one container-backed Durable Object namespace and migration, one Cloudflare Container instance, one digest-bound PHP+MariaDB image reference, one homepage Website/Worker resource, one dashboard Website/Worker resource, the digest-bound synthetic artifact location, and the target hostname/route support resources. No relational provider, raw-backup source, production route, account-wide resource, or undeclared resource is allowed. Teardown deletes the app, Worker, DO namespace, container, image/namespace resources, homepage, dashboard, route, DNS/TLS mapping, and synthetic artifact resources selected by the exact tags and prefix.
...
|---|---|---|---|
| `Absent` | No active `vektor-p20` PR resources remain, or teardown has independently proved absence | Exact stage, generation, and absence checks are recorded | `Requested`, `NeedsOperator` |
...
| `SeedReady` | Reviewed schema/metadata-only generator emits the accepted synthetic artifact and digest | 65-table manifest, approximately 45,955 rows, cohort/privacy report, named UI states, and scan report exist | `Planned`, `NeedsOperator`, `Failed` |
...
| `Seeding` | Worker, container-backed DO, one Container, homepage, and dashboard resources exist | MariaDB migration replay and deterministic seed initialization run inside `vektor-p20` | `Live`, `NeedsOperator`, `Failed`, `Retiring` |
...
2. Generate exactly the reviewed 65-table schema, including MySQL-compatible types, keys, indexes, constraints, enum/status domains, and representative volume of approximately 45,955 rows. A count or schema mismatch fails closed.
...
- the artifact contains exactly 65 tables and approximately 45,955 rows within the reviewed fixed count tolerance;
...
12. Run generation twice with identical inputs. Compare artifact digest, schema manifest, approximately 45,955-row count, distributions, cohort/privacy report, UI-state report, and generator manifest.
...
15. Run Alchemy plan with explicit app `vektor`, stage `p20`, target, source SHA, image digest, seed digest, remote state, route-contract digest, and exact allow-list. The plan must contain the Worker, container-backed Durable Object namespace/migration, one Cloudflare Container instance, digest-bound PHP+MariaDB image, homepage Website/Worker resource, dashboard Website/Worker resource, synthetic artifact location, target mapping, and only their support resources.
16. Reject a plan containing `vektorprogrammet.no`, a production account/route/credential/outbound destination, D1 or DO SQLite as application database, a second PR stage, an external relational database, account-wide cleanup, mutable image tag, unpinned action, or undeclared resource.
...
18. Apply the plan through trusted base-branch tooling in GitHub Actions. Record operation identifiers, resource tags, image/seed digests, and generation without recording credentials.
19. Boot the one `vektor-p20` container, start MariaDB, run Symfony migration replay, and initialize the synthetic seed deterministically. Health must state that MariaDB is container-local preview-session state and that replacement rehydrates the seed.
...
| E-0020-04 | Alchemy plan, resource graph, tags, digests, and remote-state generation | The exact Worker → DO → Container → Symfony path plus homepage/dashboard resources is deterministic and CI-portable | Runtime reachability or domain semantics |
| E-0020-05 | Synthetic manifest, policy/schema digests, artifact digest, two-run comparison, cohort/privacy/UI-state report | The 65-table approximately 45,955-row seed is deterministic, synthetic, privacy-safe, policy-compliant, and uploadable | Raw-source safety beyond the recorded closed-world checks |
...
| E-0020-11 | Close teardown output and independent absence checks | Preview Worker, DO, Container, image/namespace, homepage, dashboard, route, DNS/TLS, and seed artifact effects are absent after close | Account-wide absence or unrelated stages |
...
| E-0020-13 | PR attempt ledger and separate main-dev ledger | No more than two PR-wide mutating attempts; cancellation and SHA changes do not reset; main-dev is separate | Correctness of provider internals |
| E-0020-14 | Final path, secret, raw-data, and production-host scan | Only allowed implementation paths changed; no sensitive data or forbidden host entered repository/evidence | Secrets held outside repository |
...
- PR #21 does not stack on PR #20, or the identity table, exact target, host grammar, stage, prefix, concurrency key, or cleanup selector differs.
...
- The stage, target, source SHA, image digest, seed digest, route-contract digest, or remote-state key is implicit, mutable, or shared with another pull request.
...
- A plan, request, runtime egress, rendered output, configuration, log, evidence item, DNS/route/deployment target, or credentialed endpoint names `vektorprogrammet.no`.
- A plan uses D1, Durable Object SQLite, SQLite test storage, or container disk as Symfony's application database authority; or an external relational database is introduced.
- The Worker → container-backed DO → Container → Symfony path, one-container PHP+MariaDB image, homepage resource, or dashboard resource is absent or duplicated.
- Symfony cannot boot against container-local MariaDB, migration replay is omitted/replaced by schema creation, or replacement rehydration is not deterministic.
- The synthetic generator opens raw source, uses source rows/identifiers/timestamps/topology, unknown schema, random/current time/network, fails exact 65-table/approximately 45,955-row/minimum-cohort/UI-state/privacy checks, or emits different identical-run digests.
...
- Close teardown uses a guessed stage, account-wide cleanup, or a different state generation/key; independent absence evidence is missing.
- The orphan reconciler deletes an active, unknown, API-error, non-PR, main-dev, or homepage stage; it deletes without the exact prefix/tags, two observations one hour apart, 24-hour TTL, stable generation, no lease, and allow-listed resources.
## Definition of done
...
2. The exact identity target `p20/vektor-p20` resolves to `https://p20.vektor.phibkro.org`, deploys the exact PR #21 source SHA through GitHub Actions, and passes the authorized host grammar tests.
...
4. Alchemy v2 declares app `vektor`, stage `p20`, the Worker → container-backed DO → Container path, homepage and dashboard resources, explicit deterministic names/tags, and CI-portable remote state.
...
6. The container-backed DO owns routing/lifecycle only. No Durable Object SQLite, D1, SQLite test storage, external relational provider, or container filesystem abstraction is Symfony's application database authority.
7. Full Symfony migration replay and deterministic synthetic seed initialization reach terminal success in MariaDB.
8. The synthetic seed is exactly the reviewed 65-table schema and approximately 45,955 rows, passes deterministic repeated digests, distributions, minimum cohorts, linkage/privacy rules, named UI states, and fail-closed scans, and uploads no raw backup.
9. The PR ledger key is exactly repository+PR+target+stage, proves no more than two total mutating attempts across all SHAs, and proves cancellation did not perform or imply teardown. Main-dev uses a separate ledger and does not consume p20 budget.
...
11. Browser evidence records status, console, pageerror, hydration, navigation, same-origin, egress-denial, and zero-request observations for `vektorprogrammet.no`.
...
13. Close teardown and independent absence checks prove that Worker, DO, Container, image/namespace, homepage, dashboard, route, DNS/TLS, synthetic artifact, and remote-state effects are absent.
14. The orphan reconciler deletes only the exact safe orphan after prefix/tags, authenticated closed-PR observations twice one hour apart, stable remote generation, no lease, allow-listed resources, and 24-hour TTL; active, main-dev, homepage, unknown, and API-error stages remain.
15. No preview evidence, configuration, URL, request, log, or source path uses `vektorprogrammet.no`; pre-existing source references are either overridden or blocked by egress before observation.
16. Trust and preflight blockers are explicitly recorded. Same-repository credentialed deployment only; fork PRs are CI/plan-only unless exact-SHA operator approved. No secret, provider profile, production identifier, raw backup, personal data, or mutable image tag enters repository or evidence.
...
| Allowed action | Create and commit this spec with message `docs(spec): close preview delivery review gates` |
...
| Trusted mutation tooling | Reviewed PR #21 base-branch workflow and pinned action SHAs | PR-head code cannot receive provider credentials or mutate resources. |
...
| Relational database | MariaDB inside the one isolated `vektor-p20` container | Container-local state is preview-session only; DO SQLite, D1, and external providers are not authority. |
...
This document is the complete design contract for automated PR preview delivery in PR #21. It is ready for independent review because the entry architecture is executable: credentialed preview is same-repository only; exact-head CI is credential-free; trusted base-branch tooling consumes digest-bound image and synthetic seed; the mandatory path is Worker → container-backed DO → Container → Symfony with container-local MariaDB; homepage and dashboard resources are explicit; lifecycle, orphan, attempt, route, evidence, and production-boundary laws are frozen. It contains no implementation claim. The first target is `p20/vektor-p20` at `https://p20.vektor.phibkro.org`; `vektorprogrammet.no` is never a preview target or outbound destination. Raw backups remain local and unopened by CI. A maximum of two total PR-wide mutating attempts applies. The remaining nine PRs wait for p20 green.
