...
The projection root separates the nine regular files named by `COMMITTED_PROJECTIONS` from retained journey-evidence directories. An unknown root regular file, `accepted-intent.json`, alias, or symlink blocks read, diff, and write. A top-level directory is retained evidence: readers and writers share one machine-local projection lock, projection replacement holds the exclusive lock through descriptor-relative copy, exchange, verification, and cleanup, and the transaction preserves every retained directory's bytes, entry types, and permission modes. Evidence producers that target this tree MUST acquire the same exclusive lock before opening or writing any retained path and hold it until every descriptor closes; unrelated journey-evidence CLIs reject projection-tree outputs. Fixture falsifiers MAY use an isolated typed intent snapshot, but fixture bytes are not a production authority mechanism.
...
`direction` is `inbound`, `outbound`, or `bidirectional`. Endpoint query values, authorization headers, tokens, payloads, and personal data are not emitted. A nullable endpoint is valid when the external provider and protocol are explicit at an adapter boundary. A literal endpoint is evidence only for the invocation whose argument list contains it; an unrelated literal elsewhere in the method cannot annotate another call. A provider-like receiver or variable token is not itself a call; its matched token must be the parsed callable. Provider-specific resolved symbols and owner classes, plus transport callables such as `fetch`, `curl_*`, mailer, SMS, Google, Slack, or GitHub, are source evidence for their normalized provider and protocol families. A generic HTTP transport adapter normalizes to the `http` protocol; with no vendor name, it uses its exact sanitized resolved adapter or call-site symbol as `provider_ref` and never invents a vendor. Repeated equivalent invocations at one named call-site boundary form one integration row; distinct providers, protocols, endpoints, credential slots, or call-site symbols remain separate. Test, E2E, and fixture source remains in the C0 census but does not define production integration rows. Unknown provider or protocol values remain `unresolved` rather than being classified as local.
...
A source declaration missing the parser edge is `unresolved` with `SOURCE_PARSE_ERROR`. A parsed declaration missing the loader/import edge is `dead_unimported`. Local loader imports must resolve and unreadable local sources are `source_unavailable`; block and flow-map YAML imports are equivalent. A legacy `parameters*.yml` import is environment input rather than a source loader, and a Symfony `@Bundle/...` route import is an external bundle dependency, so neither can invalidate the local loader graph. An `@AppBundle/Controller/` resource is an explicit local controller edge. A legacy class below `src/AppBundle/Command/` that extends `Command` and defines `execute` is a Symfony bundle-discovered command root. Package runtime exports are roots, including an exact generated-output-to-source projection when the adjacent TypeScript config supplies one `outDir`, one included source root, and the projected source file exists. A relative NodeNext `.js`, `.mjs`, or `.cjs` import resolves to an exact existing JavaScript file first, then to an exact `.ts` or `.tsx` source peer; runtime imports retain their immediate importer edge. A runtime row with no source edge is `extra` and `RUNTIME_ONLY_SOURCE`. None of these states is hidden by a count or by a copied inventory.
