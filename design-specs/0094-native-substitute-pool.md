...
Integration tracing confirmed that existing admission-period and receipt actions also pass these SDK envelopes directly to body-only error decoders. Those two decoders now reuse the same canonical projection, preserving validated field pointers and their existing domain messages. Focused tests cover enveloped authentication, stale revisions, validation fields and malformed objects. This repairs the shared error boundary without adding admission or receipt business transitions.
