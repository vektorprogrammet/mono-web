...
Runtime correction: PostgreSQL uses `auth,public` search_path. Migration0032 and placement queries explicitly target canonical `public` tables, including referenced Person/Organization/School/Semester roots. Regression runs the complete migration manifest with auth first and verifies all four placement/affiliation tables are in public and absent from auth; a successful unqualified query is insufficient evidence of schema ownership.
