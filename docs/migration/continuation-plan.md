...
Applicant progress, 2026-09-20: [0107](../../design-specs/0107-authenticated-applicant-progress.md) delivers the authenticated current-semester progress journey. It derives seven states from canonical source facts. It does not store a second status value. The generated SDK and dashboard return every explicitly linked current application. They do not return assessment, contact or capability data. The exact browser/API/PostgreSQL result is in the [acceptance manifest](../../evidence/functional-parity/0107/acceptance-manifest.json). Applicant completion receipts remain a separate journey.
