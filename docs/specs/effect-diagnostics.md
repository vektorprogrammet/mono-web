...
1. Merge branch `refactor/effect-diagnostics-e2-receipt-0926` (worktree `../mono-web-effect-diag-e2-receipt`): an Effect-returning `ReceiptFileStore` on Effect `FileSystem`/`Path` in `receipt/filesystem.ts`, `r2.ts`, `import-snapshot.ts`, and `reviewed-import.ts`, plus `payment-account.ts`, `outbox-drain.ts`, `drain-main.ts`, their unit tests, and the tools call sites. The one planned suppression is `effecttsgo/node-builtin-import` on the `O_NOFOLLOW` open in `import-snapshot.ts`: `FileSystem.open` has no such flag.
2. Merge branch `refactor/effect-diagnostics-e2-delivery-0926` (worktree `../mono-web-effect-diag-e2-delivery`): `deliverJson` on Effect `HttpClient` with `DeliveryFetch` removed, its consumers, `mail/http.ts`, `password-recovery/drain-main.ts`, and the application, receipt delivery, and Cloudflare mail tests.
3. After slice E1 lands its Effect-returning Identity and AuthEngine interfaces: `router.ts` (`BackendAuthHandler` methods return `Effect<…, IdentityEngineError>`, `backendHttpHandler` and `internalBackendHttpHandler` return `Effect<Response>`, and `main.ts` runs them once in `Bun.serve`), the `instance-of-schema` sites in `router.ts`, `authority.ts`, and `http-api/system.ts`, `global-date` in `authority.ts`, and `main.ts` (`process.env` through `Config`, the shutdown sequence as an Effect program).
