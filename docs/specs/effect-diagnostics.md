...
| E2 | `effectNative` rules in `apps/backend` | 450 on `282da16f` | In progress: 273 left on `643d9e76` plus the receipt custody commit; see Remaining steps |
...
- `effecttsgo/node-builtin-import` on the `node:fs/promises` import of `apps/backend/src/receipt/import-snapshot.ts`: Effect `FileSystem.open` has no `O_NOFOLLOW`, and the snapshot reader must refuse a symlink that replaces the checked path at open time.
- `effecttsgo/node-builtin-import` on the `node:fs/promises` import of `packages/database/src/cohort-cli.ts`: for the same reason, `readPrivateCohortJson` opens the private cohort file with `O_RDONLY | O_NOFOLLOW`.
...
5. Done (branch `refactor/effect-diagnostics-e1d-0926`): the cohorts. `person-cohort.ts`, `historical-service-cohort.ts`, `identity-cohort.ts`, and `placements/current-assignment-cohort.ts` (restored from the stash) and `organization/reviewed-cohort.ts` import through `pgQuery` and `pgTransaction`. Every failure other than a cohort failure still becomes `PersistenceFailure` where it did before; the reviewed organization import keeps failing with the raw cause, now `PgQueryError`. The cohort failures are `Data.TaggedError`s with a `message` getter that returns the code. `readPrivateCohortJson` returns an Effect that fails with `invalid()` and keeps its atomic `O_RDONLY | O_NOFOLLOW` open under the suppression listed in Exceptions. The three `run*CohortCli` are Effect programs that read their variables through `Config` (unset reads as empty, which every check refuses) and write the report with `writeCohortReport` (`Schema.fromJsonString(Schema.Unknown)`, the bytes `JSON.stringify` wrote); the `runtime/*-cohort-main.ts` mains run them with `Effect.runPromise`. `organizationImportSourceDigest`, `receiptImportSourceDigest`, and `currentAssignmentImportSourceDigest` are Effects on `FileSystem` and `Path`; `apps/backend/src/receipt/reviewed-import.ts` yields the receipt one, and `tools/e2e/run-legacy-service-cutover.ts` provides `BunServices.layer` (added to the `effect/no-cross-runtime` override of `oxlint.config.ts`; **the wiring branch needs the same entry**). The two `runtime/*-cohort-rehearsal.ts` scripts run their file, command, and configuration steps through `runtime/rehearsal-platform.ts` (Bun services, `ChildProcess`, `Config`). Tool call sites and the four cohort tests run the imports through a local `flow(importX, Effect.runPromise)` alias; the tools' `new Error("InvalidSnapshot")` factories became the matching tagged cohort failures (`global-error-in-effect-failure`).
6. Migration readers: `migrations.ts`, `migration-registry.ts`, `migration-manifest-cli.ts`, `rule-reconciliation-migration-postgres-proof.ts`, `test-support/postgres.ts`, and `auth-schema-generator.config.ts` (2 findings, not listed before). `withPostgresTestDatabase` becomes `(use: (pool) => Effect) => Effect`; then convert its users to the convention: `person-cohort.test.ts`, `historical-service-cohort.test.ts` (drop their step 5 `runPromise` aliases), `password-recovery.test.ts`, and `service-principal-grants-live.test.ts`. Ruling by the lead (2026-09-26, option A): the migration reader moves to Effect `FileSystem` and `Path`, and `DatabaseLive` and `DatabaseTest` require `FileSystem | Path`, because Migrator types a migration's load effect as needing only `SqlClient` and `Database.migrate` has no requirement, so the services are captured when the layer is built. Runtimes are chosen at composition roots, never inside the package: the backend `main.ts` and both `drain-main.ts` provide `BunServices.layer` (E2c agreed that E1 edits those lines; expect conflicts next to E2c's `platformLayer` and `HttpMailLive` lines), `apps/backend/test/postgres.ts` provides `TestPlatform` of `src/test/platform.ts`, and tools that build `DatabaseLive` provide `BunServices.layer` and join the `effect/no-cross-runtime` override. One test-support module of `packages/database` is the only file that declares platform bun (group `{ role: "runtime-adapter", platform: "bun" }` in `oxlint.config.ts` and in the wiring branch): it exports `TestPlatform` (`BunFileSystem.layer` and `BunPath.layer`) and `DatabaseTestLive = DatabaseTest(...).pipe(Layer.provide(TestPlatform))`, with an export-map entry for the tools tests, so each test changes one import. The database proof mains and CLIs, which are composition roots declared node, take their platform from that module.
...
After step 5, 299 findings remain: 21 outside tests and `runtime/` (step 6: 13; step 8: 7 in `src` and the example), 59 in `runtime/`, and 219 in tests.

Stash `e1-subagent-wip-0926` of the shared repository holds sub-agent work for step 7. Its files have not changed on main since the stash base `40de8ee4`, so `git checkout 'stash@{N}' -- <file>` restores them unchanged; step 6 then changes their `DatabaseTest` import:

- Complete: `database.test.ts`, which adds two `effect/no-cross-runtime` findings until step 6 exists.
