...
5. Test harness and tests: `apps/backend/test/postgres.ts` and `test/database.ts` return Effects (a `ManagedRuntime` owns the per-file cluster; the per-test database is released in `afterEach`), and every `*.test.ts` body becomes `it.live`/`it.effect` (the test `async-function`, `global-date`, `new-promise`, and `node-builtin-import` sites). `src/test/native-http.ts` already serves each request with `Effect.acquireUseRelease` around `HttpRouter.toWebHandler`; its temporary `promised` bridge, which lets the tests await each response, goes when the last test that awaits it is converted. `apps/backend/test/runtime.ts` is deleted when its last caller is converted.
