...
| E2 | `effectNative` rules in `apps/backend` | 450 on `282da16f` | In progress: 188 left after router and main (branch `refactor/effect-diagnostics-e2d-0926`); see Remaining steps |
...
5. Test harness and tests (all 188 remaining findings: 10 in the harness, 178 in tests). Every `*.test.ts` body becomes `it.live`, or `it.effect` for a pure test (the `async-function`, `global-date`, `new-promise`, and `node-builtin-import` sites). The harness returns Effects and keeps no promise bridge, so every file that uses it must convert before the backend type-checks: the two writers below share one integration branch, and the second to finish rebases onto the first and runs the suite. Work in progress lies on the local branch `wip/effect-diagnostics-e2d-step5-0926` (`b6bb28e9`, committed without hooks because the backend does not type-check yet); take files from it with `git checkout wip/effect-diagnostics-e2d-step5-0926 -- <file>`. It holds:
   - The harness. `test/postgres.ts` builds the per-file cluster in a `ManagedRuntime` (`beforeAll` builds it, `afterAll` disposes it) and returns `createDatabase`, `run` (primary session), and `compete` (second session) as Effects; each session and the migrated template build with their own memo map, as their own runtimes did. `test/database.ts` memoizes the per-test database with `Effect.cachedInvalidateWithTTL(…, Duration.infinity)`, releases it in `afterEach` (dispose, then drop; failures in one `AggregateError`), and returns `layer` (only `Database`) and `run`. `src/test/native-http.ts` has no `promised` bridge and no `JSON.stringify`: every `fetch` is a `BackendHttpHandler`. `test/runtime.ts` is deleted; its last caller, `authority.schools.test.ts`, runs `it.effect`.
   - `tmp/effectify.ts` (force-added), a type-aware codemod on the TypeScript 6 API (`node_modules/.bun/typescript@6.0.3`; TypeScript 7 has no JS API). It rewrites `async` test bodies into `it.live` programs, `await` of an Effect into `yield*`, `await` of a promise into `yield* Effect.promise(…)`, `Promise.all` into `Effect.all(…, { concurrency: "unbounded" })`, and `expect(p).resolves` into `expect(yield* p)`. Fix its output by hand: an `it.each` case that is not an array keeps one parameter (`(run) =>`); a promise that a test awaited twice becomes one `yield*` with two assertions, because `Effect.flip` twice runs the command twice; a promise started early and awaited later becomes `Effect.forkChild` and `Fiber.join`, `Promise.withResolvers` a `Deferred`, and `try`/`finally` `Effect.ensuring`; a second PostgreSQL session becomes a layer provided with `{ local: true }`; a `yield*` left inside an `Effect.promise` callback moves out; `Schema.decodeUnknownSync` inside a program becomes `yield* Schema.decodeUnknownEffect` (`schema-sync-in-effect`).
   - Converted: `contact/http.test.ts` type-checks after the codemod; `admission/admission-period.http.test.ts`, `admission/submit-application.http.test.ts`, `authority.schools.test.ts`, and `http-api/receipt-transaction.test.ts` are fixed by hand after the last type check. None of them has run. Step 3 converted `router.oauth.test.ts`.

   Remaining files of `apps/backend/src`, with their findings on `592056f6` and their type errors after the codemod:
   - Writer 1: `receipt/http.test.ts` (36; 81 type errors), `router.test.ts` (22), `router.recruitment-authority.test.ts` (10), `worker-support.test.ts` (2; by hand, `it.effect` with the test clock), and `http-semantics.test.ts` (1; by hand).
   - Writer 2: `directory` (11), `organization` (16; `http.test.ts` chains `.then` on a response, 7 errors), `recruitment` (12; `commands.http.test.ts` keeps a `ManagedRuntime` competitor, 3 errors), `content` (7), `onboarding` (7), `team-application` (7; its `TestRequest` type), `http-api/transport.test.ts` (5), `placements` (4; three `yield*` inside `Effect.promise` callbacks), `profile` (4; an `it.each` case shape), `schools` (4), and `admission/returning-registration.test.ts` (1).
