...
- Phase 3 is partial. Reuse rules in force: `no-raw-advisory-lock-sql`, `no-hand-rolled-postgres`, `no-port-probe`, `no-git-history`, `no-dev-server`, `no-literal-window-instant`, `no-json-text-parameter`, and `no-leadership-reach`.
