...
export const PreviewWorker = (homepage: Worker, dashboard: Worker) =>
  Cloudflare.Worker("PreviewWorker", {
    ...
    env: {
      Homepage: homepage,
      Dashboard: dashboard,
      ContainerRuntime: Cloudflare.Container<PreviewContainer>("PreviewContainer", {
        context: new URL("../../../apps/server", import.meta.url).pathname,
        dockerfile: new URL("../../../apps/server/infra/preview/Dockerfile", import.meta.url)
          .pathname,
        ...
        instances: 1,
        instanceType: "lite",
        labels: Object.entries(PREVIEW_TAGS).map(([name, value]) => ({
          name,
          value,
        })),
