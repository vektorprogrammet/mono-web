...
function inputDigests(args) {
  return {
    ...
    routeContractDigest: requireDigest(
...
export function verifySource(args) {
  const result = runTool(VERIFY_TOOL, [
    "--input",
    requireOption(args, "input"),
    "--head-sha",
    requireOption(args, "head-sha"),
    "--archive-digest",
    requireOption(args, "archive-digest"),
    "--repository",
    args.repository ?? IDENTITY.repository,
    ...(args.output ? ["--output", args.output] : []),
  ]);
...
if (isMainModule(import.meta.url)) {
  try {
    ...
    process.stderr.write(
      `trusted preview command failed: ${error instanceof Error ? error.message : String(error)}\n`,
    );
    process.exitCode = 1;
  }
}
