CREATE TABLE IF NOT EXISTS native_http_idempotency_receipts (
...
CREATE INDEX IF NOT EXISTS native_http_idempotency_receipts_identity_request
  ON native_http_idempotency_receipts (identity_sha256, request_sha256);

CREATE INDEX IF NOT EXISTS native_http_idempotency_receipts_redaction
  ON native_http_idempotency_receipts (full_expires_at)
  WHERE state = 'Complete';

CREATE TABLE IF NOT EXISTS profile_http_versions (
  person_id text PRIMARY KEY REFERENCES person_profiles(person_id) ON DELETE CASCADE,
...
INSERT INTO profile_http_versions (person_id, representation_revision)
SELECT person_id, 0
FROM person_profiles
ON CONFLICT (person_id) DO NOTHING;

CREATE OR REPLACE FUNCTION ensure_profile_http_version()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS person_profiles_http_version_insert ON person_profiles;
CREATE TRIGGER person_profiles_http_version_insert
AFTER INSERT ON person_profiles
FOR EACH ROW
EXECUTE FUNCTION ensure_profile_http_version();

CREATE OR REPLACE FUNCTION increment_profile_http_authority_version()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  ON CONFLICT (person_id) DO UPDATE
    SET representation_revision = profile_http_versions.representation_revision + 1;
...
DROP TRIGGER IF EXISTS organization_global_administrator_grants_profile_http_version
  ON organization_global_administrator_grants;
CREATE TRIGGER organization_global_administrator_grants_profile_http_version
AFTER INSERT OR UPDATE OR DELETE ON organization_global_administrator_grants
FOR EACH ROW
EXECUTE FUNCTION increment_profile_http_authority_version();

DROP TRIGGER IF EXISTS organization_memberships_profile_http_version ON organization_memberships;
...
ALTER TABLE admission_period_semesters
  ADD COLUMN IF NOT EXISTS revision integer NOT NULL DEFAULT 0;
ALTER TABLE admission_period_semesters
...
ALTER TABLE admission_period_fields_of_study
  ADD COLUMN IF NOT EXISTS revision integer NOT NULL DEFAULT 0;
ALTER TABLE admission_period_fields_of_study
...
CREATE OR REPLACE FUNCTION increment_native_http_revision()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS admission_period_departments_http_revision ON admission_period_departments;
CREATE TRIGGER admission_period_departments_http_revision
BEFORE UPDATE ON admission_period_departments
FOR EACH ROW
WHEN (OLD IS DISTINCT FROM NEW)
EXECUTE FUNCTION increment_native_http_revision();

DROP TRIGGER IF EXISTS admission_period_semesters_http_revision ON admission_period_semesters;
CREATE TRIGGER admission_period_semesters_http_revision
BEFORE UPDATE ON admission_period_semesters
FOR EACH ROW
WHEN (OLD IS DISTINCT FROM NEW)
EXECUTE FUNCTION increment_native_http_revision();

DROP TRIGGER IF EXISTS admission_period_fields_of_study_http_revision
  ON admission_period_fields_of_study;
CREATE TRIGGER admission_period_fields_of_study_http_revision
BEFORE UPDATE ON admission_period_fields_of_study
FOR EACH ROW
WHEN (OLD IS DISTINCT FROM NEW)
EXECUTE FUNCTION increment_native_http_revision();

CREATE OR REPLACE FUNCTION increment_content_article_department_revision()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND OLD.article_id <> NEW.article_id THEN
    UPDATE content_articles SET revision = revision + 1 WHERE article_id = OLD.article_id;
  END IF;
  RETURN NULL;
END;
$$;

DROP TRIGGER IF EXISTS content_article_departments_http_revision ON content_article_departments;
CREATE TRIGGER content_article_departments_http_revision
AFTER INSERT OR UPDATE OR DELETE ON content_article_departments
FOR EACH ROW
EXECUTE FUNCTION increment_content_article_department_revision();

CREATE TABLE IF NOT EXISTS recruitment_invitation_response_command_receipts (
  command_id text PRIMARY KEY,
  command_sha256 text NOT NULL,
  invitation_id text NOT NULL REFERENCES recruitment_invitations(invitation_id) ON DELETE RESTRICT,
...
CREATE INDEX IF NOT EXISTS recruitment_invitation_response_command_receipts_invitation
  ON recruitment_invitation_response_command_receipts (invitation_id, committed_at);
