CREATE TABLE organization_volunteer_affiliations (
...
CREATE TABLE organization_volunteer_affiliation_audit (
  ...
  FOREIGN KEY(person_id,department_id) REFERENCES organization_volunteer_affiliations(person_id,department_id)
);
CREATE TABLE assistant_placements (
  ...
  school_id bigint NOT NULL REFERENCES schools_directory_schools(school_id),
  ...
  revision integer NOT NULL CHECK(revision > 0),
  FOREIGN KEY(person_id, department_id) REFERENCES organization_volunteer_affiliations(person_id,department_id),
  FOREIGN KEY(school_id, department_id) REFERENCES schools_directory_departments(school_id,department_id)
);
CREATE UNIQUE INDEX assistant_placements_active_unique ON assistant_placements(person_id,school_id,semester_id,block) WHERE active;
CREATE TABLE assistant_placement_audit (
  placement_id text NOT NULL REFERENCES assistant_placements(placement_id),
  revision integer NOT NULL,
  actor_person_id text NOT NULL REFERENCES person_profiles(person_id),
  occurred_at timestamptz NOT NULL,
