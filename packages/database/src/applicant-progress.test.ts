...
const readProgress = () => readApplicantProgress(personId, now);

const jsonText = Schema.encodeEffect(Schema.fromJsonString(Schema.Unknown));

layer(DatabaseTestLive(), { excludeTestServices: true })("applicant progress projection", (it) => {
  it.effect(
    "uses explicit account custody, current-semester facts and completed-conduct precedence",
    () =>
      Effect.gen(function* () {
        ...
        const received = yield* readProgress();
        ...
        const invited = yield* readProgress();
        ...
        const completed = yield* readProgress();
        ...
        expect((yield* readProgress()).applications[0]?.progress).toEqual(
        ...
        expect(yield* jsonText(completed)).not.toMatch(
