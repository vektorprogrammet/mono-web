...
This folder holds AccessControl code under another name. Authorization rule persistence under its older name.
...
- [packages/domain/src/authz](../../../domain/src/authz/AGENTS.md)
...
| -------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Identity | partnership  | Identity publishes the authenticated principal and account usability; AccessControl publishes the decision language and owns the last-usable-global-administrator invariant that account disabling must respect. One transaction, one lock order (administrator set, then person). |

## Entry points

| Import                             | Module               |
| ---------------------------------- | -------------------- |
| `@vektorprogrammet/database/authz` | [index.ts](index.ts) |
