...
it("lets a team leader appoint within the own team and nowhere else in the department", async () => {
  const own = await runtime.runPromise(appoint("reach-appoint-own", neighbour, "reach-team-own"));

  expect(own.commandId).toBe("reach-appoint-own");

  const other = await runtime.runPromise(
    Effect.flip(appoint("reach-appoint-other", member, "reach-team-other")),
  );
...
it("gives a team leader no department-wide mailing recipients", async () => {
