...
layer(suiteSeed.pipe(Layer.provideMerge(suiteLayer)), {
  ...
  it.effect("rejects unknown and missing or ambiguous current semesters", () =>
    Effect.gen(function* () {
      expect(yield* Effect.flip(read("all", SemesterId.make("unknown")))).toMatchObject({
      ...
      expect(yield* Effect.flip(future)).toMatchObject({
...
(semester_id,start_at,end_at) VALUES ('mail-ambiguous','2038-08-15','2038-10-01')`,
      ...
      try {
        expect(yield* Effect.flip(read("all"))).toMatchObject({
          referenceKind: "CurrentSemester",
        });
      } finally {
        yield* Database.use(
  ...
  it.effect(
    "fails rather than returning partial recipients when the contact store is unavailable",
    () =>
      Effect.gen(function* () {
        yield* Database.use(
        ...
        try {
          expect(yield* Effect.flip(read("all", current))).toHaveProperty(
            "_tag",
          ...
          yield* Database.use(
            (sql) =>
              sql`ALTER TABLE mailing_unavailable_contacts RENAME TO person_contact_profiles`,
          );
        }
      }),
  );

  it.effect("rechecks current leadership rather than retaining a prior authorized scope", () =>
    Effect.gen(function* () {
      yield* Database.use(
