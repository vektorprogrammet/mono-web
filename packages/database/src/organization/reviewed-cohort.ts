...
export const organizationImportSourceDigest = async (): Promise<string> =>
  organizationEvidenceDigest(
    await Promise.all([
      ...
      readFile(
        new URL("../../../domain/src/organization/review-classification.ts", import.meta.url),
        "utf8",
      ),
...
const report = async (
  ...
  snapshotKey: string,
  replay: boolean,
): Promise<ReviewedOrganizationReport> => {
  const occurrences = (
    await tx.query<ReviewedOrganizationOccurrence>(
...
export const importReviewedOrganizationCohort = async (
  pool: Pool,
  input: Schema.Json,
  client?: PoolClient,
): Promise<ReviewedOrganizationReport> => {
  ...
  try {
    ...
    for (const occurrence of snapshot.occurrences) {
      const priorDigest = bindings.get(
        JSON.stringify([occurrence.sourceKind, occurrence.sourceId]),
      );
    ...
    const existingSource = async (
      ...
      const prior = (
        await tx.query<{ source_digest: string; target_id: string }>(
    ...
    for (const item of classified.appointments) {
      ...
      if (unitAvailable && membership.teamId !== null)
        unitAvailable = !!(
          await tx.query("SELECT 1 FROM public.organization_teams WHERE team_id=$1 FOR SHARE", [
            membership.teamId,
          ])
        ).rowCount;
