...
describe("claim-fenced outbox transitions", () => {
  it("change nothing and fail with OutboxClaimLost after stale recovery took the claim", async () => {
    ...
    const evidence = await runtime.runPromise(
      Effect.gen(function* () {
        ...
        const recovered = yield* recoverStaleOutboxClaims(sql, probe, "2031-01-01T00:01:00.000Z", {
          status: "Failed",
