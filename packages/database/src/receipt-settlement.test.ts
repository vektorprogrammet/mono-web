...
const suiteLayer = Layer.merge(databaseLayer, EconomyLive.pipe(Layer.provide(databaseLayer)));
...
layer(suiteLayer, { excludeTestServices: true, timeout: "30 seconds" })(
  "Receipt settlement evidence in PGlite",
  (it) => {
    it.effect(
    ...
    `;
          ...
          expect(evidence.pagination.first).toEqual(
            Array.from(
              { length: 50 },
              (_, index) => `settlement-page-${String(index).padStart(3, "0")}`,
            ),
          );
          ...
          expect(evidence.recorded).toMatchObject({
            ...
            settlement: {
              ...
              paymentDestinationFingerprint: sha256Hex(
                new TextEncoder().encode(paymentDestination),
              ),
              externalAuthority: "settlement-test-bank",
