        ...
        Effect.gen(function* () {
          const rows = yield* sql<ClaimedOutboxRow>`
            WITH candidate AS (
              ...
              ORDER BY outbox.attempts, command_receipt.committed_at, outbox.command_id,
                outbox.ordinal
          ...
          const envelope = yield* Schema.decodeUnknownEffect(ReceiptOutboxRequestSchema)(
            row.payload_json,
            ...
            Effect.match({
              onFailure: () => ({
                ...
                failureTag: "ReceiptOutboxDecodeError",
              }),
              onSuccess: (request) =>
                request.effectId === row.effect_id && request.commandId === row.command_id
                  ? { _tag: "Deliverable" as const, request }
                  : { _tag: "Quarantine" as const, failureTag: "ReceiptOutboxEnvelopeMismatch" },
            }),
          );

          if (Predicate.isTagged(envelope, "Quarantine")) {
            yield* quarantineOutboxClaim(
              sql,
              receiptOutbox,
              { effectId: row.effect_id, claimId },
              envelope.failureTag,
            );
...
> =>
  Effect.gen(function* () {
    ...
    Effect.catchTag("OutboxClaimLost", ({ effectId }) =>
      Effect.succeed(ReceiptOutboxDeliveryResult.ClaimLost({ effectId })),
    ),
  );
