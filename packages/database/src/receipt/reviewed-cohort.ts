...
import { storeReceiptImportResult } from "./postgres.js";
...
const AcceptedResultSchema = Schema.Struct({
  _tag: Schema.Literal("AcceptedReceiptImport"),
...
const decodeAccepted = (input: unknown): AcceptedResult => {
  try {
    return Schema.decodeUnknownSync(AcceptedResultSchema)(input, { onExcessProperty: "error" });
  } catch {
    throw invalid("PersistedEvidenceConflict");
  }
};
...
const readLedger = Effect.fnUntraced(function* (
  ...
  const decoded = yield* Schema.decodeUnknownEffect(
    Schema.Struct({ reasons: Schema.Array(Schema.String) }),
...
const resolveEvidence = Effect.fnUntraced(function* (
  ...
  const references = yield* sql<{
    ...
    readonly source_id_mappings: unknown;
  }>`
...
const validatePrepared = (
  ...
  const results = immutable(structuredClone(input));
  ...
  const classified = importLegacyReceipts(resolved.map((item) => {
    ...
    if (!result || result.sourceOccurrence !== 0 || canonicalJson(result.provenance) !== canonicalJson(item.provenance))
      throw invalid("InvalidPreparedResults");
    const accepted = result._tag === "AcceptedReceiptImport" ? decodeAccepted(result) : null;
    if (accepted !== null && (
  ...
  for (const expected of classified) {
    ...
    if (actual.targetSemanticIdentity !== expected.targetSemanticIdentity ||
      (actual._tag === "AcceptedReceiptImport" && canonicalJson(actual) !== canonicalJson(expected)) ||
      (actual._tag === "QuarantinedReceiptImport" &&
        (actual.reconciliation !== "NotApplicable" || actual.reasons.length === 0)) ||
      (actual._tag !== "AcceptedReceiptImport" && actual._tag !== "QuarantinedReceiptImport"))
      throw invalid("InvalidPreparedResults");
  }
  return results;
};
...
export const importReviewedReceiptCohort = Effect.fn("importReviewedReceiptCohort")(function* (
  ...
  return yield* sql.withTransaction(Effect.gen(function* () {
    ...
    const bindings = yield* sql<{
      ...
      readonly accepted_result_json: unknown;
    }>`
    ...
    for (const entry of r.entries) {
      ...
      let reasons: readonly string[] = entry._tag === "Excluded" ? ["ExcludedByReview"] : [];
      ...
      yield* sql`
        INSERT INTO public.receipt_cohort_occurrences (
          ...
          ${sql.json(reasons)}, ${accepted === null ? null : sql.json(accepted)}
        )
      `;
