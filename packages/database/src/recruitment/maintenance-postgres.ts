...
import { DepartmentId, PersonId, type OrganizationPersonAuthority } from "@vektorprogrammet/domain/organization";
import {
  InterviewSchemaId,
...
const fail = (code: RecruitmentMaintenanceFailure["code"]) => new RecruitmentMaintenanceFailure({ code });
const persistence = (cause: unknown) => new RecruitmentPersistenceError({ operation: "recruitment maintenance", message: String(cause), cause });
const mapFailure = <A, E, R>(effect: Effect.Effect<A, E, R>) => effect.pipe(
  Effect.catchTags({ SqlError: persistence, SchemaError: persistence, OrganizationPersistenceError: persistence, OrganizationDecodeError: persistence }),
);
const decodeCommand = Schema.decodeUnknownEffect(RecruitmentMaintenanceCommand, { onExcessProperty: "error" });
const departmentsFor = (authority: OrganizationPersonAuthority) => [...new Set(authority.memberships.filter((entry) => entry.active && entry.teamLeader).map((entry) => entry.departmentId))];
const authorityFor = Effect.fn("Recruitment.maintenanceAuthority")(function* (sql: DatabaseOperations, personId: PersonId, now: string, lock: boolean) {
  const rows = yield* sql<{ enabled: boolean }>`SELECT NOT access_disabled AS enabled FROM auth."user" WHERE id=${personId} ${lock ? sql`FOR SHARE` : sql``}`;
  if (!rows[0]?.enabled) return yield* fail("Denied");
  return yield* resolveOrganizationPersonAuthorityWithSql(sql, personId, now, lock ? "ForShare" : "None");
});

const questionnaireRows = (sql: DatabaseOperations, id: InterviewSchemaId | null, lock: boolean) =>
  SqlSchema.findAll({
    Request: Schema.NullOr(InterviewSchemaId), Result: ManagedQuestionnaire,
    execute: (id) => sql`SELECT s.interview_schema_id AS "interviewSchemaId",s.name,s.active,s.revision,
...
const staffingRows = (sql: DatabaseOperations, scope: ReadonlyArray<DepartmentId> | null, id: string | null, lock: boolean) =>
  SqlSchema.findAll({
    Request: Schema.Void, Result: InterviewStaffing,
    execute: () => sql`SELECT i.interview_id AS "interviewId",i.department_id AS "departmentId",p.first_name || ' ' || p.last_name AS "applicantName",
...
export const readQuestionnaires = (personId: PersonId) => Effect.gen(function* () {
  const sql = yield* Database;
  return yield* sql.withTransaction(Effect.gen(function* () {
    yield* sql`SET TRANSACTION ISOLATION LEVEL REPEATABLE READ, READ ONLY`;
    const authority = yield* authorityFor(sql, personId, DateTime.formatIso(yield* DateTime.now), false);
    ...
    const history = yield* SqlSchema.findAll({ Request: Schema.Void, Result: QuestionnaireHistory,
      execute: () => sql`SELECT command_id AS "commandId",actor_person_id AS "actorPersonId",interview_schema_id AS "interviewSchemaId",reason,revision,
    ...
    return yield* Schema.decodeUnknownEffect(QuestionnaireManagement)({ questionnaires, history });
  }));
}).pipe(mapFailure);

export const readInterviewStaffing = (personId: PersonId) => Effect.gen(function* () {
  const sql = yield* Database;
  return yield* sql.withTransaction(Effect.gen(function* () {
    yield* sql`SET TRANSACTION ISOLATION LEVEL REPEATABLE READ, READ ONLY`;
    ...
    const candidates = yield* SqlSchema.findAll({ Request: Schema.Void, Result: InterviewStaffingManagement.fields.candidates.value,
      execute: () => sql`SELECT DISTINCT p.person_id AS "personId",p.first_name || ' ' || p.last_name AS "displayName",d.department_id AS "departmentId"
    ...
    const history = yield* SqlSchema.findAll({ Request: Schema.Void, Result: InterviewStaffingHistory,
      execute: () => sql`SELECT command_id AS "commandId",actor_person_id AS "actorPersonId",interview_id AS "interviewId",reason,revision,
    ...
    return yield* Schema.decodeUnknownEffect(InterviewStaffingManagement)({ interviews, candidates, history });
  }));
}).pipe(mapFailure);

const authorizeWithSql = Effect.fn("Recruitment.authorizeMaintenance")(function* (sql: DatabaseOperations, command: RecruitmentMaintenanceCommand, personId: PersonId) {
  const staffing = Predicate.isTagged(command, "ChangeInterviewStaffing");
  const people = staffing ? [personId,command.interviewerPersonId,...(command.coInterviewerPersonId === null ? [] : [command.coInterviewerPersonId])] : [personId];
  for (const id of [...new Set(people)].sort()) yield* lockPersonAuthorization(sql,id);
  const identity = staffing ? yield* guardInterviewApplicantIdentity(command.interviewId,personId) : null;
  const now = DateTime.formatIso(yield* DateTime.now);
...
export const maintainRecruitment = (input: RecruitmentMaintenanceCommand, personId: PersonId) => Effect.gen(function* () {
  ...
  return yield* sql.withTransaction(Effect.gen(function* () {
    ...
    const receipts = yield* sql<{ digest: string; result: unknown }>`SELECT command_digest AS digest,result_json AS result FROM public.recruitment_maintenance_command_receipts WHERE actor_person_id=${personId} AND command_id=${command.commandId}`;
    if (receipts[0]) {
    ...
    if (Predicate.isTagged(command,"ChangeInterviewStaffing")) {
      ...
      const result = RecruitmentMaintenanceResult.make({ _tag:"InterviewStaffingChanged",interviewId:command.interviewId,revision:after.revision });
    ...
    if (command.active && command.questions.length === 0) return yield* fail("EmptyActiveQuestionnaire");
    const id = Predicate.isTagged(command,"CreateQuestionnaire") ? InterviewSchemaId.make("questionnaire:"+sha256Hex(canonicalJsonBytes([personId,command.commandId]))) : command.interviewSchemaId;
    const before = Predicate.isTagged(command,"CreateQuestionnaire") ? null : (yield* questionnaireRows(sql,id,true))[0];
    if (before === undefined) return yield* fail("NotFound");
    if (Predicate.isTagged(command,"ReviseQuestionnaire") && before!.revision !== command.expectedRevision) return yield* fail("Stale");
    const after = ManagedQuestionnaire.make({ interviewSchemaId:id,name:command.name,active:command.active,questions:command.questions,questionCount:command.questions.length,revision:before === null ? 0 : before.revision+1,sourceState:command.questions.length === 0 ? "Unavailable" : "Available" });
    if (before === null) {
    ...
    for (const question of command.questions) yield* sql`INSERT INTO public.recruitment_interview_schema_questions(interview_schema_id,question_id,ordinal,prompt,help_text,kind,alternatives) VALUES(${id},${question.questionId},${question.ordinal},${question.prompt},${question.helpText},${question.kind},${sql.json(question.alternatives)})`;
    const result = RecruitmentMaintenanceResult.make({ _tag:"QuestionnaireSaved",interviewSchemaId:id,revision:after.revision });
    yield* sql`INSERT INTO public.recruitment_maintenance_command_receipts(actor_person_id,command_id,command_digest,result_json) VALUES(${personId},${command.commandId},${digest},${sql.json(result)})`;
