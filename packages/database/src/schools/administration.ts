...
import {
  School, SchoolId, SchoolCapacityPlan, SchoolCommand, SchoolCommandResult,
  ManagedSchool, SchoolDirectoryDepartmentSchema, SchoolAdministrationHistory, SchoolCommandFailure, SchoolsPersistenceError,
  schoolManagementDepartments, canManageSchoolDepartments,
} from "@vektorprogrammet/domain/schools";
import { Database, type DatabaseOperations } from "../service.js";
import { lockPersonAuthorization, resolveOrganizationPersonAuthorityWithSql } from "../organization/authority-postgres.js";

const fail = (code: SchoolCommandFailure["code"]) => new SchoolCommandFailure({ code });
const persistence = (cause: unknown) => new SchoolsPersistenceError({ operation: "school administration", message: String(cause), cause });
const decodeCommand = Schema.decodeUnknownEffect(SchoolCommand, { onExcessProperty: "error" });

const authorityFor = Effect.fn("Schools.authority")(function* (sql: DatabaseOperations, personId: PersonId, lock: boolean) {
  if (lock) yield* lockPersonAuthorization(sql, personId);
  const account = yield* sql<{ enabled: boolean }>`SELECT NOT access_disabled AS enabled FROM auth."user" WHERE id=${personId} ${lock ? sql`FOR SHARE` : sql``}`;
  if (!account[0]?.enabled) return yield* fail("Denied");
  const now = DateTime.formatIso(yield* DateTime.now);
  const authority = yield* resolveOrganizationPersonAuthorityWithSql(sql, personId, now, lock ? "ForShare" : "None");
  if (authority.globalAdministrator !== "Active" && schoolManagementDepartments(authority).length === 0) return yield* fail("Denied");
  return authority;
});

const schoolRows = (sql: DatabaseOperations, schoolId: SchoolId | null, lock: boolean) => SqlSchema.findAll({
  Request: Schema.NullOr(SchoolId),
  Result: School.json,
  execute: (id) => sql`SELECT school_id::float8 AS "schoolId", name, contact_person AS "contactPerson", email, phone, language, active, revision
    FROM public.schools_directory_schools WHERE ${id === null ? sql`TRUE` : sql`school_id=${id}`} ORDER BY school_id ${lock ? sql`FOR UPDATE` : sql``}`,
})(schoolId);

const departmentsFor = (sql: DatabaseOperations, schoolId: SchoolId) => SqlSchema.findAll({
  Request: SchoolId,
  Result: Schema.Struct({ departmentId: DepartmentId }),
  execute: (id) => sql`SELECT department_id AS "departmentId" FROM public.schools_directory_departments WHERE school_id=${id} ORDER BY department_id COLLATE "C"`,
})(schoolId).pipe(Effect.map((rows) => rows.map((row) => row.departmentId)));

const capacityRows = (sql: DatabaseOperations, schoolId: SchoolId) => SqlSchema.findAll({
  Request: SchoolId, Result: SchoolCapacityPlan.json,
  execute: (id) => sql`SELECT capacity_id::float8 AS "capacityId", school_id::float8 AS "schoolId", department_id AS "departmentId", semester_id AS "semesterId", monday,tuesday,wednesday,thursday,friday,revision
    FROM public.schools_capacity_plans WHERE school_id=${id} ORDER BY department_id,semester_id`,
})(schoolId);

const authorizeWithSql = Effect.fn("Schools.authorizeCommand")(function* (sql: DatabaseOperations, command: SchoolCommand, personId: PersonId) {
  const authority = yield* authorityFor(sql, personId, true);
  if (command._tag === "CreateSchool") {
  ...
  if (command._tag === "ReplaceSchoolDepartments") {
  ...
  if (command._tag === "CreateCapacity" || command._tag === "ReviseCapacity") yield* sql`SELECT department_id FROM organization_departments WHERE department_id=${command.departmentId} FOR SHARE`;
  ...
  if (command._tag === "CreateCapacity" || command._tag === "ReviseCapacity") {
    if (!canManageSchoolDepartments(authority, [command.departmentId])) return yield* fail("Denied");
    ...
    if (command._tag === "ReplaceSchoolDepartments" && !canManageSchoolDepartments(authority, command.departmentIds)) return yield* fail("Denied");
  }
  return { authority, school, departments };
});

const mapFailure = <A, E, R>(effect: Effect.Effect<A, E, R>) => effect.pipe(
  Effect.catchTags({
...
export const authorizeSchoolCommand = (command: SchoolCommand, personId: PersonId) => Effect.gen(function* () {
...
export const readSchoolManagement = (personId: PersonId) => Effect.gen(function* () {
  const sql = yield* Database;
  return yield* sql.withTransaction(Effect.gen(function* () {
    ...
    const departments = yield* SqlSchema.findAll({ Request: Schema.Void, Result: SchoolDirectoryDepartmentSchema,
      execute: () => sql`SELECT department_id AS "departmentId", name FROM organization_departments WHERE ${all ? sql`TRUE` : sql.in("department_id", scope)} ORDER BY name,department_id`,
    })(undefined);
    const semesters = yield* SqlSchema.findAll({ Request: Schema.Void, Result: Schema.Struct({ semesterId: SchoolCapacityPlan.fields.semesterId, name: Schema.String }),
      execute: () => sql`SELECT semester_id AS "semesterId", semester_id AS name FROM admission_period_semesters ORDER BY start_at DESC,semester_id`,
    })(undefined);
    const schools = yield* SqlSchema.findAll({ Request: Schema.Void, Result: ManagedSchool,
      execute: () => sql`SELECT jsonb_build_object('schoolId',s.school_id,'name',s.name,'contactPerson',s.contact_person,'email',s.email,'phone',s.phone,'language',s.language,'active',s.active,'revision',s.revision) AS school,
    ...
    const history = yield* SqlSchema.findAll({ Request: Schema.Void, Result: SchoolAdministrationHistory,
      execute: () => sql`SELECT a.command_id AS "commandId", a.school_id::float8 AS "schoolId", a.capacity_id::float8 AS "capacityId", a.department_id AS "departmentId", a.actor_person_id AS "actorPersonId", a.action,a.reason,a.revision,
        to_char(a.recorded_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') AS "recordedAt"
        FROM schools_administration_audit a WHERE ${all ? sql`TRUE` : sql`a.scope_department_ids <@ ${scope}::text[] AND cardinality(a.scope_department_ids)>0 AND
...
export const executeSchoolCommand = (input: SchoolCommand, personId: PersonId) => Effect.gen(function* () {
  const sql = yield* Database;
  ...
  return yield* sql.withTransaction(Effect.gen(function* () {
    ...
    const digest = sha256Hex(canonicalJsonBytes({ schema: "SchoolCommand/v1", personId, command }));
    const receipts = yield* sql<{ digest: string; result: unknown }>`SELECT command_digest AS digest,result_json AS result FROM schools_command_receipts WHERE actor_person_id=${personId} AND command_id=${command.commandId}`;
    if (receipts[0]) {
    ...
    let before: Schema.Json = authorized.school === null ? null : { school: authorized.school, departmentIds: authorized.departments };
    let after: Schema.Json;
    if (command._tag === "CreateSchool" || command._tag === "ReplaceSchoolDepartments") {
      const references = yield* sql<{ id: string }>`SELECT department_id AS id FROM organization_departments WHERE ${sql.in("department_id", command.departmentIds)} FOR SHARE`;
      if (references.length !== command.departmentIds.length) return yield* fail("InvalidReference");
    }
    if (command._tag === "CreateSchool") {
      const rows = yield* sql<{ schoolId: number }>`INSERT INTO schools_directory_schools(name,contact_person,email,phone,language,active)
      ...
      revision = 0;
      for (const id of command.departmentIds) yield* sql`INSERT INTO schools_directory_departments(school_id,department_id) VALUES(${schoolId},${id})`;
      after = { school: (yield* schoolRows(sql, schoolId, false))[0]!, departmentIds: command.departmentIds };
    } else {
      schoolId = command.schoolId;
      const school = authorized.school!;
      if (command._tag === "ReviseSchool" || command._tag === "ReplaceSchoolDepartments") {
        if (school.revision !== command.expectedRevision) return yield* fail("Stale");
        revision = school.revision + 1;
        if (command._tag === "ReviseSchool") {
          ...
          scopeDepartments = [...new Set([...authorized.departments, ...command.departmentIds])];
          for (const removed of authorized.departments.filter((id) => !command.departmentIds.includes(id))) {
            const proposalReferences = yield* sql`SELECT proposal_id FROM school_service_proposals WHERE department_id=${removed}
        ...
        after = { school: (yield* schoolRows(sql, schoolId, false))[0]!, departmentIds: yield* departmentsFor(sql, schoolId) };
      } else {
        ...
        const semesters = yield* sql`SELECT semester_id FROM admission_period_semesters WHERE semester_id=${command.semesterId} FOR SHARE`;
        if (semesters.length === 0) return yield* fail("InvalidReference");
        const existing = (yield* capacityRows(sql, schoolId)).find((plan) => plan.departmentId === command.departmentId && plan.semesterId === command.semesterId);
        departmentId = command.departmentId;
        scopeDepartments = [departmentId];
        before = existing ?? null;
        if (command._tag === "CreateCapacity") {
          if (existing) return yield* fail("CapacityExists");
          const rows = yield* sql<{ capacityId: number }>`INSERT INTO schools_capacity_plans(school_id,department_id,semester_id,monday,tuesday,wednesday,thursday,friday)
          ...
          capacityId = yield* Schema.decodeUnknownEffect(SchoolCapacityPlan.fields.capacityId)(rows[0]?.capacityId);
          revision = 0;
        } else {
        ...
        after = (yield* capacityRows(sql, schoolId)).find((plan) => plan.capacityId === capacityId)!;
      }
    }
