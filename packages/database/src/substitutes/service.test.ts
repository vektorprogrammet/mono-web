...
const runtime = ManagedRuntime.make(Layer.merge(database, SubstitutesLive.pipe(Layer.provide(database))));
afterAll(() => runtime.dispose());
const scope = { departmentId: DepartmentId.make("substitutes-department"), semesterId: SemesterId.make("substitutes-semester") };
const applicationId = PublicApplicationIdSchema.make("substitutes-application");
const input = { monday: true, tuesday: false, wednesday: false, thursday: false, friday: false, language: "Norwegian" as const, yearOfStudy: 3 };
const command = (value: SubstituteCommand) => Substitutes.use((service) => service.execute(applicationId, value, () => Effect.void));
beforeAll(() => runtime.runPromise(Database.use((sql) => Effect.gen(function* () {
...
describe("complete substitute commands in the caller transaction", () => {
  ...
  it("rolls preferences and canonical application year back when the caller receipt fails", async () => {
    const before = await runtime.runPromise(Substitutes.use((service) => service.readEntry(applicationId)));
    const failure = await runtime.runPromise(Effect.flip(Database.use((sql) => sql.withTransaction(Effect.gen(function* () {
      yield* command({ action: "edit", input: { ...input, yearOfStudy: 5, tuesday: true } });
      return yield* Effect.fail("receipt-write-failed");
    })))));
    expect(failure).toBe("receipt-write-failed");
    expect(await runtime.runPromise(Substitutes.use((service) => service.readEntry(applicationId)))).toEqual(before);
  });
  ...
  it("retains preferences on deactivation and rejects illegal transitions without changing the applicant profile", async () => {
    const result = await runtime.runPromise(Database.use((sql) => sql.withTransaction(Effect.gen(function* () {
      ...
      const edited = yield* command({ action: "edit", input: { ...input, yearOfStudy: 4, tuesday: true } });
      ...
      const applicants = yield* sql`SELECT year_of_study FROM admission_applicants WHERE applicant_id='substitutes-applicant'`;
