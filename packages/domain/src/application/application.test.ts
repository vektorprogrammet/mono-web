...
describe("public applicant domain", () => {
  it("normalizes identity without changing the exact field set", async () => {
    ...
    expect(normalized).toEqual({ ...input, firstName: "Ada", lastName: "Lovelace", phone: "+47 12345678", email: "ada@example.com" });
  ...
  it("uses normalized canonical bytes for replay identity", () => {
    expect(publicApplicationCommandDigest(input)).toBe(
      publicApplicationCommandDigest({ ...input, firstName: "Ada", lastName: "Lovelace", phone: "+47 12345678", email: "ada@example.com" }),
    );
  });

  it("records only ordered effect metadata and deduplicates delivery state", async () => {
    const requests = makePublicApplicationOutboxRequests(input, application, applicant, applicant.email);
    ...
    const retry = await Effect.runPromise(recordPublicApplicationEffects([requests[0]!], interpreter));
    ...
    expect(Effect.runSync(PublicApplicationSubmitObservationSchema.makeEffect({ _tag: "Submitted", commandId: input.commandId, applicationId: application.id }))).toEqual({
      _tag: "Submitted",
