...
const occurrence: OrganizationSourceOccurrence = {
  occurrenceId: "team-1",
  sourceKind: "TeamMembership",
  sourceId: "1",
  row,
  sourceRowDigest: organizationEvidenceDigest(row),
};
...
it("preserves reviewed titles and suspension while boards cannot grant leadership", () => {
  ...
  const teamOccurrence = {
    ...occurrence,
    row: suspendedRow,
    sourceRowDigest: organizationEvidenceDigest(suspendedRow),
  };

  const reviewed = Schema.decodeUnknownSync(OrganizationReview)({
    ...review,
    memberships: [
      { ...review.memberships[0], sourceRowDigest: teamOccurrence.sourceRowDigest },
      {
        ...review.memberships[0],
        sourceKind: "BoardMembership",
        sourceRowDigest: boardOccurrence.sourceRowDigest,
      },
    ],
  });
  ...
  const snapshot: ReviewedOrganizationSnapshot = {
    ...
    snapshotId: "snapshot",
    sourceWatermark: "watermark",
    transformationRevision: "transform",
    snapshotDigest: "a".repeat(64),
    referenceDigest: "b".repeat(64),
    personSnapshotKey: "c".repeat(64),
    review: reviewed,
    occurrences: [teamOccurrence, boardOccurrence],
    departments: [],
    teams: [{ id: 10, departmentId: 1, name: "Team", active: true }],
    positions: [{ id: 1, name: "Coordinator" }],
    boards: [{ id: 2, name: "National board" }],
    mappings: {
      persons: [{ sourceUserId: "legacy-user:7", personId: PersonId.make("person-canonical") }],
      departments: [
        {
          sourceDepartmentId: "legacy-department:1",
          departmentId: DepartmentId.make("native-department"),
        },
      ],
    },
  };

  const result = classifyReviewedOrganization(snapshot, { "7": "person-canonical" }, [
    {
      id: 1,
      name: "Department",
      shortName: "DEP",
      email: "department@example.invalid",
      city: "City",
    },
  ]);
  ...
  expect(
    result.appointments.map((item) => [
      ...
      item.positionName,
      item.membership.isTeamLeader,
      item.membership.isSuspended,
    ]),
  ).toEqual([
    ["TeamMembership", "Coordinator", true, true],
    ["BoardMembership", "Chair", false, false],
  ]);
});
