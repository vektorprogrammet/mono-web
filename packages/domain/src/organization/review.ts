...
const bounded = <S extends Schema.Constraint>(schema: S) => Schema.Array(schema).pipe(Schema.check(Schema.isMaxLength(10000)));
...
export const OrganizationReview = Schema.Struct({
  ...
  attestedBy: Id,
  evidenceRef: Id,
  memberships: bounded(Schema.Union([
    Schema.Struct({ sourceKind: Kind, sourceId: Id, sourceRowDigest: Digest, decision: Schema.Literal("Excluded"), evidenceRef: Id }),
    Schema.Struct({ sourceKind: Kind, sourceId: Id, sourceRowDigest: Digest, decision: Schema.Literals(["Historical", "Current", "Future"]), startAt: Rfc3339InstantSchema, endAt: Schema.NullOr(Rfc3339InstantSchema), evidenceRef: Id }),
  ])),
});
...
export const organizationEvidenceDigest = (value: Schema.Json): string => sha256Hex(canonicalJsonBytes(value));
...
export const organizationSnapshotDigest = (input: Schema.Json): string => {
  ...
  const snapshot = Object.fromEntries(Object.entries(decoded.value).filter(([key]) => key !== "snapshotDigest"));
...
export const validateOrganizationReview = (input: Schema.Json, occurrences: ReadonlyArray<OrganizationSourceOccurrence>): OrganizationReview => {
  ...
  for (const occurrence of occurrences) {
    ...
    if (keys.has(key) || !entry || entry.sourceRowDigest !== occurrence.sourceRowDigest || occurrence.sourceRowDigest !== organizationEvidenceDigest(occurrence.row)) throw new OrganizationCohortFailure("InvalidReview");
    keys.add(key);
...
export const decodeReviewedOrganizationSnapshot = (input: Schema.Json): ReviewedOrganizationSnapshot => {
