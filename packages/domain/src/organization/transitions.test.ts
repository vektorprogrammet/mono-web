...
it("keeps temporal and suspension dimensions independent", () => {
  ...
  expect(membershipIsActiveAt({ ...current, isSuspended: true }, "2026-08-21T10:00:00.000Z")).toBe(false);
  expect(membershipIsActiveAt({ ...current, isSuspended: true }, "2026-09-21T10:00:00.000Z")).toBe(false);
});
...
it.effect("revises only permitted dimensions and rejects stale or invalid transitions", () => {
  ...
  return Effect.gen(function* () {
    ...
    const suspended = yield* suspendMembership(
      current,
      yield* Schema.decodeUnknownEffect(MembershipRevisionCommandSchema)({
    ...
    const reinstated = yield* reinstateMembership(
      suspended,
      yield* Schema.decodeUnknownEffect(MembershipRevisionCommandSchema)({
...
it.effect("does not accept immutable membership fields through a revision boundary", () =>
  Effect.flip(
    ...
    Effect.tap((failure) => Effect.sync(() => expect(String(failure)).toContain("teamId"))),
  ),
);
