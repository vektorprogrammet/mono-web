  ...
  Database.use((sql) =>
    Effect.gen(function* () {
      const departments = yield* sql<{
        departmentId: DepartmentId;
        name: string;
      }>`SELECT department_id AS "departmentId",name FROM organization_departments ORDER BY name,department_id`;
      const semesters =
        yield* sql`SELECT semester_id AS "semesterId",to_char(start_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') AS "startAt",to_char(end_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') AS "endAt" FROM admission_period_semesters ORDER BY start_at DESC,semester_id`;
...
export const lockPlacementDepartment = (departmentId: DepartmentId) =>
  Database.use((sql) =>
    Effect.gen(function* () {
      const rows =
        yield* sql`SELECT department_id FROM organization_departments WHERE department_id=${departmentId} FOR UPDATE`;
...
export const readOwnAffiliation = (personId: PersonId, departmentId: DepartmentId) =>
  Database.use((sql) =>
    Effect.gen(function* () {
      const departments =
        yield* sql`SELECT department_id FROM organization_departments WHERE department_id=${departmentId}`;
      if (!departments.length) return yield* fail("scope.invalid");
      const rows =
        yield* sql`SELECT status,revision FROM organization_volunteer_affiliations WHERE person_id=${personId} AND department_id=${departmentId}`;
...
export const mutateAffiliation = (
  ...
  Database.use((sql) =>
    Effect.gen(function* () {
      ...
      yield* sql`INSERT INTO organization_volunteer_affiliations(person_id,department_id,status,revision) VALUES(${current.personId},${current.departmentId},${status},${revision}) ON CONFLICT(person_id,department_id) DO UPDATE SET status=EXCLUDED.status,revision=EXCLUDED.revision`;
      yield* sql`INSERT INTO organization_volunteer_affiliation_audit(person_id,department_id,revision,action,actor_person_id,occurred_at) VALUES(${current.personId},${current.departmentId},${revision},${action},${actor},${now})`;
...
export const readPlacementBoard = (scope: PlacementScope) =>
  Database.use((sql) =>
    Effect.gen(function* () {
      const semesters =
        yield* sql`SELECT semester_id FROM admission_period_semesters WHERE semester_id=${scope.semesterId}`;
      if (!semesters.length) return yield* fail("scope.invalid");
      const affiliations =
        yield* sql`SELECT a.person_id AS "personId",a.department_id AS "departmentId",a.status,a.revision,p.first_name AS "firstName",p.last_name AS "lastName" FROM organization_volunteer_affiliations a JOIN person_profiles p USING(person_id) WHERE a.department_id=${scope.departmentId} ORDER BY p.last_name,p.first_name,a.person_id`;
      const placements =
        yield* sql`SELECT x.placement_id AS "placementId",x.person_id AS "personId",x.department_id AS "departmentId",x.semester_id AS "semesterId",x.school_id::double precision AS "schoolId",x.day,x.workdays,x.block,x.active,x.revision,p.first_name AS "firstName",p.last_name AS "lastName",s.name AS "schoolName" FROM assistant_placements x JOIN person_profiles p USING(person_id) JOIN schools_directory_schools s USING(school_id) WHERE x.department_id=${scope.departmentId} AND x.semester_id=${scope.semesterId} ORDER BY x.placement_id`;
      const schools =
        yield* sql`SELECT s.school_id::double precision AS "schoolId",s.name FROM schools_directory_schools s JOIN schools_directory_departments d USING(school_id) WHERE d.department_id=${scope.departmentId} AND s.active ORDER BY s.name,s.school_id`;
