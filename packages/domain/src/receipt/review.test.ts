...
const entry = ReceiptReviewEntry.members[1].make({
  ...
  person: {
    occurrenceId: "person:7",
    sourceUserId: "legacy-user:7",
    personId: PersonId.make("person-7"),
  },
  department: {
    sourceDepartmentId: "legacy-department:1",
...
it("binds raw stale refund dates and account commitments without interpreting either as payment", () => {
  ...
  expect(() =>
    decodeReviewedReceiptSnapshot({
      ...
      review: {
        ...snapshot.review,
        entries: [{ ...entry, payment: { ...entry.payment, commitment: "changed" } }],
      },
    }),
  ).toThrow();
});
...
it("allows an explicitly unresolved owner, never an invented source ownership mapping", () => {
  ...
  const decision = {
    ...entry,
    person: null,
    sourceRowDigest: receiptSourceRowDigest(ownerless),
    payment: { ...entry.payment, commitment: null },
  };
  ...
  const input = {
    ...
    review: {
      ...snapshot.review,
      receiptSourceRevision: receiptSourceRevision([ownerless]),
      entries: [decision],
    },
  };
  ...
  expect(() =>
    decodeReviewedReceiptSnapshot({
      ...
      review: {
        ...snapshot.review,
        entries: [{ ...entry, person: { ...entry.person!, sourceUserId: "legacy-user:8" } }],
      },
    }),
  ).toThrow();
});
...
it("requires explicit timezone shape but leaves impossible calendars to native quarantine", () => {
  ...
  const result = importLegacyReceipt(
    ...
    "receipt-12",
    {
      sourceRepository: snapshot.review.sourceRepository,
      sourceRevision: snapshot.review.sourceRevision,
      snapshotId: snapshot.review.snapshotId,
      sourceWatermark: snapshot.review.sourceWatermark,
      transformationRevision: snapshot.review.transformationRevision,
      sourceDigest: entry.sourceRowDigest,
      destinationIdentity: "receipt-12",
    },
  );

  if (!Predicate.isTagged(result, "QuarantinedReceiptImport"))
    throw new Error("Quarantine required");
...
it("does not infer approval from a legacy refund date", () => {
  ...
  const input = {
    ...
    review: {
      ...snapshot.review,
      receiptSourceRevision: receiptSourceRevision([refunded]),
      entries: [reviewed],
    },
  };
  ...
  ).toMatchObject({ approvedAt: "2026-08-25T12:00:00Z" });
});
...
it("rejects undeclared private values instead of silently including them in evidence", () => {
  ...
  expect(() =>
    decodeReviewedReceiptSnapshot({
      ...
      review: {
        ...snapshot.review,
        entries: [{ ...entry, payment: { ...entry.payment, plaintext: "12345678901" } }],
      },
    }),
  ).toThrow();
});
...
it("normalizes equivalent reviewed timestamps for native persistence and reconciliation", () => {
  ...
  const utc = importLegacyReceipt(
    {
      ...
      refundDate: "2026-08-25T12:00:00Z",
    },
    "receipt-12",
    provenance,
  );
  ...
  const offset = importLegacyReceipt(
    {
      ...source,
      submittedAt: "2026-08-20T13:00:00+02:00",
      refundDate: "2026-08-25T14:00:00+02:00",
    },
    "receipt-12",
    provenance,
  );
  ...
  if (
    !Predicate.isTagged(utc, "AcceptedReceiptImport") ||
    !Predicate.isTagged(offset, "AcceptedReceiptImport")
  )
...
it("rejects arbitrary private strings as quarantine reason evidence", () => {
  ...
  expect(decodeReasons(["UnresolvedOwner", "UnreadableFile"])).toEqual([
    "UnresolvedOwner",
    "UnreadableFile",
  ]);
