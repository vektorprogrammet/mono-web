...
import { canonicalJson, canonicalJsonBytes, sha256Hex } from "./evidence.js";
import {
  REPLAY_SQL,
  type BatchPlan,
  type D1Binding,
...
const appendAcceptedCase = async (): Promise<{
  ...
  try {
    ...
    cases.push(
      await withSeed(async ({ db }) => {
        ...
        const headAfter = await readFixtureHead(db);
        assert(
          headAfter !== null && JSON.stringify(headBefore) === JSON.stringify(headAfter),
          "stale changed head",
        );
    ...
    cases.push(
      await withSeed(async ({ db }) => {
        ...
        assert(
          !outcomeB.ok && errorReason(outcomeB.error) === "STALE_VERSION",
    ...
    cases.push(
      await withSeed(async ({ db }) => {
        ...
        const replay = await runWithTutorD1(
...
const replayCases = async (): Promise<CaseRecord> => {
  ...
  const badRows: ReadonlyArray<BadRow> = [
    ...
    },
    {
      ...
      reasonCode: "REPLAY_FOLD",
      makeEvent: (base) => ({ event: { ...base, correlationId: "corr-0017-other" }, indexed: {} }),
    },
    {
