...
export const isApiPlatformGeneratedRouteName = (value: string | null): boolean =>
  generatedApiPlatformRouteParts(value) !== null;
...
const sourceFailureRef = (
  context: ManifestContext,
  path: string,
  reason: string,
  role: string,
  captureMode: "static" | "runtime" | "generated" = "static",
): string =>
...
const apiResourceSourceRef = (
  ...
  lineEnd: number | null,
  role = "mono_api_resource_authority",
): string =>
  addSourceReference(context, {
    authorityLine: "mono",
    authorityRole: role,
...
const quotedValue = (payload: string, key: string): string | null =>
  quotedValueSafe(payload, key).value;
...
const apiSourcePaths = (context: ManifestContext): string[] =>
  context.scans.mono.files
    .filter(
      (file) =>
        file.availability === "available" &&
        !file.unsafe &&
        (file.path.endsWith(".php") || file.path.endsWith(".inc")),
    )
    .filter(
      (file) =>
        matchesLiteralPattern(file.path, "apps/server/src/App/**/Api/Resource/**/*.php") ||
        matchesLiteralPattern(file.path, "apps/server/src/App/**/Api/State/**/*.php") ||
        matchesLiteralPattern(file.path, "apps/server/src/App/**/Infrastructure/Entity/**/*.php"),
    )
    .map((file) => file.path)
    ...
    for (const attribute of source.matchAll(attributes)) {
      ...
      for (const entry of entries) {
        ...
        declarations.push({
          logicalPath: path,
          ordinal,
          sourceRefIds: sortUnique(sourceRefs),
          resourceClassRef,
          resourceKey,
          operationName,
          method,
...
const apiSignature = (
  operation:
    | Pick<
        ApiDeclaration,
        "resourceClassRef" | "operationName" | "method" | "uriTemplate" | "operationId"
      >
    | RuntimeOperation,
): string =>
  canonicalJson({
...
const runtimeSourceRef = (context: ManifestContext, path: string, role: string): string =>
  addSourceReference(context, {
    authorityLine: "mono",
    authorityRole: role,
    rootRef: "mono",
...
const runtimeOperationFromUnknown = (value: unknown): RuntimeOperation | null => {
  ...
  return {
    method,
    uriTemplate,
    operationId: stringOrNull(operationIdValue, "field"),
    operationName: stringOrNull(operationNameValue, "field"),
    resourceClassRef: stringOrNull(resourceClassValue, "resource"),
    resourceKey: stringOrNull(resourceKeyValue, "resource"),
    providerRef: stringOrNull(providerValue, "resource"),
    processorRef: stringOrNull(processorValue, "resource"),
...
const runtimeFixturePath = (context: ManifestContext): string | null =>
  RUNTIME_FIXTURE_PATHS.find((path) =>
    context.scans.mono.files.some(
...
const unavailableCollector = (
  reason: string,
  exitCode = 127,
  _stdoutBytes: Uint8Array<ArrayBufferLike> = new Uint8Array(),
  _stderrBytes: Uint8Array<ArrayBufferLike> = new TextEncoder().encode(reason),
  executableDigests: RuntimeExecutableDigests = NO_EXECUTABLE_DIGESTS,
...
export const runTrustedPhpCollectorWithServices = (
  ...
  if (stage === null)
    return unavailableCollector(
      ...
      127,
      new Uint8Array(),
      new TextEncoder().encode("COLLECTOR_INPUTS_UNAVAILABLE"),
      executableDigests,
      executableProvenance,
    );
  try {
...
const collectRuntime = (
  ...
  if (allowFixture) {
    if (fixtureInput !== undefined) {
      ...
      const observation = recordRuntimeObservation(context, {
        ...
        arguments: [],
        stdout: canonicalJson(operations),
        stderr: "",
        exitCode: 0,
        result: operations,
        availability: "available",
      ...
      const openApiObservation = recordRuntimeObservation(context, {
        ...
        command: `fixture ${fixturePath}`,
        arguments: [],
        stdout: canonicalJson(openApiPayload),
        stderr: "",
        exitCode: 0,
        result: openApiPayload,
        availability: "available",
        revisionRefId,
        outOfBand: true,
      });
  ...
  if (metadataRun.availability !== "available")
    return unavailable(
      ...
      metadataArgs,
      metadataRun.reason ?? "RUNTIME_UNAVAILABLE",
  ...
  const metadataObservation = recordRuntimeObservation(context, {
    ...
    command: "api-platform-metadata",
    arguments: metadataArgs,
    stdout: canonicalJson(operations),
    stderr: "",
    exitCode: metadataRun.exitCode,
    result: operations,
    availability: "available",
    revisionRefId,
  ...
  if (openApiRun.availability !== "available") {
    ...
    const openApiObservation = recordRuntimeObservation(context, {
      ...
      arguments: openApiArgs,
      stdout: reasonBytes,
      stderr: reasonBytes,
      exitCode: openApiRun.exitCode,
      result: null,
      availability: "unavailable",
      revisionRefId,
  ...
  try {
    ...
    return {
      ...
      sourceRefIds: [sourceRef],
      failures: [
        { status: "source_unavailable", reasonCode: reason, rowIds: [], sourceRefIds: [sourceRef] },
      ],
    };
  }
  ...
  const openApiObservation = recordRuntimeObservation(context, {
    ...
    command: "api:openapi:export",
    arguments: openApiArgs,
    stdout: openApiRun.stdoutBytes,
    stderr: openApiRun.stderrBytes,
    exitCode: openApiRun.exitCode,
    result: openApiPayload,
    availability: "available",
    revisionRefId,
...
const makeStaticRows = (
  ...
  declarations.map((declaration) => {
    ...
    const details: ApiOperationDetails = {
      ...
      operation_name: declaration.operationName,
      method: declaration.method,
      uri_template: declaration.uriTemplate,
      operation_id: declaration.operationId,
      provider_ref: declaration.providerRef,
      processor_ref: declaration.processorRef,
    ...
    return {
      ...
      signature: apiSignature(declaration),
      status,
      observation_kinds: ["static_source"],
      source_ref_ids: declaration.sourceRefIds,
      ...
      mismatch: mismatch(
        "unresolved",
        [],
        sortUnique(reasons)[0] ?? "RUNTIME_OPERATION_UNRESOLVED",
      ),
...
const makeRuntimeRow = (
  ...
  const details: ApiOperationDetails = {
    ...
    processor_ref: operation.processorRef,
  ...
  return {
    ...
    authority_line: "mono",
    canonical_key: canonicalKey,
    signature: apiSignature(operation),
    status: "extra",
    observation_kinds: ["runtime_resolution"],
    source_ref_ids: sortUnique(sourceRefIds),
    revision_ref_ids: [context.scans.mono.revisionRefId],
    runtime_observation_ref_ids: [observation.runtime_observation_ref_id],
    coverage_ref_ids: [],
    accepted_intent_ref_ids: [],
    duplicate_group_id: null,
    mismatch: mismatch("extra", [], "RUNTIME_ONLY_SOURCE"),
...
const declaredValueMatchesRuntime = <Value>(
  declared: Value | null,
  runtime: Value | null,
): boolean => declared === null || declared === runtime;
...
const sameOperation = (
  left: Pick<
    ApiDeclaration,
    "resourceClassRef" | "method" | "uriTemplate" | "operationId" | "operationName"
  >,
...
interface ApiPlatformRouteReconciliation {
  ...
  readonly routeEvidenceByRuntimeIndex: ReadonlyMap<
    number,
    {
      readonly sourceRefIds: readonly string[];
    ...
    routeEvidenceByRuntimeIndex.set(
      ...
      previousEvidence === undefined
        ? {
            ...
            runtimeObservationRefIds: row.runtime_observation_ref_ids,
          }
        : {
            sourceRefIds: sortUnique([...previousEvidence.sourceRefIds, ...row.source_ref_ids]),
            runtimeObservationRefIds: sortUnique([
              ...previousEvidence.runtimeObservationRefIds,
...
const addRouteEvidenceToApiRows = (
  ...
  matchedStaticOperations: ReadonlyMap<number, ApiDeclaration>,
  routeEvidenceByRuntimeIndex: ReadonlyMap<
    number,
    {
      readonly sourceRefIds: readonly string[];
      readonly runtimeObservationRefIds: readonly string[];
  ...
  for (const [runtimeIndex, evidence] of routeEvidenceByRuntimeIndex) {
    ...
    if (linkIndex >= 0) {
      ...
      if (link !== undefined)
        links[linkIndex] = {
          ...link,
          source_ref_ids: sortUnique([...link.source_ref_ids, ...evidence.sourceRefIds]),
        };
    }
  }
};
  ...
  edges.flatMap((edge) => {
    ...
    return toRowIds.length === 0
      ...
      : [
          {
            ...edge,
            edge_id: edgeId(edge.edge_type, edge.from_ref_ids, toRowIds),
            to_row_ids: toRowIds,
          },
        ];
  });
...
const openApiOperationValid = (operation: Record<string, unknown>): boolean => {
  ...
  return Object.entries(operation.responses).every(
    ([status, response]) =>
      ...
      isOpenApiRecord(response) &&
      openApiResponseValid(response),
  );
};
  ...
  for (const [sectionName, section] of Object.entries(components)) {
    ...
    if (
      Object.values(section).some(
        (entry) =>
          !isOpenApiRecord(entry) || (sectionName === "responses" && !openApiResponseValid(entry)),
      )
    )
      return false;
  }
...
const normaliseOpenApiDocument = (payload: unknown): NormalizedOpenApiDocument | null => {
  ...
  for (const path of Object.keys(validated.paths).sort(compareByteOrder)) {
    ...
    for (const [rawMethod, value] of Object.entries(item).sort(([left], [right]) =>
      ...
      operations.push({
        ...
        digest: sha256(
          canonicalJson({
            document_sha256: validated.documentSha256,
            response_schema_sha256: responseDigest,
          }),
        ),
      });
    }
  ...
  return {
    operations: operations.sort(
      (left, right) =>
        compareByteOrder(left.identity, right.identity) ||
        compareByteOrder(left.digest, right.digest),
    ),
    documentSha256: validated.documentSha256,
  };
};
...
const operationSetDigest = (operations: readonly NormalizedOperation[]): string =>
  sha256(canonicalJson(operations));

const reconcileOpenApi = (
  context: ManifestContext,
  ...
  return {
    ...
    status,
    source_manifest_sha256: canCompare ? sourceManifestSha256 : null,
    committed_source_ref_ids: [committedRef],
    regenerated_source_ref_ids: runtime.openApiPayload === null ? [] : runtime.sourceRefIds,
    committed_document_sha256: committedDocument?.documentSha256 ?? null,
    regenerated_document_sha256: regeneratedDocument?.documentSha256 ?? null,
    committed_sha256: committed === null ? null : operationSetDigest(committed),
    regenerated_sha256: regenerated === null ? null : operationSetDigest(regenerated),
    only_committed: onlyCommitted,
    only_regenerated: onlyRegenerated,
...
const openApiValidationFailure = (
  ...
  if (committedResult.status !== "available") {
    return {
      status: "source_unavailable",
      reasonCode: committedResult.reason,
      rowIds: [],
      sourceRefIds: [
        sourceFailureRef(context, OPENAPI_PATH, committedResult.reason, "mono_openapi_projection"),
      ],
  ...
  if (memberScan !== "valid") {
    return memberScan === "duplicate"
      ? {
          ...
          reasonCode: "OPENAPI_SCHEMA_INVALID",
          rowIds: [],
          sourceRefIds: [openApiSourceRef(context)],
        }
      : {
          status: "source_unavailable",
          reasonCode: "OPENAPI_SOURCE_PARSE_ERROR",
          rowIds: [],
          sourceRefIds: [openApiSourceRef(context)],
        };
  }
...
const openApiProjectionRef = (
  operation:
    ...
    | ApiOperationDetails
    | RuntimeOperation,
  projection: readonly NormalizedOperation[] | null,
): string | null => {
  ...
  for (const ref of refs) {
    ...
    if (collector !== null) {
      ...
      if (
        file === undefined ||
        file.availability !== "available" ||
        file.digest !== `sha256:${collector[1]}`
      ) {
        failures.push({
          status: "schema_invalid",
...
const h3Operation = (value: unknown): H3ApiIdentity => {
  ...
  if (
    separator < 1 ||
    tail <= separator ||
    !Number.isSafeInteger(declarationLine) ||
    declarationLine < 1
  )
    return { resourceClassRef: null, operationName: null, declarationLine: null };
  return {
...
const makeH3ApiRow = (
  ...
  return {
    ...
    source_ref_ids: sortUnique(sourceRefs),
    revision_ref_ids: [context.scans.mono.revisionRefId],
    runtime_observation_ref_ids: [],
    coverage_ref_ids: [],
    accepted_intent_ref_ids: [],
    duplicate_group_id: null,
    mismatch: mismatch("unresolved", [], "H3_UNMATCHED_DRIFT"),
    reason_codes: ["H3_DERIVATION_ONLY", "H3_UNMATCHED_DRIFT"],
    related_row_ids: [],
    details,
  };
};

const makeH3RouteRow = (
  context: ManifestContext,
  path: string,
  ordinal: number,
  sourceRefs: readonly string[],
  ...
  const details: MonoRouteDetails = {
    declaration_kind: "imported_route",
...
const h3Edge = (
  edgeName: string,
  edgeType: DerivationEdge["edge_type"],
  fromRefs: readonly string[],
  ...
  return {
    ...
    edge_type: edgeType,
    from_ref_ids: sortUnique(fromRefs),
    to_row_ids: ids,
    derivation: edgeName,
  };
};
...
const addH3Edges = (
  ...
  for (const supportPath of [
    ...
    if (supportPath === H3_SOURCE_MANIFEST_PATH) {
      ...
      failures.push({
        status: "schema_invalid",
        reasonCode: "H3_COLLECTOR_INVALID",
        rowIds: [],
        sourceRefIds: [supportRef],
      });
    }
  ...
  for (const artifact of artifactRows) {
    ...
    const observation: InventoryObservation = {
      ...
      source_ref_ids: artifactObservationRefs,
      value_digest: artifactDigest,
      normative: false,
    ...
    for (const [index, record] of artifact.records.entries()) {
      if (record === null || typeof record !== "object" || Array.isArray(record)) {
        failures.push({
          ...
          reasonCode: "H3_DERIVATION_ONLY",
          rowIds: [],
          sourceRefIds: [artifact.artifactRef],
        });
        continue;
      }
      ...
      if (artifact.kind === "resource") {
        ...
        if (matched.length > 0) {
          ...
          failures.push({
            status: "schema_invalid",
            reasonCode: "H3_DERIVATION_ONLY",
            rowIds: [derived.row_id],
            sourceRefIds: fromRefs,
          });
        }
      } else {
  ...
  return {
    ...
    apiEdges: [...new Map(apiEdges.map((edge) => [edge.edge_id, edge])).values()],
    routeEdges: [...new Map(routeEdges.map((edge) => [edge.edge_id, edge])).values()],
    apiObservations,
    routeObservations,
    sourceRefIds: sortUnique(sourceRefIds),
    failures,
  };
};
...
export const collectApiOperationsWithServices = (
  ...
  const staticMatches = staticRows
    ...
    .sort(
      (left, right) =>
        operationDeclarationSpecificity(right.declaration) -
          operationDeclarationSpecificity(left.declaration) ||
  ...
  for (const { staticRow, declaration } of staticMatches) {
    ...
    rows[staticIndex] = {
      ...
      runtime_observation_ref_ids: [runtime.observation.runtime_observation_ref_id],
      mismatch: mismatch(
        changed ? "changed" : status === "unresolved" ? "unresolved" : "none",
        related,
        reason,
      ),
      reason_codes:
        reason === null ? unresolvedReasons : sortUnique([...unresolvedReasons, reason]),
      related_row_ids: related,
    };
    rows[runtimeRowIndex] = {
      ...runtimeRow,
      status,
      mismatch: mismatch(changed ? "changed" : "none", [staticRow.row_id], reason),
  ...
  if (runtime.openApiObservation !== null)
    observations.push({
      observation_id: observationId(
        "generated_projection",
        runtime.sourceRefIds,
        runtime.openApiObservation.result_sha256,
      ),
      observation_kind: "generated_projection",
      source_ref_ids: runtime.sourceRefIds,
      value_digest: runtime.openApiObservation.result_sha256,
      normative: false,
  ...
  for (const row of rows) {
    ...
    rows[index] = {
      ...row,
      details: { ...details, openapi_projection_ref: projectionRef },
      observation_kinds: row.observation_kinds.includes("generated_projection")
        ? row.observation_kinds
  ...
  addRouteEvidenceToApiRows(
    ...
    runtimeRows,
    matchedStaticOperations,
    routeReconciliation.routeEvidenceByRuntimeIndex,
    links,
  );
  ...
  const inventory = makeEnvelope(
    ...
    rows,
    links,
    [...observations, ...h3.apiObservations],
    h3.apiEdges,
  ...
  for (const row of rows) {
    if (row.status === "changed")
      failures.push({
        ...
        rowIds: [row.row_id, ...row.related_row_ids],
        sourceRefIds: row.source_ref_ids,
      });
    else if (row.status === "unresolved")
      failures.push({
        status: row.reason_codes.includes("RUNTIME_UNAVAILABLE")
          ? "runtime_unavailable"
          : row.reason_codes.includes("UNSAFE_SOURCE")
            ? "source_unavailable"
            : "unresolved",
        reasonCode: row.reason_codes[0] ?? "RUNTIME_OPERATION_UNRESOLVED",
        rowIds: [row.row_id],
        sourceRefIds: row.source_ref_ids,
      });
      ...
      failures.push({
        status: "unresolved",
        reasonCode: "RUNTIME_ONLY_SOURCE",
        rowIds: [row.row_id],
        sourceRefIds: row.source_ref_ids,
      });
  }
  ...
  if (openApiFailure !== null) failures.push(openApiFailure);
    ...
    failures.push({
      status: "stale",
      reasonCode: "STALE_OPENAPI_PROJECTION",
      rowIds: [],
    ...
    failures.push({
      status: "unresolved",
      reasonCode: "OPENAPI_PROJECTION_UNRESOLVED",
      rowIds: [],
      sourceRefIds: reconciliation.committed_source_ref_ids,
    });
  return {
...
export const reportFailuresFromApi = (
  ...
  failures.map((failure) => ({
    failure_id: `failure-${sha256Hex(canonicalJson({ status: failure.status, reason_code: failure.reasonCode, row_ids: sortUnique(failure.rowIds), source_ref_ids: sortUnique(failure.sourceRefIds) }))}`,
    status: failure.status,
    reason_code: failure.reasonCode,
    row_ids: sortUnique(failure.rowIds),
    source_ref_ids: sortUnique(failure.sourceRefIds),
    accepted_intent_ref_ids: [],
  }));
