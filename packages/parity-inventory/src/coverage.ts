...
const decodeIntent = (value: unknown, expectedRevisionIds: readonly string[]): IntentDecodeResult => {
  if (!isRecord(value) || !hasExactly(value, ["schema_version", "intents", "journeys"]) || value.schema_version !== "functional-parity-accepted-intent/v1" || !Array.isArray(value.intents) || !Array.isArray(value.journeys)) return { register: null, issues: [issue("INTENT_SCHEMA_INVALID")] }
