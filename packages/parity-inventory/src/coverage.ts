...
const hasExactly = (value: Record<string, unknown>, required: readonly string[]): boolean =>
...
const isDisposition = (value: unknown): value is AcceptedDisposition =>
  typeof value === "string" && (ACCEPTED_DISPOSITIONS as readonly string[]).includes(value);
...
const parseStep = (
  value: unknown,
  index: number,
): { readonly step: JourneyStep | null; readonly issues: readonly CoverageIssue[] } => {
  const allowed = [
    "step_id",
    "surface",
    "row_ids",
  ...
  if (!isRecord(value) || !hasExactly(value, allowed))
  ...
  if (
    ...
    runtimeEvidenceRefs.some((ref) => !safeIntentScalar(ref, "runtime_evidence_ref"))
  )
...
const parseIntent = (
...
): { readonly intent: AcceptedIntentRecord | null; readonly issues: readonly CoverageIssue[] } => {
  const allowed = [
    "intent_ref_id",
    "intent_revision",
    "intent_digest",
  ...
  if (
    ...
    rowIds.some((rowIdValue) => !ROW_ID.test(rowIdValue)) ||
    kinds.some((kind) => !PARITY_KINDS.has(kind as InventoryKind)) ||
...
const decodeIntent = (
  ...
  if (!validateAcceptedIntentShape(value))
    return { register: null, issues: [issue("INTENT_SCHEMA_INVALID")] };
...
export const tryDecodeAcceptedIntentRegister = (
  value: unknown,
  expectedRevisionIds: readonly string[] = [],
): IntentDecodeResult => decodeIntent(value, sorted(expectedRevisionIds));

export const decodeAcceptedIntentRegister = (
  value: unknown,
  expectedRevisionIds: readonly string[] = [],
...
): { readonly rows: readonly InventoryRow[]; readonly issues: readonly string[] } => {
...
const dispositionFor = (kind: MismatchKind): AcceptedDisposition | null =>
  MISMATCH_DISPOSITIONS[kind] ?? null;
...
const applyDispositions = (
  ...
  for (const intent of register.intents) {
    ...
    if (
      ...
      [...expectedKinds][0] === "none" ||
      FORBIDDEN_INTENT_KINDS.has([...expectedKinds][0] as MismatchKind)
    ) {
      issues.push(
        issue(
...
export const resolveJourneyCoverage = (params: {
  ...
  for (const journey of params.register.journeys) {
    ...
    const journeyRow: InventoryRow = {
      ...
      runtime_observation_ref_ids: sortUnique(
        journey.steps.flatMap((step) => step.runtime_evidence_ref_ids),
      ),
  ...
  const journeyRowsByRef = new Map(
    journeyRows.map((row) => [(row.details as UserJourneyDetails).journey_ref_id, row]),
  );
...
const rowStatusMismatchValid = (row: InventoryRow): boolean => {
  ...
  if (row.status === "accounted")
    return (
      row.mismatch.disposition === MISMATCH_DISPOSITIONS[kind] &&
      row.mismatch.accepted_intent_ref_ids.length > 0
    );
  return (
    row.status === kind &&
...
export const validateCrossArtifactInvariants = (input: CrossArtifactInvariantInput): boolean => {
  ...
  for (const journey of input.register?.journeys ?? []) {
    ...
    const row = input.userJourneyCoverage.rows.find(
      (candidate) =>
        (candidate.details as UserJourneyDetails).journey_ref_id === journey.journey_ref_id,
    );
    if (
      intent === undefined ||
    ...
    for (const step of journey.steps) {
      ...
      if (
        step.canonical_signatures.some(
          (signature) => !rows.some((candidate) => candidate.signature === signature),
        )
      )
        return false;
    }
