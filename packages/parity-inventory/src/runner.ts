...
const crossReferenceValidation = (
  ...
  mismatches: readonly ReportMismatch[],
): boolean => {
  ...
  if (
    inventories.some(
      (inventory) =>
        new Set(inventory.rows.map((row) => row.row_id)).size !== inventory.rows.length,
    )
  )
    return false;
  if (
  ...
  for (const inventory of inventories) {
    for (const row of inventory.rows) {
      if (
        row.source_ref_ids.some((id) => !sourceIds.has(id)) ||
        row.revision_ref_ids.some((id) => !revisionIds.has(id)) ||
    ...
    for (const link of inventory.links) {
      if (
        ...
        link.source_ref_ids.some((id) => !sourceIds.has(id))
      )
        return false;
    }
    ...
    for (const edge of inventory.derivation_edges) {
      if (
        ...
        edge.to_row_ids.some((id) => !rowIds.has(id))
      )
        return false;
    }
  }
...
const reconcileRoutes = (
  ...
  for (const row of legacyRows) {
    if (row.status === "duplicate")
      mismatches.push({
        kind: "duplicate",
        row_ids: [row.row_id, ...row.mismatch.counterpart_row_ids],
        disposition: "none",
        accepted_intent_ref_ids: [],
      });
  ...
  return {
    ...
    mismatches: [...unique.values()].sort(
      (a, b) =>
        compareByteOrder(a.kind, b.kind) ||
        compareByteOrder(a.row_ids.join("\u0000"), b.row_ids.join("\u0000")),
    ),
    links,
  };
};
...
const reportWith = (params: {
  ...
  inventory_artifact_sha256: Object.fromEntries(
    Object.entries(params.artifactBytes)
      .filter(([name]) => name !== "zero-gap-report.json")
  ...
  mismatches: [...params.mismatches].sort(
    (a, b) =>
      compareByteOrder(a.kind, b.kind) ||
      compareByteOrder(a.row_ids.join("\u0000"), b.row_ids.join("\u0000")),
  ),
...
const generateFromContext = (
  ...
  if (preliminaryApi.h3RouteRows.length > 0) {
    ...
    const mergedRows = mono.rows.map((row) => {
      ...
      return h3Row === undefined
        ...
        : {
            ...row,
            observation_kinds: h3Row.observation_kinds,
            source_ref_ids: sortUnique([...row.source_ref_ids, ...h3Row.source_ref_ids]),
            runtime_observation_ref_ids: sortUnique([
              ...row.runtime_observation_ref_ids,
              ...h3Row.runtime_observation_ref_ids,
            ]),
          };
    });
  ...
  const reportMismatches = [
    ...new Map(
      [
        ...
        ...reportMismatchesFromRows(allRows),
  ...
  for (const source of unavailableSources) {
    ...
    failures.push(
      buildFailure(
        "source_unavailable",
        source.failure_reason ?? "SOURCE_UNAVAILABLE",
        [],
        [source.source_id],
      ),
    );
  ...
  for (const failure of preliminary.failures)
    failures.push(
      buildFailure(
        failure.reason_code === "RUNTIME_UNAVAILABLE" ||
          failure.reason_code.startsWith("COLLECTOR_")
          ? "runtime_unavailable"
          : failure.status === "source_unavailable"
            ? "source_unavailable"
            : "unresolved",
        failure.reason_code,
        [],
  ...
  if (reconciliation.status === "stale")
    failures.push(
      buildFailure(
        "stale",
        "STALE_OPENAPI_PROJECTION",
        api.rows.map((row) => row.row_id),
        [],
      ),
    );
  if (reconciliation.status === "unresolved")
    failures.push(
      buildFailure(
        "unresolved",
        "OPENAPI_RECONCILIATION_UNRESOLVED",
        api.rows.map((row) => row.row_id),
        [],
      ),
    );
  for (const row of allRows) {
    if (row.reason_codes.includes("UNSAFE_SOURCE")) {
      failures.push(
        buildFailure("source_unavailable", "UNSAFE_SOURCE", [row.row_id], row.source_ref_ids),
      );
      continue;
    }
    if (row.status === "duplicate")
      failures.push(
        buildFailure(
          ...
          [row.row_id, ...row.mismatch.counterpart_row_ids],
          row.source_ref_ids,
        ),
      );
      ...
      failures.push(
        buildFailure(
          "unresolved",
          row.reason_codes[0] ?? "SOURCE_PARSE_ERROR",
          [row.row_id],
          row.source_ref_ids,
        ),
      );
      ...
      failures.push(
        buildFailure("gaps_found", "DEAD_UNIMPORTED_SOURCE", [row.row_id], row.source_ref_ids),
      );
    else if (row.status === "missing")
      failures.push(
        buildFailure("gaps_found", "MISSING_COUNTERPART", [row.row_id], row.source_ref_ids),
      );
    else if (row.status === "extra")
      failures.push(
        buildFailure("gaps_found", "EXTRA_COUNTERPART", [row.row_id], row.source_ref_ids),
      );
    else if (row.status === "changed")
      failures.push(
        buildFailure(
          "gaps_found",
          row.reason_codes.includes("STATIC_RUNTIME_MISMATCH")
            ? "STATIC_RUNTIME_MISMATCH"
            : "CHANGED_SIGNATURE",
          [row.row_id, ...row.related_row_ids],
          row.source_ref_ids,
        ),
      );
    else if (row.status === "uncovered")
      failures.push(
        buildFailure("gaps_found", "COVERAGE_REF_REQUIRED", [row.row_id], row.source_ref_ids),
      );
  ...
  const schemaValidation =
    ...
    validateInventory(mono) &&
    validateInventory(api) &&
    validateInventory(commandWrites) &&
    validateInventory(scheduledBackgroundWorkflows) &&
  ...
  const report = reportWith({
    ...
    projectionWrite:
      mode === "write"
        ? { status: "blocked", target_ref: PROJECTION_DIRECTORY }
    ...
    deterministicDiff: mode === "write" ? "not_run" : "different",
    schemaValidation,
  ...
  return {
    ...
    c2Rows: [
      ...commandWrites.rows,
      ...scheduledBackgroundWorkflows.rows,
      ...externalIntegrations.rows,
    ],
  };
};
    ...
    if (
      ...
      return yield* Effect.fail(
        new ParityRuntimeError({
          ...
          path: options.root,
          message: "fixture runtime input is only valid in fixture_injection mode",
        }),
      );
    }
        ...
        : options.intentRegisterPath === undefined
          ? yield* Effect.fail(
              new ParityRuntimeError({
                ...
                path: options.root,
                message: "--intent-register is required for diff and write modes",
              }),
            )
          : yield* readPinnedIntentRegisterEffect(
              options.intentRegisterPath,
              options.legacyRoot,
              options.root,
        ...
        : options.evidenceRegisterPath === undefined
          ? yield* Effect.fail(
              new ParityRuntimeError({
                ...
                path: options.root,
                message: "--evidence-register is required for diff and write modes",
              }),
            )
          : yield* readPinnedRuntimeEvidenceRegisterEffect(
              options.evidenceRegisterPath,
              options.legacyRoot,
              options.root,
              PROJECTION_DIRECTORY,
            );
...
const fixtureExpectation = (falsifierId: FalsifierId): FixtureExpectation | null => {
  switch (falsifierId) {
    case "F0_deterministic_replay":
      return { status: "falsifier_passed", reasonCode: "DETERMINISTIC_REPLAY" };
    case "F1_missing_required_source":
      return { status: "source_unavailable", reasonCode: "SOURCE_UNAVAILABLE" };
    case "F2_source_hash_drift":
      return { status: "source_hash_drift", reasonCode: "SOURCE_HASH_DRIFT" };
    case "F3_duplicate_legacy_route":
      return {
        status: "duplicate",
        reasonCode: "DUPLICATE_CANONICAL_IDENTITY",
        routeName: "fixture_duplicate",
    ...
    case "F6_extra_counterpart":
      return {
        ...
        routeName: "fixture_extra",
        routeAuthority: "mono",
      };
    case "F7_method_path_mismatch":
      return {
        status: "gaps_found",
        reasonCode: "CHANGED_SIGNATURE",
        routeName: "fixture_changed",
      };
    case "F8_openapi_stale":
      return { status: "stale", reasonCode: "STALE_OPENAPI_PROJECTION" };
    case "F9_runtime_unavailable":
      return { status: "runtime_unavailable", reasonCode: "RUNTIME_UNAVAILABLE" };
    case "F10_static_runtime_mismatch":
      return {
        status: "unresolved",
        reasonCode: "STATIC_RUNTIME_MISMATCH",
        routeName: "fixture_api",
      };
    case "F11_intent_missing_or_stale":
      return { status: "accepted_intent_invalid", reasonCode: "ACCEPTED_INTENT_MISSING" };
    case "F12_uncovered_journey":
      return { status: "gaps_found", reasonCode: "COVERAGE_REF_REQUIRED" };
    case "F13_unknown_effect":
      return { status: "unresolved", reasonCode: "UNKNOWN_EFFECT" };
    case "F14_absent_schedule":
      return { status: "gaps_found", reasonCode: "ABSENT_SCHEDULE" };
    case "F15_secret_or_pii_input":
      return { status: "source_unavailable", reasonCode: "UNSAFE_SOURCE" };
    case "F16_h3_authority_copy":
      return { status: "schema_invalid", reasonCode: "H3_DERIVATION_ONLY" };
    case "F17_locale_order":
      return { status: "falsifier_passed", reasonCode: "LOCALE_ORDER_CANONICAL" };
    case "F18_stale_artifact_diff":
      return { status: "stale", reasonCode: "STALE_ARTIFACT" };
    case "F19_ignore_residual_precedence":
      return { status: "falsifier_passed", reasonCode: "RESIDUAL_PRECEDENCE" };
    default:
      return null;
  }
};

const syntheticFixtureFiles: readonly {
  readonly root: "legacy" | "mono";
  ...
  {
    root: "legacy",
    path: "src/AppBundle/Controller/Api/FixtureController.php",
    contents: "<?php\nfinal class FixtureApi {}\n",
  },
  {
    root: "legacy",
    path: "src/AppBundle/Controller/FixtureController.php",
  ...
  },
  {
    ...
    contents: JSON.stringify({
      scripts: { runtime: "bun infra/schedules.ts" },
      exports: { ".": { default: "./infra/schedules.ts" } },
    }),
  },
  {
    root: "mono",
    path: "infra/schedules.ts",
    contents:
      "export function FixtureHandler(): void {}\nschedule('fixture_cron', '0 0 * * *', FixtureHandler)\n",
  },
  {
    root: "legacy",
    path: "src/AppBundle/Service/Fixture.php",
    contents: "<?php\nfinal class FixtureService {}\n",
  },
  {
    root: "mono",
    path: "apps/server/config/routes.yaml",
    contents:
      "fixture_base:\n    resource: ../src/App/Fixture/Controller/FixtureController.php\n    path: /fixture/base\n    methods: ['GET']\nfixture_alt:\n    resource: ../src/App/Fixture/Controller/FixtureController.php\n    path: /fixture/alt\n    methods: ['GET']\n",
  },
  {
    root: "mono",
    path: "apps/server/src/App/Api/Resource/Fixture.php",
    contents:
  ...
  {
    root: "legacy",
    path: "src/AppBundle/Repository/FixtureRepository.php",
    contents:
  ...
  {
    root: "legacy",
    path: "app/config/scheduler.yml",
    contents:
      "fixture_cron:\n  cron: '0 0 * * *'\n  handler: AppBundle\\Fixture\\Command\\FixtureCommand\n",
  },
  {
    root: "mono",
    path: "apps/server/src/App/Infrastructure/Command/FixtureCommand.php",
    contents:
  ...
  {
    root: "mono",
    path: "apps/server/src/App/Infrastructure/Repository/FixtureRepository.php",
    contents:
      "<?php\nnamespace App\\Fixture\\Infrastructure\\Repository;\nfinal class FixtureRepository { public function save(string $value): void {} }\n",
  },
  {
    root: "mono",
    path: "apps/server/config/services.yaml",
    contents:
      "services:\n  fixture_command:\n    class: App\\Fixture\\Infrastructure\\Command\\FixtureCommand\n",
  },
  ...
  {
    root: "mono",
    path: "apps/server/tools/security-h3/0015/generate.ts",
    contents: "export const fixture = true\n",
  },
  {
    root: "mono",
    path: "apps/homepage/src/routes/home.tsx",
    contents: "export default function Home(){return null}\n",
  },
  {
    root: "mono",
    ...
    contents: JSON.stringify([
      {
        ...
        methods: ["GET"],
        operation_id: "route:fixture_base",
        source_ref_ids: ["source:apps/server/src/App/Api/Resource/Fixture.php:1:fixture"],
      },
    ]),
  },
  {
    ...
    contents: JSON.stringify([
      {
        ...
        methods: ["GET"],
        operation_id: "api:App\\Fixture\\Api\\Resource\\FixtureResource:Get:1",
        source_ref_ids: ["source:apps/server/src/App/Api/Resource/Fixture.php:1:fixture"],
      },
    ]),
  },
...
const seedFixtureIntentRegister = (
  ...
  const coverageSteps = [
    ...steps,
    {
      step_id: "fixture-step-00-target-row",
      surface: "legacy_route" as const,
      row_ids: targetCoverageRows.map((row) => row.row_id),
...
const c2SecurityFixtureInputs = (): readonly FixtureSourceInput[] => {
  const credential = new TextDecoder().decode(
    Uint8Array.from([
      115, 107, 95, 108, 105, 118, 101, 95, 99, 50, 95, 102, 105, 120, 116, 117, 114, 101, 95, 115,
  ...
  const endpointSecret = new TextDecoder().decode(
    Uint8Array.from([
      ...
      75, 108, 77, 110, 79, 112, 81, 114, 83, 116, 85, 118, 87, 120, 89, 122, 95, 49, 50, 51, 52,
      53,
    ]),
  );
  const tracked = [
      ...
      fileSystem.writeFile(
        ...
        JSON.stringify({
          ...
          paths: {
            "/fixture/api": {
              get: {
                operationId: "fixture_api_changed",
        ...
        JSON.stringify([
          {
            ...
            operation_name: "Get",
            method: "POST",
            uri_template: "/fixture/api-mismatch",
            operation_id: "fixture_api",
          },
        ]),
        "utf8",
...
const fixtureResultReport = (
  ...
  const report = reportWith({
    ...
    falsifierId,
    status: reportStatus,
    exitCode: matched ? 13 : 12,
    projectionWrite: { status: "blocked", target_ref: PROJECTION_DIRECTORY },
    sourceManifestSha256: sourceDigestForManifest(generated.sourceManifest),
    artifactBytes: generated.bytes,
    inventories: [
      generated.legacyRoutes,
      generated.monoRoutes,
      generated.apiOperations,
      generated.commandWrites,
      generated.scheduledBackgroundWorkflows,
      generated.externalIntegrations,
      generated.userJourneyCoverage,
    ],
    failures,
    mismatches: generated.report.mismatches,
    deterministicDiff: deterministicDiff ?? (deterministic ? "equal" : "different"),
    schemaValidation: generated.report.verification.schema_validation,
    ...
    if (falsifierId === undefined || C0_FALSIFIERS[falsifierId] !== true)
      return yield* Effect.fail(
        new ParityRuntimeError({
          operation: "fixture_injection",
          path: options.root,
          message: "fixture_injection requires a registered falsifier",
        }),
      );
    ...
    try {
      if (falsifierId === "F0_deterministic_replay") {
        ...
        try {
          ...
          return fixtureResultReport(
            ...
            first,
            expectation,
            deterministic ? "falsifier_passed" : "command_error",
            deterministic ? expectation.reasonCode : "NONDETERMINISTIC_OUTPUT",
            deterministic,
          );
        } finally {
          yield* Effect.sync(() =>
      ...
      if (falsifierId === "F2_source_hash_drift") {
        ...
        return fixtureResultReport(
          ...
          after,
          expectation,
          drifted ? "source_hash_drift" : "command_error",
          drifted ? expectation.reasonCode : "SOURCE_HASH_DRIFT_NOT_DETECTED",
          drifted,
        );
      }
      if (falsifierId === "F11_intent_missing_or_stale") {
        ...
        return fixtureResultReport(
          ...
          generated,
          expectation,
          deterministic ? (causal?.status ?? "command_error") : "command_error",
          deterministic
            ? (causal?.reason_code ?? "FALSIFIER_CAUSALITY_MISMATCH")
      ...
      if (falsifierId === "F12_uncovered_journey") {
        ...
        return fixtureResultReport(
          ...
          generated,
          expectation,
          changed ? (causal?.status ?? "command_error") : "command_error",
          changed
            ? (causal?.reason_code ?? "FALSIFIER_CAUSALITY_MISMATCH")
            : "FALSIFIER_CAUSALITY_MISMATCH",
          changed,
      ...
      if (falsifierId === "F17_locale_order") {
        ...
        return fixtureResultReport(
          ...
          expectation,
          deterministic ? "falsifier_passed" : "command_error",
          deterministic ? expectation.reasonCode : "LOCALE_ORDER_NONDETERMINISTIC",
          deterministic,
          observation,
        );
      }
        ...
        try {
          ...
          return generated === undefined
            ...
            : fixtureResultReport(
                ...
                generated,
                expectation,
                stale ? "stale" : "command_error",
                stale ? expectation.reasonCode : "FALSIFIER_CAUSALITY_MISMATCH",
                stale,
                stale ? (causal ?? null) : null,
                diffResult.projectionDiff,
      ...
      if (falsifierId === "F19_ignore_residual_precedence") {
        ...
        const residualRefs = [...generated.routeRows, ...generated.apiRows, ...generated.c2Rows]
          ...
          .some(
            (sourceRef) =>
              generated.sourceManifest.sources.find(
        ...
        return fixtureResultReport(
          ...
          expectation,
          deterministic ? "falsifier_passed" : "command_error",
          deterministic ? expectation.reasonCode : "RESIDUAL_PRECEDENCE_BROKEN",
          deterministic,
          observation,
        );
      }
      if (falsifierId === "F16_h3_authority_copy") {
        ...
        return fixtureResultReport(
          ...
          generated,
          expectation,
          passed ? "schema_invalid" : "command_error",
          passed ? expectation.reasonCode : "H3_AUTHORITY_COPY_ACCEPTED",
          passed,
          causal,
        );
      }
      ...
      if (falsifierId === "F13_unknown_effect") {
        ...
        causalMatch =
          ...
          baselineCausalClean &&
          generatedTarget !== undefined &&
          generatedTarget.status === "unresolved" &&
          generatedTarget.reason_codes.includes("UNKNOWN_EFFECT") &&
          causalFailure !== null;
      } else if (falsifierId === "F14_absent_schedule") {
        const generatedAbsentRows = generated.c2Rows.filter(
          (row) =>
            row.authority_line === "mono" &&
            row.inventory_kind === "schedule_background" &&
            row.status === "absent" &&
          ...
          : expectation.routeName === undefined
            ? (generated.failures.find(
                (failure) =>
                  ...
                  failure.status === expectation.status &&
                  mutationChanged &&
                  !baselineFailureIds.has(failure.failure_id),
              ) ?? null)
            : (generated.failures.find(
                (failure) =>
                  failure.reason_code === expectation.reasonCode &&
...
const runTerminalStageEffect = (
  ...
  Effect.gen(function* () {
    ...
    const hasUnsafe =
      ...
      [...generated.routeRows, ...generated.apiRows, ...generated.c2Rows].some((row) =>
    ...
    const forbiddenEmpty = (extraFailures: readonly ReportFailure[]): boolean =>
      forbiddenStatesEmpty(
        inventories,
        generated.openapiReconciliation,
        extraFailures,
        crossReferencesValid,
      );
    ...
    if (options.mode === "write" && !writeDenied) {
      ...
      if (!validateGeneratedArtifactSet(generated)) {
        ...
        report = reportWith({
          ...
          failures,
          mismatches: reportMismatches,
          deterministicDiff: "not_run",
          schemaValidation: false,
          crossReferenceValidation: crossReferencesValid,
          forbiddenStatesEmpty: false,
        });
      } else {
        yield* projectionEffects.writeProjectionSet(
          options.root,
          PROJECTION_DIRECTORY,
          generated.bytes,
        ...
        if (!written) {
          ...
          const writeReport = reportWith({
            mode: "write",
            falsifierId: null,
            status: "projection_written",
            exitCode: 14,
            projectionWrite: { status: "written", target_ref: PROJECTION_DIRECTORY },
            sourceManifestSha256: sourceDigestForManifest(generated.sourceManifest),
            artifactBytes: generated.bytes,
            inventories,
            failures,
      ...
      report = reportWith({
        ...
        deterministicDiff:
          options.mode === "write"
            ? "not_run"
            : projectionDiff || failures.length > 0
              ? "different"
              : "equal",
        schemaValidation,
        crossReferenceValidation: crossReferencesValid,
      ...
      if (!validateReportBundle(candidate, before)) {
        ...
        report = reportWith({
          ...
          failures,
          mismatches: reportMismatches,
          deterministicDiff: options.mode === "write" ? "not_run" : "different",
          schemaValidation: false,
          crossReferenceValidation: crossReferencesValid,
          forbiddenStatesEmpty: false,
        });
      ...
      if (options.falsifierId === undefined) {
        return yield* Effect.fail(
          new ParityRuntimeError({
            operation: "fixture_injection",
            path: options.root,
            message: "fixture_injection requires --falsifier F0..F19",
          }),
        );
      }
...
> =>
  generateFixtureFromWorkspaceEffect(
    ...
    workspace,
  ).pipe(Effect.map((source) => trustedFixtureArtifacts(source, mode)));
