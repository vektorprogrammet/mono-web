...
import {
  ...
  run,
  runTrustedFixtureTerminalCycle,
} from "../src/runner.js";
import {
  ...
  try {
    return await Effect.runPromise(
      run({
        root,
        legacyRoot,
        intentRegisterPath: path,
        evidenceRegisterPath: evidencePath,
        mode,
  ...
  for (const staleReport of [cycle.missingDiffReport, cycle.differentDiffReport]) {
    expect(staleReport).toMatchObject({
      ...
      failures: expect.arrayContaining([
        expect.objectContaining({ status: "stale", reason_code: "STALE_ARTIFACT" }),
      ]),
    });
  }
...
test("accepted dispositions remove their collected failures from the report", () => {
  ...
  expect(
    reportableFailureAfterDisposition(
      failure,
      new Map<string, InventoryRow["status"]>([["row-accounted", "accounted"]]),
    ),
  ).toMatchObject({
    row_ids: ["row-open"],
    reason_code: "EXTRA_COUNTERPART",
  ...
  expect(
    reportableFailureAfterDisposition(
      failure,
      new Map<string, InventoryRow["status"]>([
  ...
  expect(result.report.failures).toEqual(
    expect.arrayContaining([
      expect.objectContaining({
        ...
        status: "unresolved",
        row_ids: expect.any(Array),
        source_ref_ids: expect.any(Array),
      }),
    ]),
  );
  expect(
    result.artifacts?.commandWrites.rows.some(
      (row) =>
...
test("F14 leaves absent schedules unaccounted until an accepted_absent intent is supplied", async () => {
  ...
  expect(
    result.artifacts.scheduledBackgroundWorkflows.rows.filter(
      (row) => row.authority_line === "mono",
    ),
  ).toEqual([absent]);
  expect(absent.mismatch).toMatchObject({
    kind: "absent",
    disposition: "none",
    accepted_intent_ref_ids: [],
  });
  ...
  expect(accepted).toMatchObject({
    ...
    mismatch: {
      ...
      accepted_intent_ref_ids: ["intent://fixture/absent-schedule"],
    },
  });
...
test("multiline PHPDoc routes ignore continuation stars but reject malformed tails", async () => {
  ...
  try {
    put(
      legacyRoot,
      "src/AppBundle/Controller/DocblockController.php",
      `<?php
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Controller/AssistantController.php",
      `<?php
namespace AppBundle\\Controller;
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Controller/Api/PartyController.php",
      `<?php
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Controller/Api/AccountController.php",
      `<?php
namespace AppBundle\\Controller\\Api;
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Controller/MalformedController.php",
      `<?php
namespace AppBundle\\Controller;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Controller/AssistantController.php",
      `<?php
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Controller/MalformedController.php",
      `<?php
namespace App\\Controller;
use Symfony\\Component\\Routing\\Attribute\\Route;
final class MalformedController
...
`,
    ...
    expect(
      ...
      line_start: 6,
      line_end: 6,
      symbol: "AppBundle\\Controller\\AssistantController::indexAction",
    });
    expect(
      sourceFor(monoAssistant!, "apps/server/src/App/Controller/AssistantController.php"),
    ).toMatchObject({
      line_start: 6,
      line_end: 6,
      symbol: "App\\Controller\\AssistantController::indexAction",
    });
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
    rmSync(monoRoot, { recursive: true, force: true });
...
test("package runtime roots ignore type exports and non-runtime script arguments", async () => {
  ...
  try {
    ...
    put(
      monoRoot,
      "infra/newline-followup.ts",
      scheduleSource("newline_followup_schedule", "NewlineFollowupHandler"),
    );
    put(monoRoot, "infra/empty-word.ts", scheduleSource("empty_word_schedule", "EmptyWordHandler"));
    put(
      monoRoot,
      "infra/leading-dash.ts",
      scheduleSource("leading_dash_schedule", "LeadingDashHandler"),
    );
    put(
      monoRoot,
    ...
    put(
      ...
      "import { sendSms } from './domain.js'\nexport class MailerAdapter { send(): void { sendSms() } }\n",
    );
    put(
      monoRoot,
      "packages/sdk/src/domain.ts",
      "export function sendSms(): void { fetch('https://sms.example.test/send') }\n",
    );
    const context = await contextFor(legacyRoot, monoRoot);
    expect(
      context.rootCensus.find(
        (row) => row.root_ref === "mono" && row.path === "packages/sdk/package.json",
      )?.classification,
    ).toBe("matched");
    expect(
      context.rootCensus.find(
        (row) => row.root_ref === "mono" && row.path === "packages/sdk/tsconfig.json",
      )?.classification,
    ).toBe("matched");
    ...
    for (const identity of [
      ...
      "node_check_schedule",
      "newline_schedule",
      "newline_followup_schedule",
      "empty_word_schedule",
      "leading_dash_schedule",
    ]) {
      expect(byIdentity(identity)).toMatchObject({
        status: "unresolved",
...
test("typed constructor assignments authorize external effect receivers", async () => {
  ...
  try {
    ...
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/Workflow.php",
...
test("write projection gate blocks unresolved C2 effects", async () => {
  ...
  try {
    ...
    expect(result.report.failures).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ reason_code: "UNKNOWN_EFFECT", status: "unresolved" }),
      ]),
    );
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
...
test("effect resolution rejects local receiver shadowing and resolves aliased multi-hop properties", async () => {
  ...
  try {
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/ShadowCommand.php",
    ...
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/MultiCommand.php",
    ...
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Repository/AliasRepo.php",
    ...
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Repository/LeafRepo.php",
      "<?php\nnamespace App\\Fixture\\Infrastructure\\Repository;\nfinal class LeafRepo { public function save(): void {} }\n",
    );
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Service/ApplicationManager.php",
    ...
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/ApprovalCommand.php",
    ...
    expect(multi?.details).toMatchObject({
      ...
      target_refs: ["App\\Fixture\\Infrastructure\\Repository\\LeafRepo::save"],
    });
...
test("unqualified property types use the declaring namespace and never global short names", async () => {
  ...
  try {
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/SharedRepo.php",
      "<?php\nnamespace App\\Fixture\\Infrastructure\\Command;\nfinal class SharedRepo { public function save(): void {} }\n",
    );
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/OtherSharedRepo.php",
      "<?php\nnamespace App\\Fixture\\Infrastructure\\Other;\nfinal class SharedRepo { public function save(): void {} }\n",
    );
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/NamespaceCommand.php",
      "<?php\nnamespace App\\Fixture\\Infrastructure\\Command;\nfinal class NamespaceCommand { private SharedRepo $repo; public function __invoke(): void { $this->repo->save(); } }\n",
    );
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/AmbiguousCommand.php",
...
final class User {
    ...
    put(
      legacyRoot,
      "src/AppBundle/Entity/ExternallyPersistedUser.php",
      `<?php
namespace AppBundle\\Entity;
final class ExternallyPersistedUser {
    ...
    expect(entityRows.some((row) => symbolRefFor(row)?.endsWith("\\User::setRoles") === true)).toBe(
      false,
    );
    expect(entityRows.some((row) => symbolRefFor(row)?.endsWith("\\User::setUser") === true)).toBe(
      false,
    );
    expect(
      entityRows.find(
        (row) => symbolRefFor(row)?.endsWith("ExternallyPersistedUser::setUser") === true,
      ),
    ).toMatchObject({
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Identity/Infrastructure/Entity/Profile.php",
      "<?php\nnamespace App\\Identity\\Infrastructure\\Entity;\nfinal class Profile { public function setUser($user): void {} }\n",
    );
    put(
      monoRoot,
      "apps/server/src/App/Identity/Infrastructure/Repository/MixedProfile.php",
      "<?php\nnamespace App\\Identity\\Infrastructure\\Repository;\nfinal class MixedProfile { public function setUser($user): void {} }\n",
    );
    put(
      monoRoot,
      "apps/server/src/App/Identity/Infrastructure/ApplicationManager.php",
      "<?php\nnamespace App\\Identity\\Infrastructure;\nuse App\\Identity\\Infrastructure\\Entity\\Profile;\nfinal class ApplicationManager { private Profile $profile; public function mutate(): void { $this->profile->setUser($this); } }\n",
    );
    put(
      monoRoot,
      "apps/server/src/App/Identity/Infrastructure/InterviewManager.php",
    ...
    expect(profileLegacy).toMatchObject({
      status: "covered",
      details: { target_refs: ["AppBundle\\Entity\\Profile::setUser"] },
    });
    expect(profileMono).toMatchObject({
      status: "covered",
      details: { target_refs: ["App\\Identity\\Infrastructure\\Entity\\Profile::setUser"] },
    });
    expect(
      c2.commandWrites.links.some(
        (link) =>
          link.relation_kind === "matches" &&
          link.from_row_id === profileLegacy?.row_id &&
          link.to_row_id === profileMono?.row_id,
      ),
    ).toBe(true);
...
test("resolved outbound adapters need no inline URL and Sms setters are not integration calls", async () => {
  ...
  try {
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Service/Mailer.php",
      "<?php\nnamespace App\\Fixture\\Infrastructure\\Service;\nfinal class Mailer { public function send(): void {} }\n",
    );
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Service/SmsSender.php",
      "<?php\nnamespace App\\Fixture\\Infrastructure\\Service;\nfinal class SmsSender { public function send(): void {} }\n",
    );
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Service/Sms.php",
      "<?php\nnamespace App\\Fixture\\Infrastructure\\Service;\nfinal class Sms { public function setMessage(): void {} public function setSender(): void {} public function setRecipients(): void {} }\n",
    );
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/NotifyCommand.php",
    ...
    expect(integrationPaths).toContain(
      "apps/server/src/App/Infrastructure/Command/NotifyCommand.php",
    );
    expect(
...
test("typed fixture-injected integrations redact credentials and raw payloads", async () => {
  ...
  const fixtureRows = integrations.rows.filter((row) =>
    row.source_ref_ids.some(
      (ref) =>
        result.artifacts?.sourceManifest.sources.find((source) => source.source_id === ref)
  ...
  const integrationPaths = integrations.rows.flatMap((row) =>
    row.source_ref_ids.map(
      (ref) =>
        result.artifacts?.sourceManifest.sources.find((source) => source.source_id === ref)?.path ??
        null,
    ),
  );
  ...
  try {
    ...
    const row = c2.integrations.rows.find((candidate) =>
      candidate.source_ref_ids.some(
        (ref) =>
          context.sourcePathById.get(ref)?.path ===
          "apps/server/src/App/Infrastructure/Service/SlackClient.php",
      ),
    );
...
test("command import edges distinguish registered and dead declarations", async () => {
  ...
  try {
    ...
    put(
      legacyRoot,
      "app/config/event_subscribers.yml",
      'services:\n  AppBundle\\EventSubscriber\\:\n    resource: "../../src/AppBundle/EventSubscriber"\n',
    );
    put(
      legacyRoot,
      "src/AppBundle/EventSubscriber/LoadedSubscriber.php",
    ...
    const unregisteredRowIds = new Set(
      c2.commandWrites.rows
        .filter(
          (row) =>
            "owner_ref" in row.details &&
            ["App\\Fixture\\ACommand", "App\\Fixture\\BCommand"].includes(
    ...
    expect(
      c2.commandWrites.rows.some(
        (row) =>
          "owner_ref" in row.details &&
          row.details.owner_ref === "App\\Fixture\\OrphanCommand" &&
    ...
    expect(
      c2.commandWrites.rows.some(
        (row) => "entry_kind" in row.details && row.details.entry_kind === "unknown",
      ),
    ).toBe(false);
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
...
test("routing roots establish controller write reachability", async () => {
  ...
  try {
    ...
    ).toEqual(["app/config/routing.yml", "src/AppBundle/Controller/WriteController.php"]);
    expect(
      c2.commandWrites.links.some(
        (link) =>
          link.relation_kind === "matches" &&
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Controller/LocatorController.php",
      String.raw`<?php
namespace AppBundle\Controller;
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Service/Writer.php",
      String.raw`<?php
namespace AppBundle\Service;
final class Writer {
    private $em;
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Repository/RecordRepository.php",
      String.raw`<?php
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Controller/LocatorController.php",
      String.raw`<?php
namespace App\Fixture\Controller;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/Writer.php",
      String.raw`<?php
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/RecordRepository.php",
      String.raw`<?php
namespace App\Fixture\Infrastructure;
final class RecordRepository {
    public function find(int $id): object { return new \stdClass(); }
...
`,
    ...
    expect(
      c2.commandWrites.links.some(
        (link) =>
          link.relation_kind === "matches" &&
          link.from_row_id === legacy?.row_id &&
          link.to_row_id === mono?.row_id,
      ),
    ).toBe(true);
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Service/AccessControlService.php",
      String.raw`<?php
namespace AppBundle\Service;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Identity/Infrastructure/AccessControlService.php",
      String.raw`<?php
namespace App\Identity\Infrastructure;
final class AccessControlService {
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Service/SlackMessenger.php",
      String.raw`<?php
namespace AppBundle\Service;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Support/Infrastructure/Slack/SlackMessenger.php",
      String.raw`<?php
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Controller/FeedbackController.php",
      String.raw`<?php
namespace AppBundle\Controller;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Content/Controller/FeedbackController.php",
      String.raw`<?php
...
`,
    ...
    for (const method of [
      ...
      "createRule",
      "markRuleAsUnhandledIfNotExists",
    ]) {
      ...
      expect(legacy).toMatchObject({
        status: "covered",
        details: { effect_classes: ["durable_write"] },
      });
      expect(mono).toMatchObject({
        status: "covered",
        details: { effect_classes: ["durable_write"] },
    ...
    expect(monoFeedback).toMatchObject({
      status: "covered",
      details: { effect_classes: ["durable_write", "outbound"] },
    });
    expect(
      c2.commandWrites.links.some(
        (link) =>
          link.relation_kind === "matches" &&
          link.from_row_id === legacyFeedback?.row_id &&
          link.to_row_id === monoFeedback?.row_id,
      ),
    ).toBe(true);
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Entity/Repository/SemesterRepository.php",
      String.raw`<?php
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Shared/Repository/SemesterRepository.php",
      String.raw`<?php
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Controller/SurveyController.php",
      String.raw`<?php
namespace App\Fixture\Controller;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/SurveyRepository.php",
      String.raw`<?php
namespace App\Fixture\Infrastructure;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/SurveyTakenRepository.php",
      String.raw`<?php
namespace App\Fixture\Infrastructure;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/AccessControlService.php",
      String.raw`<?php
namespace App\Fixture\Infrastructure;
final class AccessControlService {
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/Policy.php",
      String.raw`<?php
namespace App\Fixture\Infrastructure;
final class Policy {
    ...
    expect(
      c2.commandWrites.rows.some(
        (row) =>
          ...
          row.details.symbol_ref ===
            "App\\Fixture\\Controller\\SurveyController::showSurveysAction",
      ),
    ).toBe(false);
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
...
`,
    ...
    expect(relocatedLegacy).toMatchObject({
      status: "covered",
      details: {
        entry_kind: "controller_write",
        symbol_ref: "AppBundle\\Controller\\RelocatedController::updateAction",
        effect_classes: ["durable_write"],
      },
    });
    expect(relocatedMono).toMatchObject({
      status: "covered",
      details: {
        entry_kind: "controller_write",
        symbol_ref: "App\\Admissions\\Controller\\RelocatedController::updateAction",
        effect_classes: ["durable_write"],
      },
    });
    expect(subscriberLegacy).toMatchObject({
      status: "covered",
      details: {
        entry_kind: "event_handler",
        symbol_ref: "AppBundle\\EventSubscriber\\RelocatedSubscriber::__invoke",
        effect_classes: ["read_only"],
      },
    });
    expect(subscriberMono).toMatchObject({
      status: "covered",
      details: {
        entry_kind: "event_handler",
        symbol_ref: "App\\Admissions\\EventSubscriber\\RelocatedSubscriber::__invoke",
        effect_classes: ["read_only"],
      },
    });
    ...
    expect(divergentLegacy).toMatchObject({
      status: "missing",
      details: { effect_classes: ["durable_write"] },
    });
    expect(divergentMono).toMatchObject({
      status: "extra",
      details: { effect_classes: ["durable_write", "identity_or_authority"] },
    });
    ...
    expect(renamedLegacy).toMatchObject({
      status: "missing",
      details: { symbol_ref: "AppBundle\\Controller\\RenamedController::createAction" },
    });
    expect(renamedMono).toMatchObject({
      status: "extra",
      details: { symbol_ref: "App\\Admissions\\Controller\\RenamedController::deleteAction" },
    });
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
    rmSync(monoRoot, { recursive: true, force: true });
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Service/RelocatedService.php",
      String.raw`<?php
namespace AppBundle\Service;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/SurveyManager.php",
      String.raw`<?php
namespace App\Fixture\Infrastructure;
final class RelocatedService {
    private $em;
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Service/DivergentService.php",
      String.raw`<?php
namespace AppBundle\Service;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/SurveyNotifier.php",
      String.raw`<?php
namespace App\Fixture\Infrastructure;
final class DivergentService {
    private $em;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/Slack/SlackMessenger.php",
      String.raw`<?php
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Service/SlackMailer.php",
      String.raw`<?php
namespace AppBundle\Service;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/Slack/SlackMailer.php",
      String.raw`<?php
namespace App\Fixture\Infrastructure\Slack;
final class SlackMailer {
    private $messenger;
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Service/InterviewNotificationManager.php",
      String.raw`<?php
namespace AppBundle\Service;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/TeamMembershipService.php",
      String.raw`<?php
namespace App\Fixture\Infrastructure;
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Command/RelocatedCommand.php",
      String.raw`<?php
namespace AppBundle\Command;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/Command/RelocatedCommand.php",
      String.raw`<?php
namespace App\Fixture\Infrastructure\Command;
...
`,
    ...
    put(
      legacyRoot,
      "src/AppBundle/Command/DivergentCommand.php",
      String.raw`<?php
namespace AppBundle\Command;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/Command/DivergentCommand.php",
      String.raw`<?php
namespace App\Fixture\Infrastructure\Command;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/Subscriber/RelocatedSubscriber.php",
      String.raw`<?php
...
`,
    ...
    expect(relocatedSubscriberLegacy).toMatchObject({
      status: "covered",
      details: { entry_kind: "event_handler", effect_classes: ["durable_write"], target_refs: [] },
    });
    expect(relocatedSubscriberMono).toMatchObject({
      status: "covered",
      details: { entry_kind: "event_handler", effect_classes: ["durable_write"], target_refs: [] },
    });
    ...
    expect(relocatedServiceLegacy).toMatchObject({
      status: "covered",
      details: {
        entry_kind: "integration_write",
        effect_classes: ["durable_write"],
        target_refs: [],
      },
    });
    expect(relocatedServiceMono).toMatchObject({
      status: "covered",
      details: {
        entry_kind: "integration_write",
        effect_classes: ["durable_write"],
    ...
    expect(relocatedCommandLegacy).toMatchObject({
      status: "covered",
      details: { command_name: "fixture:relocated", effect_classes: ["durable_write"] },
    });
    expect(relocatedCommandMono).toMatchObject({
      status: "covered",
      details: { command_name: "fixture:relocated", effect_classes: ["durable_write"] },
    });
    ...
    expect(slackLegacy).toMatchObject({
      status: "covered",
      details: { effect_classes: ["outbound"], target_refs: ["Client::sendMessage"] },
    });
    expect(slackMono).toMatchObject({
      status: "covered",
      details: { effect_classes: ["outbound"], target_refs: ["Client::post"] },
    });
    ...
    expect(slackMailerLegacy).toMatchObject({
      ...
      details: {
        effect_classes: ["outbound"],
        target_refs: [
          "AppBundle\\Service\\SlackMessenger::createMessage",
          "AppBundle\\Service\\SlackMessenger::send",
        ],
      },
    });
    ...
    expect(divergentServiceMono).toMatchObject({
      status: "extra",
      details: { effect_classes: ["durable_write", "identity_or_authority"] },
    });
    ...
    expect(divergentCommandLegacy).toMatchObject({
      status: "missing",
      details: { effect_classes: ["durable_write"] },
    });
    expect(divergentCommandMono).toMatchObject({
      status: "extra",
      details: { effect_classes: ["durable_write", "identity_or_authority"] },
    });
...
test("URL-bearing file fetches reconcile GeoLocation writes without classifying local reads", async () => {
  ...
  try {
    ...
    put(
      legacyRoot,
      "src/AppBundle/Service/GeoLocation.php",
      [
        "<?php",
        "namespace AppBundle\\Service;",
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/GeoLocation.php",
      [
        "<?php",
        "namespace App\\Fixture\\Infrastructure;",
        "final class GeoLocation {",
    ...
    put(
      legacyRoot,
      "src/AppBundle/Service/LogService.php",
      [
        "<?php",
        "namespace AppBundle\\Service;",
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/LogService.php",
      [
        "<?php",
        "namespace App\\Fixture\\Infrastructure;",
        "final class LogService {",
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/SlackMessenger.php",
      [
        "<?php",
        "namespace App\\Fixture\\Infrastructure;",
        "final class SlackMessenger {",
    ...
    put(
      legacyRoot,
      "src/AppBundle/Service/CompanyEmailMaker.php",
      [
        "<?php",
        "namespace AppBundle\\Service;",
    ...
    put(
      legacyRoot,
      "src/AppBundle/Service/LocalReader.php",
      [
        "<?php",
        "namespace AppBundle\\Service;",
    ...
    put(
      monoRoot,
      "apps/server/src/App/Identity/Infrastructure/CompanyEmailMaker.php",
      [
        "<?php",
        "namespace App\\Identity\\Infrastructure;",
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/LocalReader.php",
      [
        "<?php",
        "namespace App\\Fixture\\Infrastructure;",
        "final class LocalReader {",
    ...
    expect(legacyCompany).toMatchObject({
      status: "covered",
      details: { effect_classes: ["durable_write", "outbound"] },
    });
    expect(monoCompany).toMatchObject({
      status: "covered",
      details: { effect_classes: ["durable_write", "outbound"] },
    });
    expect(
      c2.commandWrites.links.some(
        (link) =>
          link.relation_kind === "matches" &&
          link.from_row_id === legacyCompany?.row_id &&
          link.to_row_id === monoCompany?.row_id,
      ),
    ).toBe(true);
    expect(legacyGeo).toMatchObject({
      status: "covered",
      details: { effect_classes: ["outbound"] },
    });
    expect(monoGeo).toMatchObject({ status: "covered", details: { effect_classes: ["outbound"] } });
    expect(legacyGeo?.details.target_refs.some((target) => target.startsWith("unresolved:"))).toBe(
      false,
    );
    expect(monoGeo?.details.target_refs.some((target) => target.startsWith("unresolved:"))).toBe(
      false,
    );
    expect(
      c2.commandWrites.links.some(
        (link) =>
          link.relation_kind === "matches" &&
          link.from_row_id === legacyGeo?.row_id &&
          link.to_row_id === monoGeo?.row_id,
      ),
    ).toBe(true);
...
test("dead relocated command rows link without losing dead status", async () => {
  ...
  try {
    put(
      legacyRoot,
      "src/AppBundle/Command/DeadRelocatedCommand.php",
      String.raw`<?php
namespace AppBundle\Command;
...
`,
    ...
    put(
      monoRoot,
      "apps/server/src/App/Fixture/Infrastructure/Command/DeadRelocatedCommand.php",
      String.raw`<?php
namespace App\Fixture\Infrastructure\Command;
...
`,
    ...
    expect(
      c2.commandWrites.links.filter(
        (link) =>
          link.relation_kind === "matches" &&
          rows.some((row) => row.row_id === link.from_row_id || row.row_id === link.to_row_id),
      ),
    ).toHaveLength(1);
...
test("mono Infrastructure integration sources have explicit command-write coverage", async () => {
  ...
  try {
    execFileSync(
      "git",
      ["clone", "--local", "--no-hardlinks", "--no-checkout", REPO_ROOT, cloneRoot],
      { stdio: "ignore" },
    );
    ...
    expect(commandWritePaths).toContain(
      "apps/server/src/App/Support/Infrastructure/Slack/SlackMailer.php",
    );
      ...
      c2.schedules.rows.filter((row) =>
        row.source_ref_ids.some((ref) => {
          ...
          return (
            path === "infra/a.ts" ||
            path === "infra/b.ts" ||
            path === "apps/server/config/packages/cache.yaml"
          );
        }),
      ),
    ).toEqual([]);
    expect(
      c2.schedules.rows.filter((row) => row.authority_line === "mono" && row.status === "absent"),
    ).toHaveLength(1);
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
    rmSync(monoRoot, { recursive: true, force: true });
...
test("generic local requests, sends, and declarations do not create integrations", async () => {
  ...
  try {
    ...
    put(
      monoRoot,
      "packages/google-client.ts",
      "export class GoogleClient { fetch(dynamicEndpoint) { return dynamicEndpoint }\n}\n",
    );
    put(
      monoRoot,
      "packages/preview.ts",
      "export function handle(request: Request): Promise<Response> { request.headers.get('host'); if (request.method !== 'GET') throw new Error('method'); return fetch(request) }\n",
    );
    put(
      monoRoot,
      "packages/tool/tests/client.test.ts",
      "test('fixture', () => fetch('https://api.example.test/v1/items'))\n",
    );
    const context = await contextFor(legacyRoot, monoRoot);
    ...
    expect(
      c2.integrations.rows.filter((row) =>
        row.source_ref_ids.some((ref) =>
          genericPaths.has(context.sourcePathById.get(ref)?.path ?? ""),
        ),
      ),
    ).toEqual([]);
    expect(
...
test("command rows require positive declarations or write effects", async () => {
  ...
  try {
    ...
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Repository/ReadRepository.php",
      "<?php\nnamespace App\\Fixture\\Infrastructure\\Repository;\nfinal class ReadRepository { public function find(): void {} }\n",
    );
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Repository/WriteRepository.php",
      "<?php\nnamespace App\\Fixture\\Infrastructure\\Repository;\nfinal class WriteRepository { public function persist(): void {} public function flush(): void {} }\n",
    );
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/ReadCommand.php",
    ...
    expect(
      writes.every(
        (row) =>
          "effect_classes" in row.details && row.details.effect_classes.includes("durable_write"),
      ),
    ).toBe(true);
    expect(
      commandRows.find((row) =>
        hasPath(row, "apps/server/src/App/Infrastructure/Command/ReadCommand.php"),
      ),
    ).toMatchObject({ details: { effect_classes: ["read_only"] } });
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
    rmSync(monoRoot, { recursive: true, force: true });
  }
  ...
  try {
    ...
    expect(
      integrations.rows
        .filter((row) =>
          row.source_ref_ids.some((ref) =>
            sourcePaths.has(context.sourcePathById.get(ref)?.path ?? ""),
          ),
        )
...
test("YAML block scalars cannot forge loader imports or class authority", async () => {
  ...
  try {
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/BlockCommand.php",
    ...
    expect(
      c2.commandWrites.links.some(
        (link) => link.to_row_id === row?.row_id && link.relation_kind === "imports",
      ),
    ).toBe(false);
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
...
test("command imports require rooted loader reachability and reject loader cycles", async () => {
  ...
  try {
    ...
    put(
      monoRoot,
      "apps/server/config/services-a.yaml",
      "imports:\n  - resource: services-b.yaml\nservices:\n  a:\n    class: App\\Fixture\\ACommand\n",
    );
    put(
      monoRoot,
      "apps/server/config/services-b.yaml",
...
test("framework effect anchors remain authoritative without local receiver types", async () => {
  ...
  try {
    ...
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/SmsValueCommand.php",
...
test("schedule identities and credential slots are decoded before artifact identity", async () => {
  ...
  try {
    ...
    expect(
      c2.schedules.rows.some((row) => row.reason_codes.includes("SCHEDULE_IDENTITY_UNRESOLVED")),
    ).toBe(true);
    expect(
      c2.integrations.rows.some((row) => row.reason_codes.includes("CREDENTIAL_SLOT_UNRESOLVED")),
    ).toBe(true);
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
    rmSync(monoRoot, { recursive: true, force: true });
...
test("loader authority requires exact roots and supports FQCN keys plus PSR-4 resources", async () => {
  ...
  try {
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/Loaded/LoadedCommand.php",
    ...
    put(
      monoRoot,
      "apps/server/src/App/Infrastructure/Command/Loaded/ExcludedCommand.php",
    ...
    put(
      monoRoot,
      "apps/server/config/services-dead.yaml",
      "services:\n  orphan:\n    class: App\\Fixture\\OrphanCommand\n",
    );
    const context = await contextFor(legacyRoot, monoRoot);
...
test("owner-null integration modules require positive loader reachability", async () => {
  ...
  try {
    put(
      monoRoot,
      "packages/decoy.ts",
      "export const send = () => fetch('https://api.slack.com/v1/send')\n",
    );
    const context = await contextFor(legacyRoot, monoRoot);
