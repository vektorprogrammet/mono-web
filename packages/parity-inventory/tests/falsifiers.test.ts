...
import { acceptedIntentRevisionRefId, loadAcceptedIntentRegister, type IntentSourceInput } from "../src/coverage.js"
import { createManifestContextFromSnapshots, isUnsafeSourcePath, sanitizeScalar, unsafeScalarReason, unsafeSourceScalarReason, type ManifestContext } from "../src/source-manifest.js"
  ...
  for (const falsifierId of ["F8_openapi_stale", "F9_runtime_unavailable", "F10_static_runtime_mismatch", "F16_h3_authority_copy"] as const) {
    test(`${falsifierId} preserves C1 authority boundaries`, async () => {
      ...
      if (falsifierId === "F8_openapi_stale") {
        ...
        const h3Copy = result.artifacts?.apiOperations.rows.find((row) => "uri_template" in row.details && row.details.uri_template === "/fixture/h3-authority-copy")
        expect(h3Copy).toBeDefined()
        if (h3Copy === undefined) throw new Error("H3 authority copy row missing")
        expect(h3Copy).toMatchObject({ authority_line: "cross_line", status: "unresolved", observation_kinds: ["derived_h3"] })
        expect(result.report.failures).toEqual(expect.arrayContaining([expect.objectContaining({ status: "schema_invalid", reason_code: "H3_DERIVATION_ONLY", row_ids: [h3Copy.row_id] })]))
      }
    })
  }
})
...
describe("C3 falsifier receipts", () => {
  for (const [falsifierId, failureStatus, reasonCode] of c3ReceiptMatrix) {
    test(`${falsifierId} emits a causal sanitized receipt`, async () => {
      ...
      expect(result.report.status).toBe("falsifier_passed")
      expect(result.report.falsifier_id).toBe(falsifierId)
      const receipts = result.report.failures.filter((failure) => failure.reason_code === reasonCode)
      ...
      if (falsifierId === "F12_uncovered_journey") {
        ...
        expect(result.artifacts?.routeRows.some((row) => row.row_id === receipt?.row_ids[0] && row.status === "uncovered")).toBe(true)
      }
...
const terminalReport = (mode: "diff" | "write", status: "zero_gap" | "projection_written", exitCode: 0 | 14, deterministicDiff: "equal" | "not_run"): Record<string, unknown> => ({
  ...
  mode,
  falsifier_id: null,
  projection_write: mode === "write" ? { status: "written", target_ref: "evidence/functional-parity" } : { status: "not_requested", target_ref: null },
...
test("terminal diff publishes only a clean zero-gap receipt", () => {
  ...
  expect(report).toMatchObject({ status: "zero_gap", exit_code: 0, mode: "diff", projection_write: { status: "not_requested" }, verification: { deterministic_diff: "equal", forbidden_states_empty: true } })
...
test("terminal write promotion is exit fourteen and does not run diff", () => {
  ...
  expect(report).toMatchObject({ status: "projection_written", exit_code: 14, mode: "write", projection_write: { status: "written", target_ref: "evidence/functional-parity" }, verification: { deterministic_diff: "not_run" } })
})
test("terminal claims require the closed generated artifact bundle", async () => {
  ...
  const forgedZeroGap = terminalReport("diff", "zero_gap", 0, "equal") as unknown as typeof artifacts.report
  const forgedProjectionWritten = terminalReport("write", "projection_written", 14, "not_run") as unknown as typeof artifacts.report
  expect(validateReportBundle({ ...artifacts, report: forgedZeroGap }, observation)).toBe(false)
  expect(validateReportBundle({ ...artifacts, report: forgedProjectionWritten }, observation)).toBe(false)
})
...
const createIntentAuthority = (root: string, legacyRoot: string): { readonly path: string; readonly directory: string } => {
...
const intentContextFor = async (text: string): Promise<{ readonly context: ManifestContext; readonly monoRoot: string; readonly legacyRoot: string; readonly intentInput: IntentSourceInput }> => {
...
const runWithIntentAuthority = async (root: string, legacyRoot: string, mode: "diff" | "write") => {
  ...
  try {
    return await Effect.runPromise(run({ root, legacyRoot, intentRegisterPath: authority.path, mode }))
  } finally {
...
test("accepted-intent decoder rejects PII and duplicate JSON members", async () => {
  const pii = await intentContextFor("{\"schema_version\":\"functional-parity-accepted-intent/v1\",\"intents\":[],\"journeys\":[],\"note\":\"sk_live_fixture_secret\"}")
  const duplicate = await intentContextFor("{\"schema_version\":\"functional-parity-accepted-intent/v1\",\"intents\":[],\"intents\":[],\"journeys\":[]}")
  try {
    expect(loadAcceptedIntentRegister(pii.context, pii.intentInput)).toMatchObject({ register: null, issues: [expect.objectContaining({ reasonCode: "UNSAFE_SOURCE", status: "accepted_intent_invalid" })] })
    expect(loadAcceptedIntentRegister(duplicate.context, duplicate.intentInput)).toMatchObject({ register: null, issues: [expect.objectContaining({ reasonCode: "INTENT_DUPLICATE_KEY", status: "accepted_intent_invalid" })] })
  } finally {
...
test("projection state cannot provide accepted intent authority", async () => {
  const fixture = await intentContextFor("{\"schema_version\":\"functional-parity-accepted-intent/v1\",\"intents\":[],\"journeys\":[]}")
  try {
    expect(loadAcceptedIntentRegister(fixture.context)).toMatchObject({ register: null, path: null, issues: [expect.objectContaining({ reasonCode: "ACCEPTED_INTENT_MISSING", status: "accepted_intent_invalid" })] })
  } finally {
...
test("external intent authority must remain clean while pinned", () => {
  ...
  try {
    writeFileSync(authority.path, "{\"schema_version\":\"functional-parity-accepted-intent/v1\",\"intents\":[],\"journeys\":[],\"changed\":true}", "utf8")
    expect(() => Effect.runSync(readPinnedIntentRegisterEffect(authority.path, legacy, mono))).toThrow()
  } finally {
    rmSync(authority.directory, { recursive: true, force: true })
...
const cliReport = (root: string, legacyRoot: string, mode: "diff" | "write" = "diff"): { readonly status: number | null; readonly report: Record<string, unknown>; readonly output: string } => {
  const authority = createIntentAuthority(root, legacyRoot)
  try {
    const process = spawnSync("bun", ["run", "src/main.ts", "--root", root, "--legacy-root", legacyRoot, "--intent-register", authority.path, "--mode", mode], { cwd: join(import.meta.dir, ".."), encoding: "utf8" })
