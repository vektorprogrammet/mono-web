...
test("relocated subscriber notification integrations reconcile by owner and method", async () => {
  ...
  try {
    ...
    put(
      legacyRoot,
      legacyPath,
      subscriberSource(legacyClass.slice(0, legacyClass.lastIndexOf("\\"))),
    );
...
test("relocated subscriber event triggers reconcile by subscriber identity", async () => {
  ...
  try {
    ...
    expect(rows.every((row) => row.status === "covered")).toBe(true);
    expect(
      c2.schedules.links.some(
        (link) =>
          link.relation_kind === "matches" &&
          rows.some((row) => link.from_row_id === row.row_id || link.to_row_id === row.row_id),
      ),
    ).toBe(true);
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
...
test("interface and transport parser artifacts are excluded without hiding new or ambiguous integrations", async () => {
  ...
  try {
    ...
    put(
      ...
      `export interface Transport { put(url: string): void }
...
`,
    ...
    const ambiguousLegacy = [
      [
        ...
        "AppBundle\\EventSubscriber\\Admissions\\NoticeSubscriber",
      ],
      [
        "src/AppBundle/EventSubscriber/Support/NoticeSubscriber.php",
    ...
    put(
      monoRoot,
      "apps/server/src/App/Notice/Infrastructure/Subscriber/NoticeSubscriber.php",
      subscriberSource("App\\Notice\\Infrastructure\\Subscriber", "NoticeSubscriber"),
    );
    put(
      legacyRoot,
      "app/config/services.yml",
      serviceConfig(ambiguousLegacy.map(([, className]) => className)),
    );
    put(
      monoRoot,
      "apps/server/config/services.yaml",
      serviceConfig([newIntegrationClass, modernAmbiguousClass]),
    );
    ...
    expect(
      rows.some(
        (row) => pathFor(row) === legacyInterfacePath || pathFor(row) === monoInterfacePath,
      ),
    ).toBe(false);
    ...
    expect(
      ambiguous
        .filter((row) => row.authority_line === "legacy")
        .every((row) => row.status === "missing"),
    ).toBe(true);
    expect(
      ambiguous
        .filter((row) => row.authority_line === "mono")
        .every((row) => row.status === "extra"),
    ).toBe(true);
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
    rmSync(monoRoot, { recursive: true, force: true });
...
`,
    ...
    put(
      monoRoot,
      "packages/sdk/src/domains/auth.ts",
...
`,
    ...
    expect(
      integrations.rows.some((row) => pathFor(row)?.startsWith("packages/sdk/src/domains/")),
    ).toBe(false);
    expect(integrations.rows.some((row) => pathFor(row) === "packages/sdk/src/transport.ts")).toBe(
      false,
    );
  } finally {
    rmSync(legacyRoot, { recursive: true, force: true });
...
`;
    ...
    expect(
      rows.every(
        (row) =>
          "trigger_identity" in row.details &&
          row.details.trigger_identity === "app:admission:send_notifications",
      ),
    ).toBe(true);
    const rowIds = new Set(rows.map((row) => row.row_id));
    expect(
...
`,
    ...
    expect(rows.every((row) => row.status === "covered")).toBe(true);
    expect(
      rows.every((row) => "trigger_kind" in row.details && row.details.trigger_kind === "event"),
    ).toBe(true);
    expect(rows.every((row) => "enabled" in row.details && row.details.enabled === true)).toBe(
      true,
    );
    expect(
      c2.schedules.links.some(
...
`;
    ...
    put(
      monoRoot,
      "apps/server/src/App/Support/Infrastructure/Mailer/Mailer.php",
      mailerSource("App\\Support\\Infrastructure\\Mailer"),
    );
    put(
      legacyRoot,
      "app/config/services.yml",
    ...
    expect(
      rows.some(
        (row) =>
          "call_site_ref" in row.details && row.details.call_site_ref?.endsWith("Mailer::elseif"),
      ),
    ).toBe(false);
    expect(
      rows
        .filter(
          (row) =>
            ...
            row.details.call_site_ref?.includes("SlackMessenger::"),
        )
        .every((row) => row.status === "covered"),
    ).toBe(true);
    expect(
      rows
        .filter(
          (row) =>
            "call_site_ref" in row.details && row.details.call_site_ref?.includes("SlackMailer::"),
        )
        .every((row) => row.status === "covered"),
    ).toBe(true);
    expect(
      rows
        .filter(
          (row) =>
            "call_site_ref" in row.details && row.details.call_site_ref?.includes("Mailer::"),
        )
        .every((row) => row.status === "covered"),
    ).toBe(true);
  } finally {
  ...
  try {
    put(
      ...
      JSON.stringify({
        scripts: {
          "preview:dev":
            "WRANGLER_SEND_METRICS=false node node_modules/wrangler/bin/wrangler.js dev infra/preview.worker.ts --local",
        },
      }),
    );
    put(
...
`,
    ...
    put(
      monoRoot,
      "infra/alchemy/preview/worker-resource.ts",
...
`,
    ...
    put(
      monoRoot,
      "infra/alchemy/preview/worker.ts",
