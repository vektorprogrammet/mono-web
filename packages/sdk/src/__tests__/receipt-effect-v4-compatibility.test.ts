...
import { apiUrl, sdkRuntimeConfig } from "../config.js";
...
describe("Effect v4 Receipt compatibility", () => {
  ...
  it("keeps both public entrypoints, namespaces, verbs, and execution shapes", async () => {
    ...
    for (const client of [promiseClient, effectClient]) {
      ...
      expectMethods(client.admin.interviews, [
        "list",
        "assign",
        "schedule",
        "conduct",
    ...
    const promiseResult = promiseClient.admin.receipts.list({
      status: "pending",
      page: 1,
      pageSize: 2,
    });
    ...
    const effectResult = effectClient.admin.receipts.list({
      status: "pending",
      page: 1,
      pageSize: 2,
    });
    expect(Effect.isEffect(effectResult)).toBe(true);
    await testRuntime.runPromise(effectResult);
  });

  it("loads absent configuration from an injected provider without side effects", async () => {
    const configured = await testRuntime.runPromise(
      sdkRuntimeConfig.parse(ConfigProvider.fromUnknown({})),
    );

    expect(configured.apiUrl).toBeUndefined();
    expect(configured.isFixtureMode).toBe(false);
  ...
  it("fails configuration inside the operation without calling fetch", async () => {
    ...
    const effectError = await testRuntime.runPromise(
      Effect.flip(effectClient.admin.receipts.list()),
    );
    ...
    await expect(invalidPromise.admin.receipts.list()).rejects.toMatchObject({
      type: "configuration",
    });
    const invalidEffectError = await testRuntime.runPromise(
      Effect.flip(invalidEffect.admin.receipts.list()),
    );
  ...
  it("authenticates and strictly decodes the Promise Receipt list", async () => {
    ...
    expect(requests[0]?.url).toBe(
      `${FIXTURE_URL}/api/admin/receipts?status=pending&page=1&itemsPerPage=2`,
    );
  ...
  it("returns a direct Effect list value with the same Page-compatible result", async () => {
    ...
    const client = createEffectClient(FIXTURE_URL, {
      cookie: "better-auth.session_token=trace-session",
    });
  ...
  it("maps approve to refunded and treats 204 as void without parsing JSON", async () => {
    ...
    expect(Effect.isEffect(effect)).toBe(true);
    await expect(testRuntime.runPromise(effect)).resolves.toBeUndefined();
    expect(requests[0]?.url).toBe(`${FIXTURE_URL}/api/admin/receipts/42/status`);
    expect(requests[0]?.init.method).toBe("PUT");
    expect(JSON.parse(String(requests[0]?.init.body))).toEqual({ status: "refunded" });
  });
  ...
  it("rejects malformed Hydra envelopes, members, and ISO dates before exposing values", async () => {
    ...
    for (const body of malformedBodies) {
      ...
      const promiseClient = createClient(FIXTURE_URL);
      const promiseError = await promiseClient.admin.receipts
        .list()
  ...
  it("preserves typed HTTP and network error mappings on both surfaces", async () => {
    ...
    for (const [status, publicError, publicType, effectTag] of cases) {
      ...
      if (status === 422)
        expect(promiseError).toMatchObject({ fields: { description: "Too short" } });
      ...
      if (status === 422)
        expect(effectError).toMatchObject({ fields: { description: "Too short" } });
    }

    fetchMock.mockRejectedValue(new TypeError("Failed to fetch"));
    const promiseNetworkError = await createClient(FIXTURE_URL)
      .admin.receipts.list()
      .catch((error: unknown) => error);
    expect(promiseNetworkError).toBeInstanceOf(NetworkError);
    expect(promiseNetworkError).toMatchObject({ type: "network" });
    const effectNetworkError = await testRuntime.runPromise(
      Effect.flip(createEffectClient(FIXTURE_URL).admin.receipts.list()),
    );
  ...
  it("propagates fiber interruption to fetch and leaves no owned request work", async () => {
    ...
    const client = createEffectClient(FIXTURE_URL);
    const fiber = testRuntime.runFork(client.admin.receipts.list());
    await started;
    await testRuntime.runPromise(Fiber.interrupt(fiber));
    const ownerExit = await testRuntime.runPromise(Fiber.await(fiber));
...
describe("Application status typed decode", () => {
  ...
  function stubMalformedResponses(): void {
    fetchMock.mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      return makeResponse(
        200,
        url.endsWith("/101") ? applicationResponse(999) : applicationCollection(999),
      );
    });
  }
  ...
  it("rejects unknown application status through the Promise list channel", async () => {
    ...
    const error = await createClient(APPLICATION_FIXTURE_URL)
  ...
  it("preserves all seven application statuses across valid Promise and Effect list/detail results", async () => {
    ...
    for (const [rawStatus, expectedStatus, expectedLabel] of applicationStatusCases) {
      ...
      fetchMock.mockResolvedValueOnce(makeResponse(200, applicationCollection(rawStatus)));
      const effectList = await testRuntime.runPromise(effectClient.admin.applications.list());
      ...
      fetchMock.mockResolvedValueOnce(makeResponse(200, applicationResponse(rawStatus)));
      const effectDetail = await testRuntime.runPromise(effectClient.admin.applications.get(101));
