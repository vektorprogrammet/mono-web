...
describe("Receipt", () => {
  it("decodes from raw API shape", async () => {
    const result = await testRuntime.runPromise(Schema.decodeUnknownEffect(Receipt)(rawReceipt));
  ...
  it("encodes back to raw API shape (round-trip)", async () => {
    const decoded = await testRuntime.runPromise(Schema.decodeUnknownEffect(Receipt)(rawReceipt));
  ...
  it("isPending returns true when status is 'pending'", async () => {
    const receipt = await testRuntime.runPromise(Schema.decodeUnknownEffect(Receipt)(rawReceipt));
    expect(receipt.isPending).toBe(true);
  });

  it("isPending returns false when status is 'refunded'", async () => {
    const receipt = await testRuntime.runPromise(
...
describe("Application (ApplicationFromRaw)", () => {
  ...
  it("encodes back (round-trip)", async () => {
    const decoded = await testRuntime.runPromise(
      Schema.decodeUnknownEffect(ApplicationFromRaw)(rawApplication),
    );
