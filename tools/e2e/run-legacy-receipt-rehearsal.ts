...
import { chmod, cp, lstat, mkdir, readFile, readdir, rm, symlink, truncate, writeFile } from "node:fs/promises";
import { isAbsolute, join, relative, resolve, sep } from "node:path";
import { fileURLToPath } from "node:url";
import { databaseSchemaRevision } from "@vektorprogrammet/database/migrations";
import { ReceiptReview, receiptSourceRevision, receiptSourceRowDigest, type ReceiptSourceRow } from "@vektorprogrammet/domain/receipt";
...
import { decodeSnapshot, prepareReceiptSnapshot, rowDigest } from "../../apps/backend/src/receipt/import-snapshot";
import { buildLegacyReferences } from "./legacy-cutover-references";
import { legacyReceiptBaseSourceRevision, legacyReceiptRows, legacyReceiptTransformationRevision } from "./legacy-receipt-snapshot";
...
import { digest, repositoryRoot, runLocal, runLocalResult, targetFingerprint, withOrganizationDatabases, type LocalCommandResult, type RehearsalTarget } from "./legacy-organization-rehearsal-runtime";
...
const receiptFixtures = [
  ...
  id: number; status?: string; path?: string; amount?: string; date?: string;
  visual?: string | null; user?: number | null; submitted?: string;
}>;
...
const Summary = Schema.Struct({
  snapshotKey: Schema.String, replay: Schema.Boolean, input: Schema.Int,
  accepted: Schema.Int, quarantined: Schema.Int, excluded: Schema.Int,
  reconciled: Schema.Int, pending: Schema.Int, complete: Schema.Boolean,
  occurrences: Schema.Array(Schema.Struct({ sourcePrimaryKey: Schema.String, disposition: Schema.Literals(["Accepted", "Quarantined", "Excluded"]), reasons: Schema.Array(Schema.String) })),
});
...
type StoredReceipt = {
  receipt_id: string; visual_id: string; owner_person_id: string; department_id: string;
  amount_ore: number; status: string; approved_at: string | null; description: string;
  payment_account_ciphertext: string; file_ref: string; file_object_key: string;
  file_content_type: "application/pdf" | "image/png" | "image/jpeg"; file_byte_length: number;
  file_sha256: string; revision: number;
};
...
const tree = async (root: string): Promise<Record<string, string>> => {
  ...
  const walk = async (directory: string): Promise<void> => {
    for (const item of await readdir(directory, { withFileTypes: true }).catch((cause: NodeJS.ErrnoException) => {
      ...
      else entries[relative(root, path)] = item.isSymbolicLink() ? "symlink" : sha256(await readFile(path));
    }
  };
...
const receipts = async (pool: Pool): Promise<StoredReceipt[]> => (await pool.query<StoredReceipt>(`
 SELECT receipt_id,visual_id,owner_person_id,department_id,amount_ore,status,
...
const noInventedEffects = async (pool: Pool): Promise<Record<string, number>> => {
  ...
  for (const { name } of tables) {
    result[name] = Number((await pool.query<{ count: string }>(`SELECT count(*)::text AS count FROM ${name}`)).rows[0]!.count);
...
const assertNoAccountPersistence = async (pool: Pool): Promise<void> => {
  ...
  const forbidden = [account, normalizedAccount, alternateAccount, sha256(account), sha256(normalizedAccount)];

  for (const { name } of tables) {
    const rows = (await pool.query<{ row: unknown }>(`SELECT to_jsonb(value) AS row FROM ${name} value`)).rows;

    for (const row of rows) {
      const text = JSON.stringify(row);

      for (const secret of forbidden) assert.equal(text.includes(secret), false, "Account plaintext or unkeyed digest persisted");
    }
  }
};
...
const assertSummary = (report: Summary, accepted = 3): void => {
  ...
  assert.deepEqual(new Set(report.occurrences.map((row) => row.sourcePrimaryKey)), new Set(receiptFixtures.map((row) => String(row.id))));
  ...
  assert.equal(report.occurrences.find((row) => row.sourcePrimaryKey === "4")?.disposition, "Excluded");
  ...
  assert.equal(report.occurrences.find((row) => row.sourcePrimaryKey === "13")?.disposition, "Quarantined");
};

const rehearse = async () => withOrganizationDatabases(fixtureSql, async ({ sourceUrl, mysql, target, temporaryRoot }) => {
  const checks: string[] = [];
  const checked = (name: string) => checks.push(name);
  const key = randomBytes(32);
  const cipher = PaymentCustody.makePaymentAccountCipher({ keyId: "synthetic-receipt-key-v1", key });
  const wrongCipher = PaymentCustody.makePaymentAccountCipher({ keyId: cipher.keyId, key: randomBytes(32) });
  ...
  assert.equal(legacyReceiptBaseSourceRevision(source), digest((({ credentials: _credentials, ...rest }) => rest)(omitted)));
  ...
  assert.ok(source.receipts?.find((row) => String(row.id) === "7")?.receiptDate.startsWith("0000-00-00"));
  const writer = new URL(sourceUrl);
  writer.username = "root";
  await assert.rejects(readLegacySourceSnapshot(writer.toString(), "NotRequested", "Include"), /Grants/);
  ...
  await mysql(`UPDATE vektor.user SET accountNumber='${alternateAccount}',password='synthetic-password-must-not-leak' WHERE id=1`);
  ...
  assert.notEqual(receiptSourceRevision(legacyReceiptRows(changedSecrets, cipher)), financeRevision);
  ...
  checked("SELECT-only finance selection, exact decimal text, unchanged existing source shape, keyed raw-account commitment");
  ...
  const freshTarget = async (name: string): Promise<RehearsalTarget> => {
    ...
    const cutover = await runLegacyServiceCutover({
      sourceUrl, targetUrl: database.url, targetDatabase: database.database,
      snapshotId: personSnapshotId, attestedBy: "synthetic-reviewer", passwordlessPolicy: "ProvisionRecovery",
      currentAssignments: "NotRequested", organization: "NotRequested",
    });
  ...
  const acceptedPeople = (await primary.pool.query<{ source_user_id: string }>(
    "SELECT source_user_id FROM person_cohort_accepted_mappings WHERE snapshot_key=$1 ORDER BY source_user_id", [personSnapshotKey],
  )).rows;

  assert.deepEqual(acceptedPeople.map((row) => row.source_user_id), ["legacy-user:1", "legacy-user:2", "legacy-user:4", "legacy-user:5"]);

  const rootsFor = async (name: string): Promise<Roots> => {
    const root = join(temporaryRoot, name);
    await mkdir(root, { mode: 0o700 });
    const roots = { archive: join(root, "archive"), staging: join(root, "staging"), committed: join(root, "private") };
    ...
    for (const row of receiptFixtures) {
      ...
      const bytes = Match.value(row.id).pipe(Match.when(2, () => png), Match.when(3, () => jpeg), Match.when(18, () => Buffer.from("not-a-PDF-document")), Match.orElse(() => pdf));
      await writeFile(join(roots.archive, row.path ?? `row-${row.id}.pdf`), bytes, { mode: 0o600 });
    }
  ...
  const metadataFor = async (row: ReceiptSourceRow) => {
    ...
    const bytes = await Match.value(id).pipe(Match.when(19, () => readFile(join(roots.archive, path))), Match.when(2, () => png), Match.when(3, () => jpeg), Match.when(18, () => Buffer.from("not-a-PDF-document")), Match.orElse(() => pdf));

    return {
      path, sha256: id === 16 ? "0".repeat(64) : sha256(bytes),
      byteLength: bytes.byteLength + (id === 17 ? 1 : 0),
      contentType: Match.value(id).pipe(Match.when(2, () => "image/png" as const), Match.when(3, () => "image/jpeg" as const), Match.orElse(() => "application/pdf" as const)),
    };
  };

  const entries = await Promise.all(sourceRows.map(async (row) => {
    const id = Number(row.sourcePrimaryKey);
    const common = { sourcePrimaryKey: row.sourcePrimaryKey, sourceRowDigest: receiptSourceRowDigest(row), evidenceRef: `synthetic-receipt-evidence:${id}` };

    if (id === 4) return { ...common, _tag: "Excluded" as const, reason: "ExplicitSyntheticExclusion" };
    const user = row.sourceUserId?.slice("legacy-user:".length) ?? "999";

    return {
      ...common, _tag: "Import" as const,
      person: row.sourceUserId === null ? null : { occurrenceId: id === 26 ? "missing-occurrence" : `legacy-user-row-${user}`, sourceUserId: row.sourceUserId, personId: id === 24 ? "legacy-person-2" : `legacy-person-${user}` },
      department: { sourceDepartmentId: id === 25 ? "legacy-department:999" : "legacy-department:1", departmentId: "legacy-department:1" },
      receiptDate: "2026-08-20", submittedAt: "2026-08-21T12:00:00Z",
      approvedAt: row.status === "refunded" ? "2026-09-02T12:00:00Z" : null,
      file: await metadataFor(row), payment: { commitment: row.accountCommitment, evidenceRef: `synthetic-account-ownership:${user}` },
    };
  }));

  const review = Schema.decodeUnknownSync(ReceiptReview)({
    sourceRepository: repository, sourceRevision: baseRevision, receiptSourceRevision: financeRevision,
    snapshotId: "synthetic-receipt-2026", sourceWatermark, transformationRevision: await legacyReceiptTransformationRevision(),
    personSnapshotKey, referenceSnapshotId: personSnapshotId, referenceDigest: buildLegacyReferences(source).referenceDigest,
    attestedBy: "synthetic-reviewer", evidenceRef: "synthetic-review-evidence", entries,
  });

  const reviewPath = join(temporaryRoot, "receipt-review.json");
  await privateJson(reviewPath, review);

  const sensitive = [account, normalizedAccount, alternateAccount, sha256(account), sha256(normalizedAccount), description,
    "fixture1@example.invalid", "FixtureGiven", key.toString("base64"), sourceUrl, temporaryRoot, "synthetic-password-must-not-leak", "private-sql-parameter-must-not-leak", "secret@"];

  const noLeaks = (value: string): void => {
    for (const secret of sensitive) assert.equal(value.includes(secret), false, "Public output contains private source material");
    assert.equal(value.includes("acceptedResults"), false, "Private accepted results escaped into public output");
    assert.equal(value.includes("payment_account_ciphertext"), false);
  };

  noLeaks(await readFile(reviewPath, "utf8"));

  const cliArgs = (database: RehearsalTarget, files: Roots, selectedReview = reviewPath, selectedKey = keyPath) => [
    "bun", "--no-env-file", "tools/e2e/run-legacy-receipt-import.ts",
    `--review=${selectedReview}`, `--archive-root=${files.archive}`, `--staging-root=${files.staging}`,
    `--committed-root=${files.committed}`, `--payment-key=${selectedKey}`,
    "--source-env=RECEIPT_REHEARSAL_SOURCE", "--target-env=RECEIPT_REHEARSAL_TARGET",
    `--target-database=${database.database}`, "--organization-source=none",
  ];

  const invoke = async (database: RehearsalTarget, files: Roots, selectedReview = reviewPath, environment: Record<string, string> = {}, selectedKey = keyPath) => {
    const output = await runLocalResult(cliArgs(database, files, selectedReview, selectedKey), undefined, {
      RECEIPT_REHEARSAL_SOURCE: sourceUrl, RECEIPT_REHEARSAL_TARGET: database.url, ...environment,
    });
  ...
  const success = async (database: RehearsalTarget, files: Roots): Promise<Summary> => {
    ...
    assert.equal(output.code, report.complete ? 0 : 2, "Actual receipt CLI exit status contradicts reconciliation state");
  ...
  await refusal("duplicate-review-entry", { ...review, entries: [...review.entries, review.entries[0]] });
  await refusal("unknown-review-entry", { ...review, entries: [...review.entries, { ...review.entries[0], sourcePrimaryKey: "9999" }] });
  await refusal("changed-source-digest", { ...review, entries: review.entries.map((entry, index) => index ? entry : { ...entry, sourceRowDigest: "0".repeat(64) }) });
  await refusal("wrong-person-snapshot", { ...review, personSnapshotKey: "0".repeat(64) }, "PersonSnapshotConflict");
  await refusal("wrong-reference-snapshot", { ...review, referenceSnapshotId: "wrong-snapshot" }, "ReferenceProvenanceConflict");
  await refusal("wrong-reference-digest", { ...review, referenceDigest: "0".repeat(64) }, "ReferenceProvenanceConflict");
  ...
  const unavailable = { archive: join(temporaryRoot, "absent-archive"), staging: join(temporaryRoot, "absent-stage"), committed: join(temporaryRoot, "absent-committed") };
  safeFailure(await invoke(primary, unavailable, invalidPath, {
  ...
  for (const path of Object.values(unavailable)) await assert.rejects(lstat(path), { code: "ENOENT" });
  assert.equal(await targetFingerprint(primary.pool), pristine);
  ...
  for (const environment of [
    ...
    { RECEIPT_REHEARSAL_TARGET: "postgresql://user:secret@remote.example.invalid/receipt_reviewed?sslmode=disable" },
  ...
  const malformedOptions = await runLocalResult([...cliArgs(primary, roots), "--organization-source=unknown"], undefined, {
    RECEIPT_REHEARSAL_SOURCE: sourceUrl, RECEIPT_REHEARSAL_TARGET: primary.url,
  });
  ...
  assert.equal(await targetFingerprint(primary.pool), pristine);
  assert.deepEqual(await tree(roots.staging), pristineFiles);
  checked("owner-only JSON and key files, symlink rejection, explicit selections, insecure remote transport refused");
  ...
  assert.deepEqual(initialReceipts.map((row) => [row.visual_id, row.status, row.approved_at, row.amount_ore]), [
    ["RECEIPT-1", "Pending", null, 1234], ["RECEIPT-2", "Rejected", null, 1234], ["RECEIPT-3", "Approved", "2026-09-02T12:00:00.000Z", 1234],
  ]);
  ...
  for (const row of initialReceipts) {
    ...
    assert.equal(sha256(await readFile(join(roots.committed, row.file_object_key))), row.file_sha256);
  }

  await noInventedEffects(primary.pool);
  await assertNoAccountPersistence(primary.pool);
  checked("all occurrences accounted for; pending/rejected stale refunds ignored only by review; refunded means approval not payment");
  checked("missing/quarantined/null/wrong Person and wrong department ownership, malformed values, duplicate visual IDs and missing accounts quarantine");
  checked("real PDF/PNG/JPEG bytes; missing/traversal/outside symlink/digest/size/signature/oversize failures isolated");

  stage = "ActualAuthenticatedEncryption";
  const encrypted = initialReceipts[0]!;
  assert.equal(cipher.decrypt(encrypted.payment_account_ciphertext, encrypted.receipt_id), normalizedAccount);
  assert.throws(() => wrongCipher.decrypt(encrypted.payment_account_ciphertext, encrypted.receipt_id));
  assert.throws(() => cipher.decrypt(encrypted.payment_account_ciphertext, initialReceipts[1]!.receipt_id));
  const ciphertext = encrypted.payment_account_ciphertext;
  const tamperIndex = ciphertext.length - 4;
  const tampered = ciphertext.slice(0, tamperIndex) + (ciphertext[tamperIndex] === "A" ? "B" : "A") + ciphertext.slice(tamperIndex + 1);
  ...
  checked("real authenticated decryption, wrong key/context/tamper rejection, fresh nonces and no plaintext/unkeyed hash in SQL or output");
  ...
  checked("actual process restart exact replay preserves ciphertext, identities, dispositions and facts");
  ...
  assert.deepEqual(await receipts(primary.pool), initialReceipts);
  checked("fresh replay detects corrupt/deleted custody and restores original bytes from authorized archive");

  stage = "NativeEditPreservation";
  await primary.pool.query("UPDATE economy_receipts SET description='native edited receipt',amount_ore=4321,revision=revision+1 WHERE receipt_id=$1", [encrypted.receipt_id]);
  ...
  assert.deepEqual(edited.map((row) => row.payment_account_ciphertext), ciphertexts);
  checked("native edit remains authoritative; replay retains original encrypted import evidence and reports drift pending");
  ...
  await conflict("same-snapshot-review-collision", { ...review, evidenceRef: "changed-evidence" }, "SnapshotConflict");
  await conflict("cross-snapshot-review-collision", { ...review, snapshotId: "changed-snapshot", evidenceRef: "changed-evidence" }, "SourceConflict");
  await mysql(`UPDATE vektor.receipt SET description='${description}-changed' WHERE id=1`);
  const changedRows = legacyReceiptRows(await readLegacySourceSnapshot(sourceUrl, "NotRequested", "Include"), cipher);
  await conflict("changed-accepted-source", {
    ...review, snapshotId: "changed-source-snapshot", receiptSourceRevision: receiptSourceRevision(changedRows),
    entries: review.entries.map((entry) => ({ ...entry, sourceRowDigest: receiptSourceRowDigest(changedRows.find((row) => row.sourcePrimaryKey === entry.sourcePrimaryKey)!) })),
  }, "SourceConflict");
  ...
  const concurrentReports = await Promise.all([success(concurrent, concurrentRoots), success(concurrent, concurrentRoots)]);
  ...
  assert.deepEqual((await concurrent.pool.query("SELECT count(*)::int AS count FROM receipt_cohort_source_bindings")).rows, [{ count: 3 }]);
  await noInventedEffects(concurrent.pool);
  checked("concurrent real CLI imports serialize source ownership; one cohort, no duplicate receipt or effects");
  ...
  await collision.pool.query(`INSERT INTO economy_receipts(
    ...
    '2026-08-21T12:00:00Z','Pending',NULL,$1,'native-stage/unrelated.pdf','native/unrelated.pdf','application/pdf',$2,$3,4)`, [cipher.encrypt(account, "unrelated-native-receipt"), pdf.byteLength, sha256(pdf)]);
  ...
  assert.ok(collisionReport.occurrences.find((row) => row.sourcePrimaryKey === "1")?.reasons.includes("DuplicateVisualId"));
  assert.deepEqual((await receipts(collision.pool)).find((row) => row.receipt_id === "unrelated-native-receipt"), nativeBefore);
  ...
  assert.equal(sha256(await readFile(nativePath)), sha256(pdf));
  checked("persisted collision overrides classifier acceptance; prior dispositions replay; unrelated native bytes remain owned by native receipt");
  ...
  await rollback.pool.query(`CREATE FUNCTION fail_receipt_proof_insert() RETURNS trigger LANGUAGE plpgsql AS $$
  ...
  assert.equal(await targetFingerprint(rollback.pool), beforeRollback);
  assert.deepEqual(await tree(roots.committed), conflictFiles);
  await rollback.pool.query("DROP TRIGGER fail_receipt_proof_insert ON receipt_cohort_occurrences; DROP FUNCTION fail_receipt_proof_insert()");
  ...
  checked("real SQL failure after receipt writes rolls back whole cohort, cleans only owned staging, safe SQL error, retry succeeds");
  ...
  await writeFile(join(pendingRoots.committed, "committed"), "promotion-obstruction", { mode: 0o600 });
  ...
  for (const receipt of pendingReceipts) assert.equal(sha256(await readFile(join(pendingRoots.staging, receipt.file_ref))), receipt.file_sha256);
  const durablePending = (await pending.pool.query("SELECT reconciliation_result,count(*)::int AS count FROM economy_receipt_import_ledger WHERE result='Accepted' GROUP BY reconciliation_result")).rows;
  assert.deepEqual(durablePending, [{ reconciliation_result: "Pending", count: 3 }]);
  checked("actual filesystem promotion failure commits explicit Pending metadata and retains staged bytes");
  ...
  await runLocal(["pg_restore", "--dbname", restore.url, "--clean", "--if-exists", "--exit-on-error", "--no-owner", dumpPath]);
  const restoredRoot = join(temporaryRoot, "restored-files");
  await mkdir(restoredRoot, { mode: 0o700 });
  const restoredRoots = { archive: join(restoredRoot, "archive"), staging: join(restoredRoot, "staging"), committed: join(restoredRoot, "private") };
  ...
  for (const receipt of pendingReceipts) assert.equal(sha256(await readFile(join(restoredRoots.committed, receipt.file_object_key))), receipt.file_sha256);
  ...
  checked("real pg_dump/pg_restore plus private-file restore resumes incomplete work in new CLI process; missing stage rebuilt; original ciphertext and facts retained");

  stage = "SyntheticAdapterCompatibility";
  const syntheticFiles = FileCustody.makeReceiptFileStore({ stagingRoot: join(temporaryRoot, "synthetic-stage"), committedRoot: join(temporaryRoot, "synthetic-committed") });

  const syntheticRow = {
    sourcePrimaryKey: "synthetic-source", destinationIdentity: "synthetic-target", sourceUser: "synthetic-owner", sourceDepartment: "synthetic-department",
    visualId: "SYNTHETIC-ONLY", amountDecimal: "1.25", description: "Synthetic adapter compatibility", receiptDate: "2026-08-20",
    submittedAt: "2026-08-21T12:00:00Z", status: "pending", refundDate: null,
    file: { path: "pending.pdf", sha256: sha256(pdf), byteLength: pdf.length, contentType: "application/pdf" },
  };

  const { sourcePrimaryKey, destinationIdentity, ...syntheticData } = syntheticRow;

  const syntheticInput = {
    kind: "synthetic-receipt-import-0095", sourceRepository: "synthetic", sourceRevision: "1", snapshotId: "1", sourceWatermark: "0", transformationRevision: "1",
    persons: [{ sourceUser: "synthetic-owner", personId: "synthetic-owner", syntheticPaymentAccount: "synthetic:0095:not-a-payment-account" }],
    departments: [{ sourceDepartment: "synthetic-department", departmentId: "synthetic-department" }],
    rows: [{ sourcePrimaryKey, destinationIdentity, data: syntheticData, rowDigest: rowDigest(syntheticRow) }],
  };

  const synthetic = await prepareReceiptSnapshot(decodeSnapshot(syntheticInput), roots.archive, syntheticFiles);
  assert.equal(synthetic.results[0]?._tag, "AcceptedReceiptImport");
  assert.throws(() => decodeSnapshot({ ...syntheticInput, persons: [{ ...syntheticInput.persons[0], syntheticPaymentAccount: account }] }));
  assert.throws(() => decodeSnapshot({ ...syntheticInput, kind: "legacy-receipt-import" }));
  checked("existing bounded synthetic adapter still accepts only its explicit non-payment fixture account and kind");

  const report = {
    scope: "synthetic-reviewed-receipt-reconciliation", schemaRevision: databaseSchemaRevision,
    source: { repository, revision: baseRevision, receiptRevision: financeRevision, transformationRevision: review.transformationRevision, watermark: sourceWatermark },
    counts: { input: first.input, accepted: first.accepted, quarantined: first.quarantined, excluded: first.excluded },
    checks, noInventedEffects: await noInventedEffects(primary.pool),
    cli: { command: "bun --no-env-file tools/e2e/run-legacy-receipt-import.ts --review=<private-json> --archive-root=<private-archive> --staging-root=<private-stage> --committed-root=<private-store> --payment-key=<private-key> --source-env=RECEIPT_REHEARSAL_SOURCE --target-env=RECEIPT_REHEARSAL_TARGET --target-database=<disposable-database> --organization-source=none", transport: "private Unix sockets", databaseFixture: "invented MariaDB InnoDB rows" },
    limits: ["No historical data or provider access", "No payment or settlement proof", "Historical import requires a real file archive and resolved ownership, department and account evidence", "CLI process restart and logical database/file restore; no power-loss simulation"],
  };
...
const main = async (): Promise<void> => {
  ...
  if (!(relativeEvidence === ".." || relativeEvidence.startsWith(`..${sep}`) || isAbsolute(relativeEvidence))) throw new Error("Evidence must remain outside the product repository");
  process.umask(0o077);
  ...
  await privateJson(evidenceFile, {
    ...report, testedSource: { commit: sourceCommit, archiveSha256: sha256(await readFile(sourceArchive)) },
    rehearsalCommand: "bun --no-env-file tools/e2e/run-legacy-receipt-rehearsal.ts --evidence-dir=<new-private-directory>", ownedDatabasesCleaned: true,
  });

  for (const [path, mode] of [[evidenceDirectory, 0o700], [evidenceFile, 0o600], [sourceArchive, 0o600]] as const) {
  ...
  console.log(JSON.stringify({ scope: report.scope, counts: report.counts, checks: report.checks.length, evidence: "owner-only-report-and-checksummed-source", ownedDatabasesCleaned: true }, null, 2));
};

if (process.argv[1] === fileURLToPath(import.meta.url)) await main().catch(() => {
