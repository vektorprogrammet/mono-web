...
const referenceExistsAtSourceRevision = (revision: string, path: string): boolean => {
  try {
  ...
  await Promise.all(
    references.map(async (reference) => {
      try {
        ...
        if (!referenceExistsAtSourceRevision(state.sourceRevision, repositoryPath(reference)))
