...
import { generateFromRootsEffect, RunOptionsSchema } from "./src/runner.js";
...
const FreshReplayInputSchema = Schema.Struct({
  options: RunOptionsSchema,
  fixtureRuntimeInput: Schema.NullOr(
    Schema.Struct({
      path: Schema.String,
      bytes: Schema.Array(Schema.Int.check(Schema.isBetween({ minimum: 0, maximum: 255 }))),
    }),
  ),
  fixtureIntentBytes: Schema.NullOr(
...
const program = Effect.gen(function* () {
  ...
  const input = Schema.decodeUnknownSync(Schema.fromJsonString(FreshReplayInputSchema))(serialized);
