import { Array as Arr, flow, Predicate, Result, Schema } from "effect";
...
import { hasDuplicateJsonMembers, isJsonObject } from "./json-safety.js";
import type {
  Disposition,
  InventoryEnvelope,
  InventoryKind,
...
const MISMATCH_DISPOSITIONS = new Map<string, AcceptedDisposition>([
...
const FORBIDDEN_INTENT_KINDS = new Set<MismatchKind>([
  "duplicate",
  "stale",
  "openapi_stale",
  "unresolved",
...
const safeIntentScalar = (value: unknown, field: string): value is string =>
  Predicate.isString(value) &&
  value.length <= 128 &&
  SAFE_INTENT_SCALAR.test(value) &&
  unsafeScalarReason(value, field) === null;

const safeCanonicalSignature = (value: string): boolean =>
  value.length <= 512 && unsafeScalarReason(value, "accepted_intent_signature") === null;
...
const stringArray = (value: unknown): value is readonly string[] =>
  Array.isArray(value) && value.every(stringValue) && new Set(value).size === value.length;

const isDisposition = (value: unknown): value is AcceptedDisposition =>
  Predicate.isString(value) && ACCEPTED_DISPOSITIONS.some((disposition) => disposition === value);

const isSurface = (value: unknown): value is JourneySurface =>
  Predicate.isString(value) && SURFACES.some((surface) => surface === value);

const isCoverageScope = (value: unknown): value is CoverageScope =>
  value === "user_visible" ||
  value === "operator_visible" ||
  value === "background" ||
  value === "accepted_non_user_facing";

const sorted = <T extends string>(values: readonly T[]): T[] => sortUnique(values);

const issue = (
  reasonCode: string,
  status: CoverageIssue["status"] = "accepted_intent_invalid",
  rowIds: readonly string[] = [],
  sourceRefIds: readonly string[] = [],
  acceptedIntentRefIds: readonly string[] = [],
  runtimeEvidenceRefIds: readonly string[] = [],
): CoverageIssue => {
  const output = {
    status,
    reasonCode,
  ...
  if (!(runtimeEvidenceRefIds.length === 0))
    Object.assign(output, { runtimeEvidenceRefIds: sorted(runtimeEvidenceRefIds) });

  return output;
};

const currentRevisionIds = (revisions: readonly RevisionRecord[]): string[] =>
  sorted(revisions.map((revision) => revision.revision_ref_id));

export const acceptedIntentRevisionRefId = (context: ManifestContext): string =>
...
const canonicalSignature = (value: string): boolean => {
  if (
    ...
    value.includes("..") ||
    value.includes("/tmp/") ||
    /(?:^|[^A-Za-z])(?:src|rev)-[a-f0-9]{16,}(?:$|[^A-Za-z0-9])/.test(value)
  )
    return false;
...
const intentPayload = (intent: Omit<AcceptedIntentRecord, "intent_digest">) => ({
...
const journeyPayload = (journey: Omit<AcceptedJourneyRecord, "journey_digest">) => ({
...
const parseStep = (value: JourneyStep, index: number): StepDecodeResult => {
  ...
  if (
    !safeIntentScalar(value.step_id, "journey_step") ||
    !isSurface(value.surface) ||
    !stringArray(rowIds) ||
    !stringArray(signatures) ||
    (expected !== null && !safeIntentScalar(expected, "expected_contract_ref")) ||
    !stringArray(runtimeEvidenceRefs)
  )
    return { step: null, issues: [issue("INTENT_SCHEMA_INVALID")] };
  const canonicalSignatures = sorted(signatures);

  if (rowIds.length === 0 && canonicalSignatures.length === 0)
    return { step: null, issues: [issue("COVERAGE_REF_REQUIRED")] };

  if (
    canonicalSignatures.some(
      (signature) => !canonicalSignature(signature) || !safeCanonicalSignature(signature),
    )
  )
    return { step: null, issues: [issue("CANONICAL_SIGNATURE_INVALID")] };

  if (
    rowIds.some((rowIdValue) => !ROW_ID.test(rowIdValue)) ||
    runtimeEvidenceRefs.some((ref) => !RECEIPT_ID.test(ref))
  )
    return { step: null, issues: [issue("ROW_REF_INVALID")] };
...
type IntentRecordDecodeResult = {
  readonly intent: AcceptedIntentRecord | null;
  readonly issues: readonly CoverageIssue[];
};

const parseIntent = (
  value: AcceptedIntentRecord,
  expectedRevisionIds: readonly string[],
): IntentRecordDecodeResult => {
  ...
  if (
    !safeIntentScalar(intentRef, "intent_ref_id") ||
    !REF_ID.test(intentRef) ||
    !safeIntentScalar(intentRevision, "intent_revision") ||
    !stringValue(digest) ||
    !DIGEST.test(digest) ||
    !stringArray(revisions) ||
    !stringArray(sourceRefs) ||
    !stringValue(purpose) ||
    !stringArray(rowIds) ||
    !stringArray(signatures) ||
    !stringArray(kinds) ||
    !stringArray(journeyRefs) ||
    (purpose !== "coverage" && purpose !== "disposition") ||
    (disposition !== null && !isDisposition(disposition))
  )
    return { intent: null, issues: [issue("INTENT_SCHEMA_INVALID")] };

  if (
    !revisions.every((revision) => REVISION_ID.test(revision)) ||
    sorted(revisions).join("\u0000") !== sorted(expectedRevisionIds).join("\u0000")
  )
    return { intent: null, issues: [issue("ACCEPTED_INTENT_STALE", "accepted_intent_invalid")] };

  if (
    sourceRefs.some((sourceRef) => !SOURCE_ID.test(sourceRef)) ||
    rowIds.some((rowIdValue) => !ROW_ID.test(rowIdValue)) ||
    kinds.some((kind) => !PARITY_KINDS.has(kind)) ||
    journeyRefs.some((journeyRef) => !REF_ID.test(journeyRef))
  )
    return { intent: null, issues: [issue("INTENT_REF_INVALID", "accepted_intent_invalid")] };
  const canonicalSignatures = sorted(signatures);
  ...
  if (
    ...
    return {
      intent: null,
      issues: [issue("CANONICAL_SIGNATURE_INVALID", "accepted_intent_invalid")],
    };

  if (purpose === "disposition" && disposition === null)
    return { intent: null, issues: [issue("INTENT_DISPOSITION_REQUIRED")] };

  if (purpose === "coverage" && disposition !== null)
    return { intent: null, issues: [issue("INTENT_PURPOSE_CONFLICT")] };

  if (purpose === "coverage" && journeyRefs.length === 0)
    return { intent: null, issues: [issue("JOURNEY_REF_REQUIRED")] };

  if (purpose === "disposition" && rowIds.length === 0 && canonicalSignatures.length === 0)
    return { intent: null, issues: [issue("ACCEPTED_INTENT_REQUIRED")] };
  ...
  const intent: AcceptedIntentRecord = {
    ...
    row_ids: sorted(rowIds),
    canonical_signatures: canonicalSignatures,
    inventory_kinds: sorted(kinds),
    journey_ref_ids: sorted(journeyRefs),
  };

  const expectedDigest = sha256(canonicalJson(intentPayload(intent)));

  if (expectedDigest !== intent.intent_digest)
    return { intent: null, issues: [issue("ACCEPTED_INTENT_STALE")] };

  return { intent, issues: [] };
};

type JourneyDecodeResult = {
  readonly journey: AcceptedJourneyRecord | null;
  readonly issues: readonly CoverageIssue[];
};

const parseJourney = (
  value: AcceptedJourneyRecord,
  expectedRevisionIds: readonly string[],
): JourneyDecodeResult => {
  ...
  if (
    !safeIntentScalar(journeyRef, "journey_ref_id") ||
    !REF_ID.test(journeyRef) ||
    !safeIntentScalar(journeyKey, "journey_key") ||
    !safeIntentScalar(intentRef, "intent_ref_id") ||
    !REF_ID.test(intentRef) ||
    !safeIntentScalar(revision, "journey_revision") ||
    !stringValue(digest) ||
    !DIGEST.test(digest) ||
    !stringArray(revisions) ||
    !stringArray(sourceRefs) ||
    !Array.isArray(rawSteps) ||
    !isCoverageScope(value.coverage_scope)
  )
    return { journey: null, issues: [issue("INTENT_SCHEMA_INVALID")] };

  if (
    !revisions.every((revisionRef) => REVISION_ID.test(revisionRef)) ||
    sorted(revisions).join("\u0000") !== sorted(expectedRevisionIds).join("\u0000")
  )
    return { journey: null, issues: [issue("ACCEPTED_INTENT_STALE")] };

  if (sourceRefs.some((sourceRef) => !SOURCE_ID.test(sourceRef)))
    return { journey: null, issues: [issue("INTENT_REF_INVALID")] };
  ...
  if (steps.length === 0 || new Set(steps.map((step) => step.step_id)).size !== steps.length)
    issues.push(issue("JOURNEY_STEPS_INVALID"));
...
const decodeIntent = (
  value: AcceptedIntentRegister,
  expectedRevisionIds: readonly string[],
): IntentDecodeResult => {
  if (value.intents.length === 0 && value.journeys.length === 0)
    return { register: null, issues: [issue("ACCEPTED_INTENT_EMPTY")] };
  const issues: CoverageIssue[] = [];
  ...
  for (const journey of journeys) {
    ...
    if (
      owner === undefined ||
      owner.purpose !== "coverage" ||
      !owner.journey_ref_ids.includes(journey.journey_ref_id)
    )
      issues.push(issue("JOURNEY_INTENT_OWNERSHIP_INVALID"));
  }

  for (const intent of intents) {
    if (intent.purpose !== "coverage") continue;

    const ownedJourneys = journeys.filter(
      (journey) => journey.intent_ref_id === intent.intent_ref_id,
    );

    const ownedRefs = new Set(ownedJourneys.map((journey) => journey.journey_ref_id));

    if (
      intent.journey_ref_ids.some((journeyRef) => !journeyRefs.has(journeyRef)) ||
      ownedRefs.size !== intent.journey_ref_ids.length ||
      intent.journey_ref_ids.some((journeyRef) => !ownedRefs.has(journeyRef))
    )
      issues.push(issue("JOURNEY_REF_MISSING"));

    if (
      (intent.row_ids.length > 0 || intent.canonical_signatures.length > 0) &&
      (ownedJourneys.length === 0 ||
        ownedJourneys.some((journey) => journey.coverage_scope !== "accepted_non_user_facing"))
    )
      issues.push(issue("INTENT_SCOPE_INVALID"));
  }

  if (intents.length === 0 && journeys.length === 0 && issues.length === 0)
    issues.push(issue("ACCEPTED_INTENT_EMPTY"));
  ...
  return {
    register: {
      ...
      journeys: journeys.sort((left, right) =>
        compareByteOrder(left.journey_ref_id, right.journey_ref_id),
      ),
    },
    issues: [],
  };
...
export const tryDecodeAcceptedIntentRegister = flow(
  Schema.decodeUnknownResult(
    Schema.Tuple([
      Schema.declare(validateAcceptedIntentRegister),
      Schema.optionalKey(Schema.Array(Schema.String)),
    ]),
  ),
  Result.match({
    onFailure: (): IntentDecodeResult => ({
      register: null,
      issues: [issue("INTENT_SCHEMA_INVALID")],
    }),
    onSuccess: ([value, expectedRevisionIds]) =>
      decodeIntent(value, sorted(expectedRevisionIds ?? [])),
  }),
);

export const decodeAcceptedIntentRegister = flow(
  tryDecodeAcceptedIntentRegister,
  (result): AcceptedIntentRegister => {
    if (result.register === null)
      throw new Error(result.issues[0]?.reasonCode ?? "accepted intent register is invalid");
...
const acceptedIntentSource = (context: ManifestContext, input: IntentSourceInput): string => {
  if (!context.revisions.some((revision) => revision.revision_ref_id === input.revisionRefId)) {
    context.revisions.push({
      ...
      revision_kind:
        input.repositoryRef === "external_intent_authority" ? "git_commit" : "file_set_digest",
  ...
  const existing = context.sources.find(
    (source) => source.authority_role === "accepted_intent_register" && source.path === input.path,
  );
...
const unsafeIntentText = (text: string): boolean =>
  /"(?:password|passwd|secret|secrets|token|access[_-]?token|refresh[_-]?token|api[_-]?key|authorization|client[_-]?secret|payload|raw[_-]?payload|user[_-]?id|account[_-]?id|customer[_-]?id|email|phone)"\s*:/i.test(
    text,
  ) ||
  /(?:sk_(?:live|test)_|bearer\s+|(?:password|passphrase|secret|token|api[_-]?key|payload|email|phone|user[_-]?id|account[_-]?id|customer[_-]?id)\s*[:=]|[A-Z0-9._%+-]+@[^\s]+)/i.test(
    text,
  );

const hasForbiddenIntentKey = (entry: Schema.Json): boolean => {
  if (Arr.isArray<Schema.Json>(entry)) return entry.some(hasForbiddenIntentKey);

  if (!isJsonObject(entry)) return false;

  return Object.entries(entry).some(
    ([key, child]) =>
      /^(?:password|passwd|secret|secrets|token|access[_-]?token|refresh[_-]?token|api[_-]?key|authorization|client[_-]?secret|payload|raw[_-]?payload|user[_-]?id|account[_-]?id|customer[_-]?id|member[_-]?id|identity[_-]?id|email|phone)(?:[_-].*)?$/i.test(
        key,
      ) || hasForbiddenIntentKey(child),
  );
};
...
export const loadAcceptedIntentRegister = (
  context: ManifestContext,
  supplied?: IntentSourceInput,
): IntentLoadResult => {
  ...
  if (input === null)
    return {
      path: null,
      sourceRefId: null,
      sourceRevisionRefId: null,
      sourceDigest: null,
      register: null,
      issues: [issue("ACCEPTED_INTENT_MISSING")],
    };
  ...
  try {
    ...
    return {
      path: textPath,
      sourceRefId: null,
      sourceRevisionRefId: input.revisionRefId,
      sourceDigest: input.digest || null,
      register: null,
      issues: [issue(reason)],
    };
  }
  ...
  if (input.digest.length > 0 && input.digest !== digest)
    return {
      path: textPath,
      sourceRefId: null,
      sourceRevisionRefId: input.revisionRefId,
      sourceDigest: null,
      register: null,
      issues: [issue("INTENT_DIGEST_MISMATCH")],
    };
  ...
  const register = {
    ...result.register,
    intents: result.register.intents.map((intent) => ({
      ...intent,
      source_ref_ids: sortUnique([...intent.source_ref_ids, sourceRefId]),
    })),
    journeys: result.register.journeys.map((journey) => ({
      ...journey,
      source_ref_ids: sortUnique([...journey.source_ref_ids, sourceRefId]),
    })),
  };
  ...
  return {
    path: textPath,
    sourceRefId,
    sourceRevisionRefId: normalizedInput.revisionRefId,
    sourceDigest: digest,
    register,
    issues: [],
  };
...
const emptyJourneyEnvelope = (
  manifestDigest: string,
  revisionRefIds: readonly string[],
): InventoryEnvelope => ({
...
): ReadonlyMap<JourneySurface, readonly InventoryRow[]> => {
  const result = new Map<JourneySurface, InventoryRow[]>();

  for (const inventory of inventories) {
    if (inventory.inventory_kind === "user_journey") continue;
    const existing = result.get(inventory.inventory_kind);
    const rows = existing === undefined ? [] : [...existing];
    rows.push(...inventory.rows);
    result.set(inventory.inventory_kind, rows);
  }
...
const updateCoverage = (row: InventoryRow, refs: readonly string[]): InventoryRow => {
  ...
  return {
    ...row,
    coverage_ref_ids: coverageRefs,
    observation_kinds: sortUnique([...row.observation_kinds, "accepted_intent"]),
  };
};
...
const resolveTargets = (step: JourneyStep, rows: readonly InventoryRow[]): ResolvedTargets => {
  ...
  for (const signature of step.canonical_signatures) {
    ...
    const reconciledMatches =
      matches.length > 1 &&
      matches.every((row) =>
        row.related_row_ids.some((relatedRowId) => matchedIds.has(relatedRowId)),
      );
  ...
  return {
    rows: [...unique.values()].sort((left, right) => compareByteOrder(left.row_id, right.row_id)),
    issues: sorted(missing),
  };
};

type IntentTargets = { readonly rows: readonly InventoryRow[]; readonly issues: readonly string[] };

const intentTargets = (
  intent: AcceptedIntentRecord,
  ...
  for (const signature of intent.canonical_signatures) {
    const matches = rows.filter(
      (row) =>
        ...
        (intent.inventory_kinds.length === 0 ||
          intent.inventory_kinds.includes(row.inventory_kind)),
    );

    if (matches.length === 0) issues.push("CANONICAL_SIGNATURE_MISSING");
    else if (matches.some((row) => !intent.row_ids.includes(row.row_id)))
      issues.push("CANONICAL_SIGNATURE_BROAD");
    selected.push(...matches);
  }
...
const dispositionFor = (kind: MismatchKind): AcceptedDisposition | null =>
  MISMATCH_DISPOSITIONS.get(kind) ?? null;

const applyDispositions = (
  inventories: readonly InventoryEnvelope[],
  ...
  const changes = new Map<
    string,
    { readonly intent: AcceptedIntentRecord; readonly disposition: AcceptedDisposition }
  >();
  ...
  for (const intent of register.intents) {
    ...
    if (target.issues.length > 0 || target.rows.length === 0) {
      issues.push(
        issue(
          target.issues[0] ?? "ACCEPTED_INTENT_REQUIRED",
          "accepted_intent_invalid",
          intent.row_ids,
          intent.source_ref_ids,
    ...
    const kind = expectedKinds.values().next().value;

    if (
      expectedKinds.size !== 1 ||
      kind === undefined ||
      kind === "none" ||
      FORBIDDEN_INTENT_KINDS.has(kind)
    ) {
      issues.push(
        issue(
          "ACCEPTED_INTENT_FORBIDDEN_KIND",
          "accepted_intent_invalid",
          target.rows.map((row) => row.row_id),
          intent.source_ref_ids,
          [intent.intent_ref_id],
        ),
      );
    ...
    if (dispositionFor(kind) !== intent.disposition) {
      issues.push(
        issue(
          "ACCEPTED_INTENT_DISPOSITION_MISMATCH",
          "accepted_intent_invalid",
          target.rows.map((row) => row.row_id),
          intent.source_ref_ids,
          [intent.intent_ref_id],
    ...
    for (const row of target.rows) {
      ...
      if (existing !== undefined && existing.intent.intent_ref_id !== intent.intent_ref_id) {
        issues.push(
          issue(
            "ACCEPTED_INTENT_AMBIGUOUS",
            "accepted_intent_invalid",
            [row.row_id],
            intent.source_ref_ids,
            [existing.intent.intent_ref_id, intent.intent_ref_id],
          ),
        );
      ...
      rows: inventory.rows
        .map((row): InventoryRow => {
          ...
          return {
            ...
            accepted_intent_ref_ids: sortUnique([
              ...row.accepted_intent_ref_ids,
              change.intent.intent_ref_id,
            ]),
            observation_kinds: sortUnique([...row.observation_kinds, "accepted_intent"]),
            mismatch: {
              ...row.mismatch,
              ...
              accepted_intent_ref_ids: sortUnique([
                ...row.mismatch.accepted_intent_ref_ids,
                change.intent.intent_ref_id,
              ]),
            },
          };
...
const runnerDigestForSources = (
  ...
  const pairs = sourceRefIds
    .map((sourceRefId) => {
      ...
      if (source === undefined) throw new Error("source reference disappeared");
...
const resolveRuntimeEvidence = (
  ...
  const journeysByRef = new Map(
    register.journeys.map((journey) => [journey.journey_ref_id, journey]),
  );
  ...
  for (const receipt of receipts) {
    ...
    if (journey === undefined) {
      resolutions.set(`receipt:${receipt.receipt_ref_id}`, {
        ...
        issues: [
          issue(
            "RUNTIME_EVIDENCE_UNKNOWN_JOURNEY",
            "unresolved",
            [],
            [],
            [],
    ...
    const unknownSources = receipt.runner_source_ref_ids.filter(
    ...
    const receiptIssue = (
      ...
      sourceRefIds: readonly string[] = [],
    ): CoverageIssue => issue(reasonCode, status, [], sourceRefIds, [], [receipt.receipt_ref_id]);

    const receiptIssues: CoverageIssue[] = [];

    if (stale)
      receiptIssues.push(
        receiptIssue("RUNTIME_EVIDENCE_STALE", "accepted_intent_invalid", unknownSources),
      );

    if (unknownSteps.length > 0)
      receiptIssues.push(
        receiptIssue("RUNTIME_EVIDENCE_UNKNOWN_STEP", "unresolved", unknownSources),
      );

    if (unknownSources.length > 0)
      receiptIssues.push(
        receiptIssue("RUNTIME_EVIDENCE_SOURCE_REF_MISSING", "unresolved", unknownSources),
      );

    if (runnerDigestResolution.unavailable.length > 0)
      receiptIssues.push(
        receiptIssue(
          "RUNTIME_EVIDENCE_RUNNER_SOURCE_UNAVAILABLE",
          "unresolved",
          runnerDigestResolution.unavailable,
        ),
      );
    else if (runnerDigestResolution.digest !== receipt.runner_digest)
      receiptIssues.push(
        receiptIssue(
          "RUNTIME_EVIDENCE_RUNNER_DIGEST_MISMATCH",
          "unresolved",
          receipt.runner_source_ref_ids,
        ),
      );

    if (receipt.result === "failed")
      receiptIssues.push(receiptIssue("RUNTIME_EVIDENCE_FAILED", "gaps_found", unknownSources));
    ...
    for (const step of journey.steps) {
      ...
      resolutions.set(key, {
        step,
        receiptIds: sortUnique([
          ...existingReceiptIds,
          ...(receiptIssues.some((entry) => entry.reasonCode === "RUNTIME_EVIDENCE_UNKNOWN_STEP")
            ? []
            : [receipt.receipt_ref_id]),
        ]),
  ...
  for (const journey of register.journeys) {
    for (const step of journey.steps) {
      ...
      const declaredReceipts = receipts.filter((receipt) =>
        declared.includes(receipt.receipt_ref_id),
      );

      const crossJourney = declaredReceipts.filter(
        (receipt) => receipt.journey_ref_id !== journey.journey_ref_id,
      );

      const unknownDeclared = declared.filter(
        (ref) => !receipts.some((receipt) => receipt.receipt_ref_id === ref),
      );

      const issues = [...(existing?.issues ?? [])];

      if (crossJourney.length > 0)
        issues.push(
          issue("RUNTIME_EVIDENCE_CROSS_JOURNEY", "unresolved", [], [], [journey.intent_ref_id]),
        );

      if (unknownDeclared.length > 0)
        issues.push(
          issue("RUNTIME_EVIDENCE_UNKNOWN_REF", "unresolved", [], [], [journey.intent_ref_id]),
        );

      const receiptIds = sortUnique([
        ...(existing?.receiptIds ?? []),
        ...declaredReceipts
          .filter((receipt) => receipt.journey_ref_id === journey.journey_ref_id)
          .map((receipt) => receipt.receipt_ref_id),
      ]);
      ...
      if (
        requireEvidence &&
        journey.coverage_scope === "user_visible" &&
        successfulReceiptIds.length === 0
      ) {
        issues.push(
          issue("RUNTIME_EVIDENCE_REQUIRED", "gaps_found", [], journey.source_ref_ids, [
...
export const resolveJourneyCoverage = (params: {
  ...
  if (params.register === null) {
    ...
    return {
      register: null,
      userJourneyCoverage: empty,
      inventories: params.inventories,
      links: [],
      issues,
      coverageRefIds: new Map(),
    };
  }
  ...
  const journeyRows: InventoryRow<"user_journey">[] = [];
  ...
  for (const journey of params.register.journeys) {
    ...
    for (const step of journey.steps) {
      ...
      if (runtime !== undefined) {
        if (runtime.issues.length > 0) {
          unresolved = true;
          issues.push(
            ...runtime.issues.map((entry) => ({
              ...entry,
              rowIds: entry.rowIds,
              sourceRefIds: sortUnique([
                ...entry.sourceRefIds,
                ...journey.source_ref_ids,
                ...(sourceRefId === null ? [] : [sourceRefId]),
              ]),
              acceptedIntentRefIds: sortUnique([
                ...entry.acceptedIntentRefIds,
                journey.intent_ref_id,
              ]),
            })),
          );
      ...
      if (targets.issues.length > 0) {
        ...
        issues.push(
          issue(
            targets.issues[0] ?? "COVERAGE_REF_REQUIRED",
            "unresolved",
            targets.rows.map((row) => row.row_id),
            [...journey.source_ref_ids, ...(sourceRefId === null ? [] : [sourceRefId])],
            [journey.intent_ref_id],
          ),
        );
      }
    ...
    const signature = canonicalJson([
      ...
      journey.intent_ref_id,
      journey.journey_key,
      journey.steps.map((step) => step.step_id),
    ]);

    const journeyRow: InventoryRow<"user_journey"> = {
      ...
      observation_kinds: ["accepted_intent"],
      source_ref_ids: sortUnique([
        ...journey.source_ref_ids,
        ...(sourceRefId === null ? [] : [sourceRefId]),
      ]),
      revision_ref_ids: revisionRefIds,
      runtime_observation_ref_ids: sortUnique(
        resolvedSteps.flatMap((step) => step.runtime_evidence_ref_ids),
      ),
    ...
    for (const row of targetRows.values()) {
      ...
      links.push({
        relation_id: relationId("covers", journeyRow.row_id, row.row_id, sourceRefs),
        relation_kind: "covers",
        from_row_id: journeyRow.row_id,
        to_row_id: row.row_id,
        source_ref_ids: sourceRefs,
      });
    }
  }

  const journeyRowsByRef = new Map(journeyRows.map((row) => [row.details.journey_ref_id, row]));
  ...
  for (const intent of params.register.intents) {
    ...
    if (target.issues.length > 0) {
      issues.push(
        issue(
          target.issues[0] ?? "COVERAGE_REF_REQUIRED",
          "accepted_intent_invalid",
          intent.row_ids,
          intent.source_ref_ids,
          [intent.intent_ref_id],
    ...
    const ownedJourneys = params.register.journeys.filter((journey) =>
      intent.journey_ref_ids.includes(journey.journey_ref_id),
    );

    const directScopeAllowed =
      ownedJourneys.length > 0 &&
      ownedJourneys.every((journey) => journey.coverage_scope === "accepted_non_user_facing");
    ...
    const linkedJourneyRowIds = new Set(
      intent.journey_ref_ids.flatMap(
        (journeyRef) => journeyRowsByRef.get(journeyRef)?.related_row_ids ?? [],
      ),
    );

    if (target.rows.some((row) => !linkedJourneyRowIds.has(row.row_id)) && !directScopeAllowed)
      issues.push(
        issue(
          "INTENT_JOURNEY_LINK_MISMATCH",
          "accepted_intent_invalid",
          target.rows.map((row) => row.row_id),
          intent.source_ref_ids,
          [intent.intent_ref_id],
  ...
  let resolvedInventories: readonly InventoryEnvelope[] = params.inventories.map((inventory) => ({
    ...inventory,
    rows: inventory.rows.map((row) => {
      const updated = updateCoverage(row, coverageByRow.get(row.row_id) ?? []);
      const acceptedRefs = acceptedNonUserFacingByRow.get(row.row_id) ?? [];

      return acceptedRefs.length === 0
        ? updated
        : {
            ...updated,
            accepted_intent_ref_ids: sortUnique([
              ...updated.accepted_intent_ref_ids,
              ...acceptedRefs,
            ]),
          };
    }),
    ...
    if (row.status === "covered") {
      const updated: InventoryRow = {
        ...
        mismatch: {
          ...
          disposition: "none",
          accepted_intent_ref_ids: [],
          reason: "COVERAGE_REF_REQUIRED",
        },
        reason_codes: sortUnique([...row.reason_codes, "COVERAGE_REF_REQUIRED"]),
      };

      resolvedInventories = resolvedInventories.map((inventory) =>
        inventory.rows.some((candidate) => candidate.row_id === row.row_id)
          ? {
              ...inventory,
              rows: inventory.rows.map((candidate) =>
                candidate.row_id === row.row_id ? updated : candidate,
              ),
            }
          : inventory,
      );
  ...
  const journeyCoverage: InventoryEnvelope = {
    ...
    derivation_edges: journeyRows
      .values()
      .filter((row) => row.related_row_ids.length > 0)
      .map((row) => ({
        edge_id: edgeId("coverage", row.source_ref_ids, row.related_row_ids),
        edge_type: "coverage" as const,
        from_ref_ids: row.source_ref_ids,
        to_row_ids: row.related_row_ids,
        derivation: "accepted journey step covers exact row or canonical signature",
      }))
      .toArray()
      .sort((left, right) => compareByteOrder(left.edge_id, right.edge_id)),
  };
  ...
  return {
    ...
    issues,
    coverageRefIds: new Map(
      [...coverageByRow.entries()].map(([rowIdValue, refs]) => [rowIdValue, sorted(refs)]),
    ),
  };
};
  ...
  if (kind === "none")
    return (
      ...
      (row.status === "not_applicable" &&
        row.mismatch.disposition === "accepted_not_applicable" &&
        row.accepted_intent_ref_ids.length > 0)
    );

  if (kind === "uncovered")
    return (
      row.status === "uncovered" &&
      row.mismatch.disposition === "none" &&
      row.mismatch.accepted_intent_ref_ids.length === 0
    );

  if (FORBIDDEN_INTENT_KINDS.has(kind))
    return row.status === kind || (kind === "openapi_stale" && row.status === "stale");

  if (row.status === "accounted")
    return (
      row.mismatch.disposition === MISMATCH_DISPOSITIONS.get(kind) &&
      row.mismatch.accepted_intent_ref_ids.length > 0
    );

  return (
    row.status === kind &&
    (row.mismatch.disposition === "none" || row.mismatch.disposition === "rejected") &&
    row.mismatch.accepted_intent_ref_ids.length === 0
  );
};
...
export const validateCrossArtifactInvariants = (input: CrossArtifactInvariantInput): boolean => {
  ...
  const runtimeIds = new Set(
    input.manifest.runtime_observations.map(
      (observation) => observation.runtime_observation_ref_id,
    ),
  );
  ...
  const journeyRefs = new Set(
    input.register?.journeys.map((journey) => journey.journey_ref_id) ?? [],
  );
  ...
  for (const row of rows) {
    if (
      ...
      row.revision_ref_ids.length === 0 ||
      !row.source_ref_ids.every((sourceRef) => sourceIds.has(sourceRef)) ||
      !row.revision_ref_ids.every((revisionRef) => revisionIds.has(revisionRef)) ||
      !row.runtime_observation_ref_ids.every((runtimeRef) => runtimeIds.has(runtimeRef)) ||
      !row.coverage_ref_ids.every(
        (coverageRef) => journeyRefs.has(coverageRef) || intentIds.has(coverageRef),
      ) ||
      !row.accepted_intent_ref_ids.every((intentRef) => intentIds.has(intentRef)) ||
      !row.related_row_ids.every((related) =>
        rows.some((candidate) => candidate.row_id === related),
      ) ||
      !rowStatusMismatchValid(row)
    )
      return false;
  ...
  for (const intent of input.register?.intents ?? []) {
    if (
      !intent.source_ref_ids.every((sourceRef) => sourceIds.has(sourceRef)) ||
      !intent.selected_revision_ref_ids.every((revisionRef) => revisionIds.has(revisionRef)) ||
      !intent.row_ids.every((rowIdValue) => rowIds.has(rowIdValue)) ||
      !intent.canonical_signatures.every((signature) =>
        rows.some((row) => row.signature === signature),
      ) ||
      !intent.journey_ref_ids.every((journeyRef) => journeyRefs.has(journeyRef))
    )
      return false;

    if (intent.purpose === "coverage") {
      const ownedJourneys =
        input.register?.journeys.filter(
          (journey) => journey.intent_ref_id === intent.intent_ref_id,
        ) ?? [];
  ...
  for (const journey of input.register?.journeys ?? []) {
    if (
      !journey.source_ref_ids.every((sourceRef) => sourceIds.has(sourceRef)) ||
      !journey.selected_revision_ref_ids.every((revisionRef) => revisionIds.has(revisionRef)) ||
      !intentIds.has(journey.intent_ref_id)
    )
      return false;

    const owner = input.register?.intents.find(
      (intent) => intent.intent_ref_id === journey.intent_ref_id,
    );
  ...
  for (const link of [...input.links, ...input.userJourneyCoverage.links]) {
    if (
      ...
      !rowIds.has(link.to_row_id) ||
      link.source_ref_ids.length === 0 ||
      !link.source_ref_ids.every(
        (sourceRef) =>
          sourceIds.has(sourceRef) || intentIds.has(sourceRef) || journeyRefs.has(sourceRef),
      )
    )
      return false;
  }
  ...
  for (const journey of input.register?.journeys ?? []) {
    const intent = input.register?.intents.find(
      (candidate) => candidate.intent_ref_id === journey.intent_ref_id,
    );

    const row = input.userJourneyCoverage.rows.find(
      (candidate) =>
        candidate.inventory_kind === "user_journey" &&
        candidate.details.journey_ref_id === journey.journey_ref_id,
    );

    if (
      intent === undefined ||
      intent.purpose !== "coverage" ||
      !intent.journey_ref_ids.includes(journey.journey_ref_id) ||
      row === undefined ||
      row.inventory_kind !== "user_journey"
    )
      return false;
    const details = row.details;

    if (
      details.intent_ref_id !== journey.intent_ref_id ||
      details.steps.length !== journey.steps.length
    )
      return false;
    ...
    for (const step of journey.steps) {
      ...
      if (
        step.canonical_signatures.some(
          (signature) => !rows.some((row) => row.signature === signature),
        )
      )
        return false;

      if (!step.runtime_evidence_ref_ids.every((runtimeRef) => runtimeIds.has(runtimeRef)))
        return false;
      const resolvedStep = details.steps.find((candidate) => candidate.step_id === step.step_id);

      if (
        resolvedStep === undefined ||
        !step.runtime_evidence_ref_ids.every((runtimeRef) =>
          resolvedStep.runtime_evidence_ref_ids.includes(runtimeRef),
        )
      )
        return false;
    }
  ...
  for (const inventory of input.inventories)
    if (inventory.rows.some((row) => row.inventory_kind !== inventory.inventory_kind)) return false;

  return true;
};

export const coverageFailuresAsReportFailures = (
  ...
  issues.map((failure) => {
    ...
    const output = {
      failure_id:
        runtimeEvidenceRefIds.length === 0
          ...
          : stableId("failure", {
              ...
              runtime_evidence_ref_ids: runtimeEvidenceRefIds,
            }),
      status,
      reason_code: reasonCode,
      row_ids: rowIds,
    ...
    if (!(runtimeEvidenceRefIds.length === 0))
      Object.assign(output, { runtime_evidence_ref_ids: runtimeEvidenceRefIds });
