...
test("API Platform prefix reconciliation covers declared routes and retains generated extras", async () => {
  ...
  writeFileSync(
    join(monoRoot, "apps/server/config/routes.yaml"),
    "api_platform:\n  resource: .\n  type: api_platform\n  prefix: /api\n",
  );
  writeFileSync(
  ...
  try {
    ...
    routeRows.push(
      ...
      runtimeRoute(
        "row-generated-route",
        "_api_/things/{id}{._format}_get",
        "/api/things/{id}.{_format}",
        "GET",
      ),
    );
    ...
    expect(
      operationRows.some((row) => row.status === "covered" && "operation_name" in row.details),
    ).toBe(true);
    expect(
      result.rows.some(
        (row) =>
          ...
          row.details.operation_name === "NotExposed",
      ),
    ).toBe(true);
  } finally {
...
test("omitted StaticContent Get pairs with the default item operation and route evidence", async () => {
  ...
  mkdirSync(legacyRoot);
  mkdirSync(join(monoRoot, "apps/server/config"), { recursive: true });
  mkdirSync(join(monoRoot, "apps/server/src/App/Content/Infrastructure/Entity"), {
    recursive: true,
  });
  ...
  try {
    ...
    const routeRows = [
      runtimeRoute(
        ...
        "_api_/static_contents/{id}{._format}_get",
        "/api/static_contents/{id}.{_format}",
        "GET",
      ),
      runtimeRoute(
        "row-static-content-custom-route",
        "_api_/static-content/by-html-id/{htmlId}_get",
        "/api/static-content/by-html-id/{htmlId}",
        "GET",
      ),
    ];
    const result = await Effect.runPromise(
