...
const invoke = (args: readonly string[]) => spawnSync(
  process.execPath,
  ["--no-env-file", executable, ...args],
  {
    cwd: directory,
    env: { PATH: process.env.PATH, HOME: harnessDirectory, TMPDIR: harnessDirectory, NO_COLOR: "1" },
    encoding: "utf8",
    timeout: 30_000,
  },
);
...
const expectCommandError = (args: readonly string[]) => {
  ...
  expect(report.failures.map((failure: { reason_code: string }) => failure.reason_code))
    .toEqual(["COMMAND_ARGUMENT_ERROR"]);
...
describe("RAT process contract", () => {
  ...
  test("mode boundaries fail before reading authorities or running collectors", () => {
    ...
    expectCommandError([
      ...roots, "--mode", "fixture_injection", "--falsifier", "F0_deterministic_replay",
      "--evidence-register", "missing-evidence",
    ]);
    expectCommandError([
      ...roots, "--mode", "diff", "--intent-register", "missing-intent",
      "--evidence-register", "missing-evidence", "--falsifier", "F0_deterministic_replay",
    ]);
    expect(readdirSync(directory)).toEqual([]);
  });
  ...
  test("last mode and falsifier win across separators in the actual synthetic runner", () => {
    const result = invoke([
      "--root", join(directory, "missing-root"), "--legacy-root", join(directory, "missing-legacy"),
      "--mode", "write", "--", "--mode", "fixture_injection",
      "--falsifier", "F0_deterministic_replay", "--falsifier", "F1_missing_required_source",
    ]);
    expect(result.error).toBeUndefined();
