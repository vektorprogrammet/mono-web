  ...
  if (missing.length > 0) {
    process.stderr.write(
      [
        ...
        "For an explicitly selected setup, invoke `bun run tools/parity/cli.ts -- --help` and provide all required arguments.",
      ].join("\n") + "\n",
    );
    process.exitCode = 12;
  } else if (process.argv.length > 2) {
    process.stderr.write(
      "parity:verify uses its pinned repository inputs and accepts no overrides; invoke tools/parity/cli.ts directly for an explicitly selected run.\n",
    );
    process.exitCode = 12;
  } else {
