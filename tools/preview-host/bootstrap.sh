...
#
# Run inside the tools/preview-host dev-shell:
#   nix develop ./tools/preview-host -c bash ./tools/preview-host/bootstrap.sh
#
...
chmod 600 "$CREDENTIAL_FILE"
bun run "$REPO_ROOT/tools/preview-host/preview-credentials.ts" validate "$CREDENTIAL_FILE"
...
IDENTITY_SEED_PERSONS="$(
  bun run "$REPO_ROOT/tools/preview-host/preview-credentials.ts" seed-json "$CREDENTIAL_FILE"
)"
...
if [[ "$INITIAL_SEED" == 1 ]] || [[ ! -f "$CREDENTIAL_MARKER" ]] || [[ "$(cat "$CREDENTIAL_MARKER")" != "$CREDENTIAL_DIGEST" ]]; then
  ...
  PREVIEW_CREDENTIAL_FILE="$CREDENTIAL_FILE" \
    ...
    bun run "$REPO_ROOT/tools/preview-host/rotate-preview-credentials.ts"
