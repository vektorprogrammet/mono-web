...
const [
  input = resolve(root, "docs/system-walkthrough.mdx"),
  output = resolve(root, "docs/system-walkthrough.html"),
...
const title =
  headings.find((heading) => heading.depth === 1)?.title ?? "Vektorprogrammet system guide";

const toc = renderToStaticMarkup(
  createElement(
    "nav",
    { "aria-label": "On this page", className: "toc" },
    ...
    createElement(
      ...
      headings
        ...
        .map((heading) =>
          createElement(
            "li",
            { key: heading.id, className: heading.depth === 3 ? "toc-subheading" : undefined },
...
for (const match of shell.matchAll(/class="([^"]*)"/g)) {
  for (const candidate of match[1]!.split(/\s+/))
    candidates.add(
      candidate.replaceAll("&amp;", "&").replaceAll("&#x27;", "'").replaceAll("&quot;", '"'),
    );
}
...
const compiler = await compile(await readFile(resolve(directory, "style.css"), "utf8"), {
  base: directory,
  onDependency() {
    /* One-shot build: imported CSS needs no file watcher. */
  },
});
...
console.log(
  `Rendered ${input} → ${output} (${Buffer.byteLength(html)} bytes, ${graphCount} graphs).`,
);
