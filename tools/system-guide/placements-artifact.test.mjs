...
async function fixture(run) {
  ...
  try {
    ...
    git(root, "-c", "user.name=Documentation test", "-c", "user.email=docs@example.invalid", "commit", "--quiet", "-m", "source");
    ...
    await writeFile(resolve(output, receiptName), JSON.stringify({
      format: 1, status: "complete", revision,
...
test("a retained artifact rejects changed copied media, missing output, and extra files", () => fixture(async ({ root, output, revision }) => {
...
test("a retained artifact rejects dirty source and a newer clean public guide revision", () => fixture(async ({ root, output, revision }) => {
  await writeFile(resolve(root, "guide.md"), "Changed public guide\n");
  await assert.rejects(acceptArtifact(root, output, revision));
  git(root, "add", "guide.md");
  git(root, "-c", "user.name=Documentation test", "-c", "user.email=docs@example.invalid", "commit", "--quiet", "-m", "changed guide");
  const changed = git(root, "rev-parse", "HEAD");
  await assert.rejects(acceptArtifact(root, output, changed));
  await assert.rejects(acceptArtifact(root, output, revision));
}));

test("missing and incomplete receipts cannot authorize consumption", () => fixture(async ({ root, output, revision }) => {
  const receipt = JSON.parse(await readFile(resolve(output, receiptName), "utf8"));
  await writeFile(resolve(output, receiptName), JSON.stringify({ ...receipt, status: "interrupted" }));
  await assert.rejects(acceptArtifact(root, output, revision));
...
test("symlinks cannot substitute artifact files, public inputs, or output parents", () => fixture(async ({ root, output, revision, temporary }) => {
...
test("generated API source links resolve to actual repository source", async () => {
  ...
  try {
    const rendered = spawnSync(process.execPath, ["--no-env-file", "tools/system-guide/placements.ts", "generate", output], { cwd: root, encoding: "utf8", timeout: 30000 });
    assert.equal(rendered.status, 0, rendered.stdout + rendered.stderr);
    const links = new Set();
    for (const path of Object.keys(await inventory(output)).filter((path) => path.endsWith(".html"))) {
      await new HTMLRewriter().on("a[href]", { element(element) {
        const href = element.getAttribute("href");
        if (href?.startsWith("file:")) links.add(href);
      } }).transform(new Response(await readFile(resolve(output, path)))).arrayBuffer();
    }
    assert([...links].some((href) => href.includes("/packages/placements/src/service.ts#L")), "The public service has no source link");
    for (const href of links) {
      const url = new URL(href);
      const line = Number(url.hash.slice(2));
      const source = await readFile(fileURLToPath(url), "utf8");
      assert(Number.isInteger(line) && line > 0 && line <= source.split("\n").length, "Source link points outside its file");
    }
  } finally {
    await rm(temporary, { recursive: true, force: true });
  }
