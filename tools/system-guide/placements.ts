...
const revision =
  mode === "ci" || mode === "accept" ? cleanRevision(root, expectedRevision) : undefined;
...
const render = async (directory: string) => {
  ...
  for (const reflection of Object.values(project.reflections)) {
    ...
    reflection.sources = reflection.sources.filter(
      (source) => !source.fileName.split("/").includes("node_modules"),
    );
    ...
    for (const source of reflection.sources) {
      ...
      assert(
        Number.isInteger(source.line) && source.line > 0 && source.line <= lines,
        "Invalid source line",
      );
      assert.equal(
        source.url,
        sourceUrl.replace("{path}", path).replace("{line}", String(source.line)),
        "Invalid source link",
      );
      sourceCount++;
    }
